...
// QueryAnsweringResults is defined locally in this module
...
/// Query answering evaluator for knowledge graph tasks
...
pub enum QueryType {
    /// Simple fact lookup (e.g., "What is the capital of France?")
    FactLookup,
    /// Relationship query (e.g., "Who are the parents of X?")
    RelationshipQuery,
    /// Aggregation query (e.g., "How many children does X have?")
    AggregationQuery,
    /// Comparison query (e.g., "Is X taller than Y?")
    ComparisonQuery,
    /// Multi-hop reasoning (e.g., "What movies did the spouse of X appear in?")
    MultiHopReasoning,
    /// Temporal reasoning (e.g., "What was X's job before becoming Y?")
    TemporalReasoning,
    /// Negation query (e.g., "What animals are not mammals?")
    NegationQuery,
    /// Complex logical query (e.g., intersection, union)
    ComplexLogical,
}
...
pub enum QueryComplexity {
    Simple,    // Single triple patterns
    Medium,    // 2-3 triple patterns
    Complex,   // 4+ triple patterns or complex logic
    Expert,    // Requires advanced reasoning
}

/// Query answering evaluation metrics
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub enum QueryAnsweringMetric {
    /// Exact match accuracy
    ExactMatch,
    /// Partial match score (F1)
    PartialMatch,
    /// Answer completeness
    Completeness,
    /// Answer precision
    ...
    /// Answer recall
    Recall,
    /// Mean Reciprocal Rank of correct answers
    MRR,
    /// Hits@K for answer ranking
    HitsAtK(usize),
}
...
pub struct QueryAnsweringResults {
    ...
    pub results_by_type: HashMap<QueryType, QueryTypeResults>,
...
pub struct ReasoningAnalysis {
    ...
    /// Aggregation reasoning accuracy
...
impl ApplicationQueryAnsweringEvaluator {
    ...
    pub fn new() -> Self {
        Self {
            ...
            metrics: vec![
                ...
                QueryAnsweringMetric::HitsAtK(1),
    ...
    /// Add question-answer pair for evaluation
    pub fn add_qa_pair(&mut self, qa_pair: QuestionAnswerPair) {
        self.qa_pairs.push(qa_pair);
    }

    /// Generate synthetic QA pairs for testing
    pub fn generate_synthetic_qa_pairs(&mut self, num_pairs: usize) {
        // Generate diverse QA pairs for different query types
        for i in 0..num_pairs {
            let qa_pair = match i % 8 {
                0 => self.create_fact_lookup_pair(i),
                1 => self.create_relationship_pair(i),
                2 => self.create_aggregation_pair(i),
                3 => self.create_comparison_pair(i),
                4 => self.create_multi_hop_pair(i),
                5 => self.create_temporal_pair(i),
                6 => self.create_negation_pair(i),
                _ => self.create_complex_logical_pair(i),
            };
    ...
    pub async fn evaluate(
        ...
        let qa_pairs_to_evaluate: Vec<_> = self
            .qa_pairs
            .iter()
            .take(config.num_query_tests)
            .collect();

        if qa_pairs_to_evaluate.is_empty() {
            // Generate some test pairs if none exist
            let mut temp_evaluator = self.clone();
            temp_evaluator.generate_synthetic_qa_pairs(config.num_query_tests);
            return Box::pin(temp_evaluator.evaluate(model, config)).await;
        }
        ...
        for query_type in &self.query_types {
            ...
            if !type_results.is_empty() {
                ...
                let avg_response_time = type_results.iter().map(|r| r.response_time).sum::<f64>() 
                    / type_results.len() as f64;
                let success_rate = type_results.iter().filter(|r| r.accuracy > 0.5).count() as f64 
                    / type_results.len() as f64;

                results_by_type.insert(query_type.clone(), QueryTypeResults {
        ...
        for complexity in &[QueryComplexity::Simple, QueryComplexity::Medium, QueryComplexity::Complex, QueryComplexity::Expert] {
            ...
            if !complexity_results.is_empty() {
                ...
                let completion_rate = complexity_results.iter().filter(|r| !r.predicted_answers.is_empty()).count() as f64 
                    / complexity_results.len() as f64;

                results_by_complexity.insert(complexity.clone(), ComplexityResults {
        ...
        let overall_accuracy = if per_query_results.is_empty() {
            ...
            per_query_results.iter().map(|r| r.accuracy).sum::<f64>() / per_query_results.len() as f64
        };
    ...
    async fn evaluate_single_query(
        ...
        // Calculate accuracy
        let accuracy = self.calculate_answer_accuracy(
            &qa_pair.answer_entities,
            &predicted_answers,
        );
    ...
    async fn answer_query_with_embeddings(
        ...
        for entity in entities.iter().take(50) {
            if let Ok(entity_embedding) = model.get_entity_embedding(entity) {
                // Simple scoring based on name similarity (in practice, would use more sophisticated methods)
                let mut score = 0.0;
        ...
        match metric {
            ...
            QueryAnsweringMetric::Precision => {
                ...
                for result in results {
                    if !result.predicted_answers.is_empty() {
                        let expected_set: HashSet<&String> = result.expected_answers.iter().collect();
                        let predicted_set: HashSet<&String> = result.predicted_answers.iter().collect();
                        let correct = expected_set.intersection(&predicted_set).count();
                        let precision = correct as f64 / result.predicted_answers.len() as f64;
            ...
            QueryAnsweringMetric::Recall => {
                ...
                for result in results {
                    if !result.expected_answers.is_empty() {
                        let expected_set: HashSet<&String> = result.expected_answers.iter().collect();
                        let predicted_set: HashSet<&String> = result.predicted_answers.iter().collect();
                        let correct = expected_set.intersection(&predicted_set).count();
                        let recall = correct as f64 / result.expected_answers.len() as f64;
                        total_recall += recall;
                        count += 1;
                    }
                }
            ...
            QueryAnsweringMetric::MRR => {
                ...
                for result in results {
                    ...
                    let mut rank = 0;
                    for (i, predicted) in result.predicted_answers.iter().enumerate() {
                        if result.expected_answers.contains(predicted) {
                            rank = i + 1;
                            break;
                        }
                    }
                ...
                Ok(total_rr / results.len() as f64)
            }
            QueryAnsweringMetric::HitsAtK(k) => {
                let hits = results
                    .iter()
                    .filter(|r| {
                        // Check if any expected answer is in top-k predictions
                        r.predicted_answers
                            .iter()
                            .take(*k)
                            .any(|pred| r.expected_answers.contains(pred))
                    })
                    .count() as f64;
                Ok(hits / results.len() as f64)
            }
        }
    }

    /// Analyze reasoning capabilities
    fn analyze_reasoning_capabilities(
        &self,
        results: &[QueryResult],
        ...
        let multi_hop_accuracy = if multi_hop_results.is_empty() {
            ...
            multi_hop_results.iter().map(|r| r.accuracy).sum::<f64>() / multi_hop_results.len() as f64
        };
        ...
        let logical_results: Vec<_> = results
            .iter()
            .filter(|r| matches!(r.query_type, QueryType::ComplexLogical | QueryType::NegationQuery))
            .collect();
        ...
        let aggregation_accuracy = if aggregation_results.is_empty() {
            ...
            aggregation_results.iter().map(|r| r.accuracy).sum::<f64>() / aggregation_results.len() as f64
        };

        let overall_reasoning_score = (multi_hop_accuracy + temporal_accuracy + logical_accuracy + aggregation_accuracy) / 4.0;
    ...
    fn create_fact_lookup_pair(&self, id: usize) -> QuestionAnswerPair {
        QuestionAnswerPair {
            ...
            structured_query: Some(format!("SELECT ?type WHERE {{ entity{} rdf:type ?type }}", id)),
    ...
    fn create_relationship_pair(&self, id: usize) -> QuestionAnswerPair {
        QuestionAnswerPair {
            ...
            structured_query: Some(format!("SELECT ?related WHERE {{ entity{} ?relation ?related }}", id)),
            answer_entities: vec![format!("entity{}", (id + 1) % 10), format!("entity{}", (id + 2) % 10)],
            answer_literals: vec![],
            complexity: QueryComplexity::Simple,
    ...
    fn create_aggregation_pair(&self, id: usize) -> QuestionAnswerPair {
        QuestionAnswerPair {
            ...
            structured_query: Some(format!("SELECT (COUNT(?relation) as ?count) WHERE {{ entity{} ?relation ?object }}", id)),
            answer_entities: vec![],
    ...
    fn create_comparison_pair(&self, id: usize) -> QuestionAnswerPair {
        QuestionAnswerPair {
            ...
            structured_query: Some(format!("ASK {{ entity{} :size ?s1 . entity{} :size ?s2 . FILTER(?s1 > ?s2) }}", id, id + 1)),
            answer_entities: vec![],
            answer_literals: vec![if id % 2 == 0 { "true".to_string() } else { "false".to_string() }],
            complexity: QueryComplexity::Medium,
    ...
    fn create_negation_pair(&self, id: usize) -> QuestionAnswerPair {
        QuestionAnswerPair {
            ...
            structured_query: Some(format!("SELECT ?entity WHERE {{ ?entity rdf:type ?type . FILTER(?type != Type{}) }}", id % 3)),
            answer_entities: vec![format!("entity{}", (id + 4) % 10), format!("entity{}", (id + 5) % 10)],
            answer_literals: vec![],
            complexity: QueryComplexity::Complex,
    ...
    fn create_complex_logical_pair(&self, id: usize) -> QuestionAnswerPair {
        QuestionAnswerPair {
            question: format!("What entities are both Type{} and connected to entity{}?", id % 2, id),
            structured_query: Some(format!("SELECT ?entity WHERE {{ ?entity rdf:type Type{} . entity{} ?relation ?entity }}", id % 2, id)),
...
impl ApplicationTaskEvaluator {
    ...
    pub async fn evaluate_all_tasks(
        ...
        if self.config.enable_query_answering_eval {
            query_answering_results = Some(self.query_answering_evaluator.evaluate(model, &self.config).await?);
        }
        ...
        if let Some(ref qa_results) = query_answering_results {
            scores.extend(qa_results.metric_scores.values());
        }
...
mod tests {
    ...
    async fn test_application_task_evaluator() {
        ...
        let model = TransE::new(Default::default());
