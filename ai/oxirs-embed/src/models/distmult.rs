...
impl DistMult {
    ...
    pub fn new(config: ModelConfig) -> Self {
        ...
        let dropout_rate = config.model_params.get("dropout_rate").copied().unwrap_or(0.0);
    ...
    fn initialize_embeddings(&mut self) {
        ...
        debug!("Initialized DistMult embeddings: {} entities, {} relations, {} dimensions",
    ...
    fn compute_gradients(
        ...
        match self.loss_function {
            LossFunction::Logistic => {
                ...
                self.add_triple_gradients(pos_triple, pos_grad_coeff, &mut entity_grads, &mut relation_grads);
                self.add_triple_gradients(neg_triple, neg_grad_coeff, &mut entity_grads, &mut relation_grads);
            }
            LossFunction::MarginRanking => {
                // Margin ranking loss: max(0, margin + neg_score - pos_score)
                let margin = self.base.config.model_params.get("margin").copied().unwrap_or(1.0);
                let loss = margin + neg_score - pos_score;
                
                if loss > 0.0 {
                    // Gradients: -1 for positive triple, +1 for negative triple
                    self.add_triple_gradients(pos_triple, -1.0, &mut entity_grads, &mut relation_grads);
                    self.add_triple_gradients(neg_triple, 1.0, &mut entity_grads, &mut relation_grads);
                }
            }
    ...
    async fn train_epoch(&mut self, learning_rate: f64) -> Result<f64> {
        ...
        for batch_triples in shuffled_triples.chunks(self.base.config.batch_size) {
            ...
            for &pos_triple in batch_triples {
                ...
                for neg_triple in neg_samples {
                    ...
                    let triple_loss = match self.loss_function {
                        ...
                        LossFunction::SquaredLoss => {
                            (1.0 - pos_score).powi(2) + neg_score.powi(2)
                        }
                    };
            ...
            if self.base.config.model_params.get("normalize_entities").copied().unwrap_or(0.0) > 0.0 {
                normalize_embeddings(&mut self.entity_embeddings);
            }
...
impl EmbeddingModel for DistMult {
    ...
    fn add_triple(&mut self, triple: Triple) -> Result<()> {
        ...
        if predicate_str.contains("parent") || predicate_str.contains("child") || 
           predicate_str.contains("born") || predicate_str.contains("founder") {
            warn!("DistMult may not handle asymmetric relation well: {}", predicate_str);
        }
