    ...
    pub fn new(config: VisionEncoderConfig) -> Self {
        ...
        for (i, &filter_size) in config.cnn_config.filter_sizes.iter().enumerate() {
            ...
            cnn_parameters.insert(
                layer_name,
                Array4::from_shape_fn(weight_shape, |_| (random.gen::<f32>() - 0.5) * 0.1),
            );
        }
...
impl MetaLearner {
    pub fn new(config: MetaLearningConfig) -> Self {
        ...
        meta_parameters.insert(
            ...
            Array2::from_shape_fn((512, 512), |_| (random.gen::<f32>() - 0.5) * 0.1),
        );

        let mut random = Random::default();
        task_parameters.insert(
            "adaptation_weights".to_string(),
            Array2::from_shape_fn((256, 512), |_| (random.gen::<f32>() - 0.5) * 0.1),
        );
...
impl VisionLanguageGraphModel {
    ...
    async fn train_epoch(&mut self) -> Result<f64> {
        ...
        let mut random = Random::default();
        let vision_loss = 0.1 * random.gen::<f64>();
        let language_loss = 0.1 * random.gen::<f64>();
        let graph_loss = 0.1 * random.gen::<f64>();
        let fusion_loss = 0.1 * random.gen::<f64>();
...
mod tests {
    ...
    fn test_vision_encoder_image_encoding() {
        ...
        let mut random = Random::default();
        let image = Array3::from_shape_fn((224, 224, 3), |_| random.gen::<f32>());
    ...
    fn test_graph_encoder_graph_encoding() {
        ...
        let mut random = Random::default();
        let node_features = Array2::from_shape_fn((5, node_dim), |_| random.gen::<f32>());
        let edge_features = Array2::from_shape_fn((10, edge_dim), |_| random.gen::<f32>());
    ...
    async fn test_unified_embedding_generation() {
        ...
        let node_features = Array2::from_shape_fn((3, 256), |_| random.gen::<f32>());
        let edge_features = Array2::from_shape_fn((6, 128), |_| random.gen::<f32>());
    ...
    fn test_zero_shot_prediction() {
        ...
        let mut random = Random::default();
        let query = Array1::from_shape_fn(768, |_| random.gen::<f32>());

        let mut prototypes = HashMap::new();
        let mut random = Random::default();
        prototypes.insert(
            "class1".to_string(),
            Array1::from_shape_fn(768, |_| random.gen::<f32>()),
        );
        let mut random = Random::default();
        prototypes.insert(
            "class2".to_string(),
    ...
    fn test_few_shot_adaptation() {
        ...
        let support_examples = vec![
            (
                Array1::from_shape_fn(512, |_| random.gen::<f32>()),
                "cat".to_string(),
            ),
            (
                Array1::from_shape_fn(512, |_| random.gen::<f32>()),
