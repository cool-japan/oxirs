...
impl AttentionMechanism {
    ...
    pub fn focus_on(&mut self, concept: &str, intensity: f32) {
        ...
        self.attention_memory.push((concept.to_string(), intensity, now));
    ...
    fn apply_decay(&mut self) {
        ...
        self.attention_memory.retain(|(_, intensity, _)| *intensity > 0.01);
    }

    /// Get current attention distribution
    pub fn get_attention_distribution(&self) -> Vec<(String, f32)> {
        let mut attention: Vec<_> = self.focus_weights.iter()
            .map(|(k, &v)| (k.clone(), v))
            .collect();
...
impl WorkingMemory {
    ...
    pub fn get_related_concepts(&self, concept: &str) -> Vec<(String, f32)> {
        self.concept_relationships.iter()
...
impl ConsciousnessAwareEmbedding {
    ...
    pub fn generate_conscious_embedding(&mut self, entity: &str, context: &[String]) -> Result<Array1<f32>> {
        ...
        let base_embedding = self.embeddings.get(entity).cloned()
            .unwrap_or_else(|| self.create_base_embedding(entity));

        // Apply consciousness-aware modifications
        let conscious_embedding = self.apply_consciousness_enhancement(
            &base_embedding, 
            entity, 
            context
        )?;
        ...
        self.working_memory.add_concept(entity.to_string(), conscious_embedding.clone())?;

        // Store enhanced embedding
        self.embeddings.insert(entity.to_string(), conscious_embedding.clone());
    ...
    fn apply_consciousness_enhancement(
        ...
        for (related_concept, relationship_strength) in related_concepts {
            if let Some(related_embedding) = self.working_memory.active_concepts.get(&related_concept) {
    ...
    fn reflect_on_embedding(&mut self, entity: &str, embedding: &Array1<f32>) {
        ...
        self.learning_history.push((entity.to_string(), embedding.clone(), quality_score));
    ...
    fn update_consciousness_state(&mut self) -> Result<()> {
        ...
        if !self.learning_history.is_empty() {
            let recent_embeddings = self.learning_history.iter()
                .rev()
            ...
            self.consciousness_state = &self.consciousness_state * 0.8f32 + &average_recent * 0.2f32;
    ...
    pub fn get_consciousness_insights(&self) -> ConsciousnessInsights {
        ...
        let active_concepts: Vec<String> = self.working_memory.active_concepts.keys().cloned().collect();
        let recent_reflections: Vec<String> = self.meta_cognition.reflection_history.iter()
            .rev()
            .take(5)
...
mod tests {
    ...
    fn test_consciousness_aware_embedding() {
        ...
        assert!(model.working_memory.active_concepts.contains_key("test_entity"));
    }
