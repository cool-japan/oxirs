...
impl ModelRepository {
    ...
    pub fn load_model(&self, name: &str) -> Result<Box<dyn EmbeddingModel>> {
        let model_info = self.models.get(name)
            ...
            .filter_map(|entry| {
                entry.ok().and_then(|e| {
                    ...
                    if path.extension().and_then(|s| s.to_str()) == Some("bin") {
                        e.metadata().ok().map(|m| (path, m.modified().unwrap_or(std::time::UNIX_EPOCH)))
                    } else {
                        None
                    }
                })
    ...
    pub fn export_to_csv(
        ...
        for entity in model.get_entities() {
            if let Ok(embedding) = model.get_entity_embedding(&entity) {
                ...
                writeln!(file, "entity,{},{},\"{}\"", 
                        entity, embedding.dimensions, values.join(","))?;
            }
        }
        ...
        for relation in model.get_relations() {
            if let Ok(embedding) = model.get_relation_embedding(&relation) {
                ...
                writeln!(file, "relation,{},{},\"{}\"", 
                        relation, embedding.dimensions, values.join(","))?;
            }
        }
