...
use oxirs_embed::{
    // Models
    TransE, ComplEx, DistMult, RotatE,
    // Configuration and core types
    ModelConfig, NamedNode, Triple,
    // Evaluation
...
use oxirs_embed::api::{ApiState, ApiConfig, start_server};
...
fn create_sample_knowledge_graph() -> Result<Vec<Triple>> {
    ...
    let entities = vec![
        ...
        "IBM", "Google", "Microsoft", "Apple", "Tesla", "SpaceX",
        "USA", "Canada", "Germany", "Japan", "France", "UK",
        "AI", "ML", "NLP", "Computer_Vision", "Robotics", "Blockchain",
    ];
    
    let relations = vec![
        "knows", "works_for", "located_in", "ceo_of", "founded", "friend_of",
        "expert_in", "collaborates_with", "competes_with", "acquired_by",
        "similar_to", "part_of", "develops", "researches", "invented",
    ];
    ...
    for (i, entity_i) in entities.iter().enumerate() {
        for (j, relation_j) in relations.iter().enumerate() {
            for (k, entity_k) in entities.iter().enumerate() {
                if i != k && (i + j + k) % 3 == 0 { // Create some sparsity
...
async fn benchmark_models(triples: &[Triple]) -> Result<HashMap<String, ModelPerformance>> {
    ...
    results.insert("TransE".to_string(), benchmark_transe(triples, config.clone()).await?);
    
    // Benchmark ComplEx
    info!("Benchmarking ComplEx...");
    results.insert("ComplEx".to_string(), benchmark_complex(triples, config.clone()).await?);
    
    // Benchmark DistMult
    info!("Benchmarking DistMult...");
    results.insert("DistMult".to_string(), benchmark_distmult(triples, config.clone()).await?);
    
    // Benchmark RotatE
    info!("Benchmarking RotatE...");
    results.insert("RotatE".to_string(), benchmark_rotate(triples, config.clone()).await?);
...
async fn benchmark_transe(triples: &[Triple], config: ModelConfig) -> Result<ModelPerformance> {
    ...
    let train_triples: Vec<_> = triples[..train_size].iter()
        .map(|t| (t.subject.to_string(), t.predicate.to_string(), t.object.to_string()))
        .collect();
    let test_triples: Vec<_> = triples[train_size..].iter()
        .map(|t| (t.subject.to_string(), t.predicate.to_string(), t.object.to_string()))
        .collect();
...
async fn benchmark_model_impl<M: oxirs_embed::EmbeddingModel>(model: &mut M, triples: &[Triple]) -> Result<ModelPerformance> {
    // Add training data
    for triple in triples {
        model.add_triple(triple.clone())?;
    }
...
fn check_requirements(eval_results: &oxirs_embed::evaluation::EvaluationResults, training_time: f64, inference_latency: f64) -> bool {
...
fn create_large_knowledge_graph() -> Result<Vec<Triple>> {
    ...
    for i in 0..1000 {  // 1000 entities
        for j in 0..20 {  // 20 relations
            for k in 0..10 {  // 10 connections per entity-relation pair
                if (i + j + k) % 5 == 0 {  // Create sparsity
                    ...
                    let object = NamedNode::new(&format!("http://example.org/entity_{}", (i + k + 1) % 1000))?;
...
fn generate_report(results: &BenchmarkResults) {
    ...
    for (model_name, perf) in &results.model_performances {
        println!("{:<12} {:>10.2} {:>8.2} {:>6.3} {:>6.3} {:>6.3} {:>8}", 
                 ...
                 perf.hits_at_1,
                 perf.hits_at_10,
                 if perf.passes_requirements { "✅" } else { "❌" });
    }
    
    println!("\n🚀 API Server Performance:");
    println!("  Requests/second: {:.0}", results.api_performance.requests_per_second);
    println!("  P95 Latency: {:.1}ms", results.api_performance.p95_latency_ms);
    println!("  Cache Hit Rate: {:.1}%", results.api_performance.cache_hit_rate * 100.0);
    
    println!("\n⚡ Caching Performance:");
    println!("  Cache Hit Rate: {:.1}%", results.caching_performance.cache_hit_rate * 100.0);
    println!("  Speedup Factor: {:.1}x", results.caching_performance.cache_speedup_factor);
    
    println!("\n📈 Scalability Metrics:");
    println!("  Entities Processed: {}", results.scalability_metrics.entities_processed);
    println!("  Relations Processed: {}", results.scalability_metrics.relations_processed);
    println!("  Triples Processed: {}", results.scalability_metrics.triples_processed);
    println!("  Throughput: {:.0} embeddings/second", results.scalability_metrics.throughput_embeddings_per_second);
    
    println!("\n✅ TODO.md Requirements Assessment:");
    let fast_inference = results.model_performances.values().all(|p| p.inference_latency_ms < 100.0);
    ...
    println!("  Fast Inference (<100ms): {}", if fast_inference { "✅" } else { "❌" });
    println!("  High-Quality Embeddings: {}", if good_quality { "✅" } else { "❌" });
    println!("  Scalability: {}", if scalable { "✅" } else { "❌" });
    println!("  High Throughput (1K+ RPS): {}", if high_throughput { "✅" } else { "❌" });
