...
//! user emotions and satisfaction levels.

use anyhow::{Context, Result};
use scirs2_stats::stats::mean;
use scirs2_text::sentiment::{SentimentModel, SentimentScore};
...
/// Sentiment analyzer using lexicon-based and ML approaches
...
impl SentimentAnalyzer {
    ...
    pub fn new(config: SentimentConfig) -> Result<Self> {
        ...
        info!(
            "Initialized sentiment analyzer with {} positive and {} negative words",
            positive_words.len(),
            negative_words.len()
        );
    ...
    fn extract_key_phrases(&self, message: &str, words: &[&str]) -> Vec<String> {
