...
use anyhow::Result;
use scirs2_stats::ranking::{RankingAlgorithm, RankingScore};
use scirs2_stats::stats::{mean, normalize, std_dev};
...
impl ResultRanker {
    ...
    fn rank_bm25(&self, results: &mut [RetrievalResult], query: &str) -> Result<()> {
        ...
        for result in results.iter_mut() {
            let doc_length = result.triple.to_string().split_whitespace().count();
            let mut score = 0.0;

            for term in &query_terms {
                let tf = self.term_frequency(&result.triple.to_string(), term);
    ...
    fn rank_mmr(&self, results: &mut [RetrievalResult], query: &str) -> Result<()> {
        if results.is_empty() {
            return Ok(());
        }
        ...
        while !remaining.is_empty() && selected.len() < results.len() {
            ...
            for (idx, result) in remaining.iter().enumerate() {
                ...
                let max_similarity = selected
                    .iter()
                    .map(|s| {
                        self.calculate_similarity(&result.triple.to_string(), &s.triple.to_string())
                    })
                    .fold(0.0f32, f32::max);
        ...
        results.copy_from_slice(&selected);
        Ok(())
    }
    ...
    fn rank_pagerank(&self, results: &mut [RetrievalResult]) -> Result<()> {
        ...
        for result in results.iter_mut() {
            ...
            result.score = result.score * (1.0 + 0.1 * entity_count as f32);
        }
    ...
    fn rank_rrf(&self, results: &mut [RetrievalResult]) -> Result<()> {
        ...
        for (rank, result) in results.iter_mut().enumerate() {
            result.score = 1.0 / (k + rank as f32 + 1.0);
        }
    ...
    fn rerank_top_k(&self, results: &mut [RetrievalResult], query: &str) -> Result<()> {
        ...
        for i in 0..top_k {
            ...
            let enhanced_score = self.compute_enhanced_score(&results[i], query)?;
            results[i].score = enhanced_score;
        }

        // Re-sort after reranking
        results[..top_k].sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap());
        Ok(())
    }
    ...
    fn extract_features(
        ...
        let doc = result.triple.to_string();
    ...
    fn compute_enhanced_score(&self, result: &RetrievalResult, query: &str) -> Result<f32> {
        ...
        Ok(result.score + exact_match_bonus + position_bonus * 0.1 + quality_score * 0.1)
    }
    ...
    fn calculate_avg_doc_length(&self, results: &[RetrievalResult]) -> f32 {
        ...
        let total: usize = results
            .iter()
            .map(|r| r.triple.to_string().split_whitespace().count())
            .sum();
...
mod tests {
    use super::*;
    use crate::rag::types::Triple;

    fn create_test_result(text: &str, score: f32) -> RetrievalResult {
        RetrievalResult {
            triple: Triple {
    ...
    fn test_bm25_ranking() {
        ...
        assert!(results[0].triple.subject.contains("Inception"));
    }
