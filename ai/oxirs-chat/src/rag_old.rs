...
use oxirs_vec::{
    ...
    index::{AdvancedVectorIndex, DistanceMetric, IndexConfig, IndexType, SearchResult as VecSearchResult},
    similarity,
    VectorIndex,
};
use oxirs_embed::{
...
impl EnhancedVectorIndex {
    ...
    pub async fn add_document(
        ...
        let vector = embedding.into_iter().next().ok_or_else(|| {
            anyhow!("Failed to generate embedding for content")
        })?;
    ...
    pub async fn search(
        ...
        let query_vector = query_embedding.into_iter().next().ok_or_else(|| {
            anyhow!("Failed to generate embedding for query")
        })?;
