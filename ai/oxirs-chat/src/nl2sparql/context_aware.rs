...
use super::types::{QueryIntent, SPARQLGenerationResult};
use crate::messages::Message;
use crate::nlp::entity_extraction::ExtractedEntity;
...
impl ContextAwareGenerator {
    ...
    pub fn generate_with_context(
        ...
        Ok(SPARQLGenerationResult {
            ...
            prefixes: context.discovered_schema.prefixes.clone(),
    ...
    fn generate_count_query(&self, query: &str, context: &ConversationContext) -> Result<String> {
        ...
        sparql.push_str(&format!("  ?s rdf:type ?type .\n"));
    ...
    fn generate_select_query(&self, query: &str, context: &ConversationContext) -> Result<String> {
        let entities = self.extract_entities(query)?;
    ...
    fn enhance_with_context(
        ...
        if context
            ...
            .unwrap_or(false)
        {
            if !sparql.contains("ORDER BY") && sparql.contains("SELECT") {
    ...
    fn learn_from_query(&self, sparql: &str, context: &mut ConversationContext) {
        ...
        if let Some(class_match) = sparql.find("rdf:type") {
...
impl Default for ConversationContext {
    fn default() -> Self {
        Self {
            ...
            discovered_schema: DiscoveredSchema::default(),
            current_topic: None,
        }
    }
}
