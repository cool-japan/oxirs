...
type ReplayBufferData = (Pattern, HashMap<(String, String), CorrelationType>);
...
impl NeuralPatternLearner {
    ...
    async fn self_attention_forward(
        ...
        if let Some(first_output) = attention_outputs.first() {
            ...
            for output in attention_outputs.iter().skip(1) {
                // TODO: Fix concatenate API once scirs2-core has proper array concatenation
                // For now, just use the first output
