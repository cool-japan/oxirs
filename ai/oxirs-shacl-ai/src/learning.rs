...
use oxirs_shacl::{
    Shape, ShapeId, ShapeType, PropertyPath, Target, Constraint, ConstraintComponentId,
    constraints::*,
    shapes::ShapeFactory,
};
...
impl ShapeLearner {
    ...
    pub fn learn_shapes_from_store(&mut self, store: &Store, graph_name: Option<&str>) -> Result<Vec<Shape>> {
    ...
    pub fn learn_shapes_from_patterns(&mut self, store: &Store, patterns: &[Pattern], graph_name: Option<&str>) -> Result<Vec<Shape>> {
    ...
    fn learn_shape_for_class(&mut self, store: &Store, class: &NamedNode, graph_name: Option<&str>) -> Result<Shape> {
        tracing::debug!("Learning shape for class: {}", class.as_str());
        
        // Create a node shape for this class
        let shape_id = ShapeId::new(format!("{}Shape", class.as_str().replace(['/', ':', '#'], "_")));
        ...
        for property in &properties {
            ...
            let (min_count, max_count) = self.analyze_property_cardinality(store, class, property, graph_name)?;
            
            // Create property shape
            let property_path = PropertyPath::predicate(property.clone());
            let property_shape_id = ShapeId::new(format!("{}_{}PropertyShape", 
            ...
            if let Some(min) = min_count {
                property_shape.add_constraint(ConstraintComponentId::new("sh:minCount"), 
                    Constraint::MinCount(MinCountConstraint { min_count: min }));
            }
            
            if let Some(max) = max_count {
                property_shape.add_constraint(ConstraintComponentId::new("sh:maxCount"), 
            ...
            if let Ok(datatype) = self.discover_common_datatype(store, class, property, graph_name) {
                property_shape.add_constraint(ConstraintComponentId::new("sh:datatype"), 
                    Constraint::Datatype(DatatypeConstraint { datatype_iri: datatype }));
            }
        ...
        tracing::debug!("Learned shape {} with {} properties", shape_id.as_str(), properties.len());
    ...
    fn discover_properties_for_class(&self, store: &Store, class: &NamedNode, graph_name: Option<&str>) -> Result<Vec<NamedNode>> {
    ...
    fn analyze_property_cardinality(&self, store: &Store, class: &NamedNode, property: &NamedNode, graph_name: Option<&str>) -> Result<(Option<u32>, Option<u32>)> {
        ...
        let query = if let Some(graph) = graph_name {
            format!(r#"
                ...
                GROUP BY ?instance
            "#, graph, class.as_str(), property.as_str())
        } else {
            format!(r#"
                SELECT ?instance (COUNT(?value) as ?count) WHERE {{
                    ?instance a <{}> .
    ...
    fn discover_common_datatype(&self, store: &Store, class: &NamedNode, property: &NamedNode, graph_name: Option<&str>) -> Result<NamedNode> {
        let query = if let Some(graph) = graph_name {
            format!(r#"
        ...
        NamedNode::new("http://www.w3.org/2001/XMLSchema#string")
            .map_err(|e| ShaclAiError::ShapeLearning(format!("Failed to create default datatype: {}", e)))
    }
    
    /// Learn constraints for a specific property of a class
    fn learn_property_constraints(&mut self, store: &Store, class: &NamedNode, property: &NamedNode, graph_name: Option<&str>) -> Result<Vec<(ConstraintComponentId, Constraint)>> {
        let mut constraints = Vec::new();
        
        // Learn cardinality constraints
        if let Ok(cardinality) = self.learn_cardinality_constraints(store, class, property, graph_name) {
    ...
    fn learn_cardinality_constraints(&self, store: &Store, class: &NamedNode, property: &NamedNode, graph_name: Option<&str>) -> Result<Vec<(ConstraintComponentId, Constraint)>> {
        let mut constraints = Vec::new();
    ...
    fn learn_datatype_constraints(&self, store: &Store, class: &NamedNode, property: &NamedNode, graph_name: Option<&str>) -> Result<Vec<(ConstraintComponentId, Constraint)>> {
        let mut constraints = Vec::new();
        ...
        if let Some((dominant_datatype, dominant_count)) = datatype_counts.first() {
            ...
            if confidence >= self.config.min_confidence {
                constraints.push((
                    ...
                    Constraint::Datatype(DatatypeConstraint { datatype_iri: dominant_datatype.clone() })
                ));
            }
        }
    ...
    fn learn_range_constraints(&self, store: &Store, class: &NamedNode, property: &NamedNode, graph_name: Option<&str>) -> Result<Vec<(ConstraintComponentId, Constraint)>> {
        let mut constraints = Vec::new();
        ...
        if values.len() >= 10 { // Only add range constraints if we have enough data
            ...
            constraints.push((
                ...
                Constraint::MinInclusive(MinInclusiveConstraint { min_value: min_value.clone() })
            ));
            
            constraints.push((
                ConstraintComponentId::new("sh:MaxInclusiveConstraintComponent"),
                Constraint::MaxInclusive(MaxInclusiveConstraint { max_value: max_value.clone() })
            ));
        }
    ...
    fn pattern_to_shape(&mut self, _store: &Store, pattern: &Pattern, _graph_name: Option<&str>) -> Result<Shape> {
        // Create shape based on pattern type
        match pattern {
            Pattern::ClassUsage { class, confidence, .. } if *confidence >= self.config.min_confidence => {
                let shape_id = ShapeId::new(format!("{}Shape", class.as_str()));
                Ok(ShapeFactory::node_shape_with_class(shape_id, class.clone()))
            }
            Pattern::PropertyUsage { property, confidence, .. } if *confidence >= self.config.min_confidence => {
                let shape_id = ShapeId::new(format!("{}PropertyShape", property.as_str()));
                let property_path = PropertyPath::predicate(property.clone());
                Ok(Shape::property_shape(shape_id, property_path))
            }
            _ => Err(ShaclAiError::ShapeLearning("Pattern does not meet confidence threshold".to_string()))
        }
    }
    ...
    pub fn train_model(&mut self, training_data: &ShapeTrainingData) -> Result<crate::ModelTrainingResult> {
        tracing::info!("Training shape learning model on {} examples", training_data.examples.len());
