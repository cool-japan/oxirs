...
use oxirs_shacl::{
    constraints::*, Constraint, ConstraintComponentId, PropertyPath, Shape, ShapeId, Target, ValidationConfig,
    ValidationReport,
};
...
impl OptimizationEngine {
    ...
    fn optimize_constraint_ordering(
        ...
        for shape in shapes {
            ...
            constraint_scores.sort_by(|a, b| b.2.partial_cmp(&a.2).unwrap_or(std::cmp::Ordering::Equal));
    ...
    fn estimate_constraint_selectivity(&self, constraint: &Constraint, store: &Store) -> Result<f64> {
        match constraint {
    ...
    fn optimize_shape_merging(&self, shapes: Vec<Shape>) -> Result<Vec<Shape>> {
        tracing::debug!("Analyzing {} shapes for merging opportunities", shapes.len());
        ...
        for candidate in merge_candidates {
            if candidate.similarity_score >= self.config.algorithms.shape_merge_threshold {
                if let Some(merged_shape) = self.merge_shapes(&candidate, &optimized_shapes)? {
                    ...
                    optimized_shapes.retain(|s| s.id != candidate.shape1_id && s.id != candidate.shape2_id);
    ...
    fn calculate_shape_similarity(&self, shape1: &Shape, shape2: &Shape) -> Result<f64> {
        ...
        // 1. Target similarity
        let target_similarity = self.calculate_target_similarity(&shape1.targets, &shape2.targets)?;
        similarity_scores.push(target_similarity * 0.3); // 30% weight
        
        // 2. Constraint similarity
        let constraint_similarity = self.calculate_constraint_similarity(&shape1.constraints, &shape2.constraints)?;
    ...
    fn calculate_constraint_similarity(&self, constraints1: &IndexMap<ConstraintComponentId, Constraint>, constraints2: &IndexMap<ConstraintComponentId, Constraint>) -> Result<f64> {
        if constraints1.is_empty() && constraints2.is_empty() {
            return Ok(1.0);
        }
        
        let all_constraint_keys: HashSet<_> = constraints1.keys().chain(constraints2.keys()).collect();
    ...
    fn are_constraints_similar(&self, c1: &Constraint, c2: &Constraint) -> Result<bool> {
        match (c1, c2) {
            (Constraint::Datatype(d1), Constraint::Datatype(d2)) => Ok(d1.datatype_iri == d2.datatype_iri),
            (Constraint::NodeKind(n1), Constraint::NodeKind(n2)) => Ok(n1.node_kind == n2.node_kind),
            (Constraint::MinCount(m1), Constraint::MinCount(m2)) => Ok((m1.min_count as i32 - m2.min_count as i32).abs() <= 2),
            (Constraint::MaxCount(m1), Constraint::MaxCount(m2)) => Ok((m1.max_count as i32 - m2.max_count as i32).abs() <= 2),
    ...
    fn merge_shapes_union(&self, shape1: &Shape, shape2: &Shape) -> Result<Shape> {
        let merged_id = ShapeId::new(format!("{}_{}_merged", shape1.id.as_str(), shape2.id.as_str()));
    ...
    fn merge_shapes_intersection(&self, shape1: &Shape, shape2: &Shape) -> Result<Shape> {
        let merged_id = ShapeId::new(format!("{}_{}_intersect", shape1.id.as_str(), shape2.id.as_str()));
        let mut merged_shape = Shape::new(merged_id, shape1.shape_type.clone());
    ...
    fn analyze_performance_bottlenecks(
        ...
        tracing::debug!("Analyzing performance bottlenecks from {} validation reports", validation_history.len());
    ...
    fn analyze_execution_time_bottlenecks(&self, validation_history: &[ValidationReport]) -> Result<Option<PerformanceBottleneck>> {
        // Calculate execution time statistics
        let execution_times: Vec<f64> = validation_history
            .iter()
            .filter_map(|report| {
                report.metadata.metadata.get("execution_time_ms")
        ...
        let recent_avg = execution_times[execution_times.len().saturating_sub(5)..]
            .iter().sum::<f64>() / execution_times[execution_times.len().saturating_sub(5)..].len() as f64;
        
        // Check for degradation
        let degradation_ratio = recent_avg / avg_time;
        
        if degradation_ratio > 1.5 { // 50% increase in execution time
    ...
    fn analyze_memory_bottlenecks(&self, validation_history: &[ValidationReport]) -> Result<Option<PerformanceBottleneck>> {
        // Extract memory usage data
        let memory_usages: Vec<f64> = validation_history
            .iter()
            .filter_map(|report| {
                report.metadata.metadata.get("peak_memory_mb")
                    .and_then(|v| v.parse::<f64>().ok())
            })
            .collect();
        ...
        let max_memory = memory_usages.iter().cloned().fold(f64::NEG_INFINITY, f64::max);
        let avg_memory = memory_usages.iter().sum::<f64>() / memory_usages.len() as f64;
        
        // Check for high memory usage
        if max_memory > 1024.0 || avg_memory > 512.0 { // High memory thresholds
            ...
            let bottleneck = PerformanceBottleneck {
                ...
                severity,
                impact_score,
                affected_operations: vec!["memory_allocation".to_string(), "garbage_collection".to_string()],
            };
    ...
    fn analyze_constraint_bottlenecks(&self, validation_history: &[ValidationReport]) -> Result<Vec<PerformanceBottleneck>> {
        let mut bottlenecks = Vec::new();
        ...
        for (constraint_type, timings) in constraint_failures {
            ...
            if avg_timing > 10.0 { // Constraint takes more than 10ms on average
    ...
    /// Analyze shape-specific bottlenecks
    fn analyze_shape_specific_bottlenecks(&self, validation_history: &[ValidationReport]) -> Result<Vec<PerformanceBottleneck>> {
        let mut bottlenecks = Vec::new();
    ...
    fn analyze_shape_complexity_for_recommendations(
        ...
        if self.detect_similar_shapes(shapes)? {
            recommendations.push(OptimizationRecommendation {
                recommendation_type: OptimizationRecommendationType::ShapeMerging,
                priority: RecommendationPriority::Medium,
                description: "Similar shapes detected, consider merging to reduce validation overhead".to_string(),
                estimated_benefit: 0.3,
                implementation_effort: ImplementationEffort::Medium,
        ...
        tracing::debug!("Generated {} shape complexity recommendations", recommendations.len());
        Ok(recommendations)
    }
    
    /// Identify expensive constraints in a shape
    fn identify_expensive_constraints(&self, constraints: &IndexMap<ConstraintComponentId, Constraint>) -> Result<Vec<String>> {
        let mut expensive_constraints = Vec::new();
        
        for (constraint_id, constraint) in constraints {
            let cost = self.estimate_constraint_cost(constraint)?;
            
            if cost > 0.7 { // High cost threshold
    ...
    fn identify_redundant_constraints(&self, constraints: &IndexMap<ConstraintComponentId, Constraint>) -> Result<Vec<String>> {
        let mut redundant_constraints = Vec::new();
        
        // Check for common redundancy patterns
        let has_min_count = constraints.values().any(|c| matches!(c, Constraint::MinCount(_)));
        let has_max_count = constraints.values().any(|c| matches!(c, Constraint::MaxCount(_)));
        let has_class = constraints.values().any(|c| matches!(c, Constraint::Class(_)));
        let has_node_kind = constraints.values().any(|c| matches!(c, Constraint::NodeKind(_)));
