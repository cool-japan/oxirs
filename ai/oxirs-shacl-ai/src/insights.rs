...
impl ValidationInsight {
    ...
    pub fn is_high_priority(&self) -> bool {
        matches!(self.severity, InsightSeverity::Critical | InsightSeverity::High)
    }
...
impl PerformanceInsight {
    ...
    pub fn performance_status(&self) -> PerformanceStatus {
        match (&self.trend_direction, self.severity) {
            (TrendDirection::Increasing, _) if self.metric_name == "throughput" => PerformanceStatus::Improving,
            (TrendDirection::Decreasing, _) if self.metric_name != "throughput" => PerformanceStatus::Improving,
            (TrendDirection::Stable, InsightSeverity::Low | InsightSeverity::Info) => PerformanceStatus::Stable,
...
impl InsightCollection {
    ...
    pub fn high_priority_count(&self) -> usize {
        ...
        count += self.validation_insights.iter().filter(|i| i.is_high_priority()).count();
        count += self.quality_insights.iter().filter(|i| matches!(i.severity, InsightSeverity::Critical | InsightSeverity::High)).count();
        count += self.performance_insights.iter().filter(|i| matches!(i.severity, InsightSeverity::Critical | InsightSeverity::High)).count();
        count += self.shape_insights.iter().filter(|i| matches!(i.severity, InsightSeverity::Critical | InsightSeverity::High)).count();
        count += self.data_insights.iter().filter(|i| matches!(i.severity, InsightSeverity::Critical | InsightSeverity::High)).count();
    ...
    pub fn filter_by_severity(&self, min_severity: InsightSeverity) -> InsightCollection {
        ...
        filtered.validation_insights = self.validation_insights.iter()
            .filter(|i| i.severity >= min_severity)
            .cloned()
            .collect();
        
        filtered.quality_insights = self.quality_insights.iter()
            .filter(|i| i.severity >= min_severity)
            .cloned()
            .collect();
        
        filtered.performance_insights = self.performance_insights.iter()
            .filter(|i| i.severity >= min_severity)
            .cloned()
            .collect();
        
        filtered.shape_insights = self.shape_insights.iter()
            .filter(|i| i.severity >= min_severity)
            .cloned()
            .collect();
        
        filtered.data_insights = self.data_insights.iter()
            .filter(|i| i.severity >= min_severity)
            .cloned()
            .collect();
...
impl InsightSummary {
    ...
    pub fn from_collection(collection: &InsightCollection) -> Self {
        ...
        insights_by_type.insert("validation".to_string(), collection.validation_insights.len());
        insights_by_type.insert("quality".to_string(), collection.quality_insights.len());
        insights_by_type.insert("performance".to_string(), collection.performance_insights.len());
        ...
        for insight in &collection.validation_insights {
            *insights_by_severity.entry(insight.severity.clone()).or_insert(0) += 1;
        }
...
impl InsightTrait for ValidationInsight {
    ...
    fn severity(&self) -> &InsightSeverity { &self.severity }
    fn confidence(&self) -> f64 { self.confidence }
    fn recommendations(&self) -> &[String] { &self.recommendations }
    fn category(&self) -> InsightCategory { InsightCategory::Validation }
}
...
impl InsightTrait for QualityInsight {
    ...
    fn confidence(&self) -> f64 { self.confidence }
    fn recommendations(&self) -> &[String] { &self.recommendations }
    fn category(&self) -> InsightCategory { InsightCategory::Quality }
}
...
impl InsightTrait for PerformanceInsight {
    ...
    fn confidence(&self) -> f64 { self.confidence }
    fn recommendations(&self) -> &[String] { &self.recommendations }
    fn category(&self) -> InsightCategory { InsightCategory::Performance }
}
...
impl InsightTrait for ShapeInsight {
    ...
    fn confidence(&self) -> f64 { self.confidence }
    fn recommendations(&self) -> &[String] { &self.recommendations }
    fn category(&self) -> InsightCategory { InsightCategory::Shape }
}
...
impl InsightTrait for DataInsight {
    ...
    fn severity(&self) -> &InsightSeverity { &self.severity }
    fn confidence(&self) -> f64 { self.confidence }
    fn recommendations(&self) -> &[String] { &self.data_improvements }
    fn category(&self) -> InsightCategory { InsightCategory::Data }
}
...
mod tests {
    ...
    fn test_validation_insight_creation() {
        ...
        assert_eq!(insight.primary_recommendation(), Some("Review shape definitions"));
    }
