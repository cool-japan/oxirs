...
fn test_learning_query_result_types() {
    ...
    match select_result {
        LearningQueryResult::Select { variables, bindings } => {
...
fn test_pattern_to_shape_conversion() {
    ...
    assert!(result.is_ok(), "High confidence pattern should convert to shape");
    ...
    assert!(result.is_err(), "Low confidence pattern should not convert to shape");
}
