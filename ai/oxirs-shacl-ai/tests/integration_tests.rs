...
async fn test_full_ai_assistant_workflow() {
    ...
    // Test shape learning
    let learned_shapes = assistant.learn_shapes_from_store(&store, None)
        .await
        .expect("Shape learning failed");
    assert!(!learned_shapes.is_empty(), "No shapes were learned");
    
    // Test pattern discovery
    let patterns = assistant.discover_patterns(&store, None)
        .await
        .expect("Pattern discovery failed");
    assert!(!patterns.is_empty(), "No patterns were discovered");
    
    // Test quality assessment
    let quality_report = assistant.assess_quality(&store, &learned_shapes, None)
        .await
    ...
    let prediction_result = assistant.predict_validation_outcome(&store, &learned_shapes, None)
        .await
...
fn test_error_handling() {
    ...
    let result = tokio_test::block_on(async {
        assistant.learn_shapes_from_store(&empty_store, None).await
    });
    
    // Should handle empty store gracefully
    match result {
        Ok(shapes) => assert!(shapes.is_empty(), "Should return empty shapes for empty store"),
        Err(e) => match e {
    ...
    tokio_test::block_on(async {
        ...
        for i in 0..5 {
            ...
            set.spawn(async move {
                ...
                assistant_clone.discover_patterns(&*store_clone, Some(&graph_name)).await
            });
        }
...
fn test_memory_usage() {
    ...
    tokio_test::block_on(async {
        let _patterns = assistant.discover_patterns(&store, None).await
    ...
    assert!(memory_increase < 100 * 1024 * 1024, 
           "Memory usage increase should be bounded: {} bytes", memory_increase);
}
...
fn test_performance_benchmarks() {
    ...
    let _shapes = tokio_test::block_on(async {
        assistant.learn_shapes_from_store(&store, None).await
    }).expect("Shape learning failed");
    let shape_learning_duration = start.elapsed();
    
    // Benchmark pattern discovery
    let start = Instant::now();
    let _patterns = tokio_test::block_on(async {
        assistant.discover_patterns(&store, None).await
    }).expect("Pattern discovery failed");
    let pattern_discovery_duration = start.elapsed();
    
    // Performance assertions (adjust based on expected performance)
    assert!(shape_learning_duration.as_millis() < 5000, 
           "Shape learning should complete within 5 seconds: {:?}", shape_learning_duration);
    assert!(pattern_discovery_duration.as_millis() < 3000, 
           "Pattern discovery should complete within 3 seconds: {:?}", pattern_discovery_duration);
