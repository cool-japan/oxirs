...
use super::query_planner::{QueryPlanner, QueryPlan};
...
impl FederationManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        tracing::info!("Initializing federation manager with {} endpoints", self.config.endpoints.len());
        
        // Introspect and merge all remote schemas
        let merged_schema = self.schema_stitcher.merge_schemas(&self.config.endpoints).await
        ...
        for endpoint in &self.config.endpoints {
            if let Err(e) = self.update_endpoint_statistics(&endpoint.id).await {
                tracing::warn!("Failed to update statistics for endpoint {}: {}", endpoint.id, e);
            }
        }
    ...
    pub async fn execute_graphql_query(&self, query: &crate::ast::Document) -> Result<serde_json::Value> {
        let merged_schema = self.get_merged_schema().await?;
        
        // Plan the query execution
        let query_plan = self.query_planner.plan_query(query, &merged_schema).await
            .context("Failed to plan federated query")?;
            
        tracing::debug!("Executing federated query with {} steps", query_plan.steps.len());
        
        // Execute the plan
        self.query_planner.execute_plan(&query_plan).await
    ...
    pub async fn execute_sparql_query(&self, query: &str) -> Result<serde_json::Value> {
        ...
        dataset_federation.federate_sparql_query(query).await
    ...
    pub async fn add_endpoint(&mut self, endpoint: RemoteEndpoint) -> Result<()> {
        ...
        let remote_schema = self.schema_stitcher.introspect_remote(&endpoint).await
            .context("Failed to introspect new endpoint")?;
            
        // Update merged schema
        let mut merged_schema = self.get_merged_schema().await?;
        self.schema_stitcher.merge_schema_into(&mut merged_schema, &remote_schema, &endpoint)
    ...
    pub async fn update_endpoint_statistics(&self, endpoint_id: &str) -> Result<()> {
        let mut dataset_federation = self.dataset_federation.write().await;
        dataset_federation.update_endpoint_statistics(endpoint_id).await
    ...
    async fn check_endpoint_health(&self, endpoint: &RemoteEndpoint) -> bool {
        if let Some(health_url) = &endpoint.health_check_url {
            ...
            self.schema_stitcher.introspect_remote(endpoint).await.is_ok()
        }
    }
