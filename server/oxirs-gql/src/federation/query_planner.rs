...
use crate::ast::{Document, OperationDefinition, SelectionSet, Field, Selection};
...
impl QueryPlanner {
    ...
    async fn analyze_operation(&self, operation: &OperationDefinition, schema: &Schema) -> Result<Vec<QueryStep>> {
        let mut steps = Vec::new();
        
        match operation.operation_type {
            crate::ast::OperationType::Query => {
                steps.extend(self.analyze_selection_set(&operation.selection_set, schema).await?);
            }
    ...
    async fn analyze_selection_set(&self, selection_set: &SelectionSet, schema: &Schema) -> Result<Vec<QueryStep>> {
        let mut steps = Vec::new();
        ...
        for selection in &selection_set.selections {
            match selection {
                Selection::Field(field) => {
                    ...
                    service_queries.entry(service_id.clone())
                        .or_insert_with(Vec::new)
                        .push(field.name.clone());
                        
                    // Recursively analyze nested selections
                    if let Some(nested_selection_set) = &field.selection_set {
                        let nested_steps = Box::pin(self.analyze_selection_set(nested_selection_set, schema)).await?;
                        steps.extend(nested_steps);
                    }
                }
                Selection::InlineFragment(fragment) => {
                    let nested_steps = Box::pin(self.analyze_selection_set(&fragment.selection_set, schema)).await?;
                    steps.extend(nested_steps);
                }
    ...
    fn optimize_step_order(&self, steps: &mut Vec<QueryStep>) {
        ...
        while !remaining_steps.is_empty() {
            ...
            for i in 0..remaining_steps.len() {
                ...
                let dependencies_resolved = step.dependencies.iter()
    ...
    pub async fn execute_plan(&self, plan: &QueryPlan) -> Result<serde_json::Value> {
        ...
        if plan.can_execute_parallel {
            ...
            let futures: Vec<_> = plan.steps.iter().enumerate()
                .map(|(idx, step)| {
                    let step = step.clone();
                    async move {
    ...
    async fn execute_step(&self, step: &QueryStep) -> Result<serde_json::Value> {
        ...
        Ok(result.get("data").unwrap_or(&serde_json::Value::Null).clone())
    }
    
    /// Merge execution results from multiple steps
    fn merge_execution_results(&self, results: &HashMap<usize, serde_json::Value>) -> Result<serde_json::Value> {
