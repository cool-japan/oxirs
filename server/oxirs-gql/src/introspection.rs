...
use crate::types::{
    Schema, GraphQLType, ObjectType, InterfaceType, UnionType, EnumType, 
    InputObjectType, ScalarType, FieldType, ArgumentType, EnumValue
};
...
impl SchemaIntrospection {
    ...
    pub fn get_types(&self) -> Vec<TypeIntrospection> {
        ...
        for (_, gql_type) in &self.schema.types {
            types.push(TypeIntrospection::new(gql_type.clone(), Arc::clone(&self.schema)));
        }
    ...
    fn get_builtin_scalar_types(&self) -> Vec<TypeIntrospection> {
        vec![
            TypeIntrospection::builtin_scalar("String", "The `String` scalar type represents textual data"),
            TypeIntrospection::builtin_scalar("Int", "The `Int` scalar type represents non-fractional signed whole numeric values"),
            TypeIntrospection::builtin_scalar("Float", "The `Float` scalar type represents signed double-precision fractional values"),
            TypeIntrospection::builtin_scalar("Boolean", "The `Boolean` scalar type represents `true` or `false`"),
            TypeIntrospection::builtin_scalar("ID", "The `ID` scalar type represents a unique identifier"),
...
impl TypeIntrospection {
    ...
    pub fn builtin_scalar(name: &str, description: &str) -> Self {
        let scalar = ScalarType::new(name.to_string())
    ...
    pub fn fields(&self, include_deprecated: bool) -> Option<Vec<FieldIntrospection>> {
        match &self.gql_type {
            GraphQLType::Object(obj) => {
                let mut fields: Vec<FieldIntrospection> = obj.fields.values()
    ...
    pub fn interfaces(&self) -> Option<Vec<TypeIntrospection>> {
        match &self.gql_type {
            GraphQLType::Object(obj) => {
                Some(obj.interfaces.iter()
                    .filter_map(|interface_name| {
                        self.schema.get_type(interface_name)
                            .map(|gql_type| TypeIntrospection::new(gql_type.clone(), Arc::clone(&self.schema)))
                    })
                    .collect())
            }
    ...
    pub fn possible_types(&self) -> Option<Vec<TypeIntrospection>> {
        match &self.gql_type {
            GraphQLType::Interface(iface) => {
                Some(self.schema.types.values()
                    .filter_map(|gql_type| {
                        match gql_type {
                            GraphQLType::Object(obj) if obj.interfaces.contains(&iface.name) => {
                                Some(TypeIntrospection::new(gql_type.clone(), Arc::clone(&self.schema)))
                            }
                            _ => None,
                        }
                    })
    ...
    pub fn input_fields(&self) -> Option<Vec<InputValueIntrospection>> {
        match &self.gql_type {
            GraphQLType::InputObject(input_obj) => {
                Some(input_obj.fields.values()
                    .map(|field| {
                        // Convert InputFieldType to ArgumentType for compatibility
                        let arg = ArgumentType::new(field.name.clone(), field.field_type.clone())
                            .with_description(field.description.clone().unwrap_or_default())
                            .with_default_value(field.default_value.clone().unwrap_or(crate::ast::Value::NullValue));
    ...
    pub fn of_type(&self) -> Option<TypeIntrospection> {
        match &self.gql_type {
            GraphQLType::List(inner_type) => {
                Some(TypeIntrospection::new(*inner_type.clone(), Arc::clone(&self.schema)))
            }
...
impl FieldIntrospection {
    ...
    pub fn introspection_field(name: &str, type_name: &str, description: &str) -> Self {
        let field = FieldType::new(
            name.to_string(),
            GraphQLType::Scalar(ScalarType::new(type_name.to_string())
                .with_description(description.to_string())),
        ).with_description(description.to_string());
    ...
    pub fn deprecated() -> Self {
        Self::new(
            ...
            vec![InputValueIntrospection::new(
                ArgumentType::new(
                    "reason".to_string(),
                    GraphQLType::Scalar(ScalarType::new("String".to_string())
    ...
    pub fn specified_by() -> Self {
        Self::new(
            ...
            vec![InputValueIntrospection::new(
                ArgumentType::new(
                    "url".to_string(),
                    GraphQLType::NonNull(Box::new(GraphQLType::Scalar(ScalarType::new("String".to_string())
...
impl FieldResolver for IntrospectionResolver {
    async fn resolve_field(
        ...
        match field_name {
            "__schema" => {
                Ok(self.resolve_schema().await?)
            }
            "__type" => {
                let type_name = args.get("name")
            ...
            "queryType" => {
                Ok(self.resolve_query_type().await?)
            }
            "mutationType" => {
                Ok(self.resolve_mutation_type().await?)
            }
            "subscriptionType" => {
                Ok(self.resolve_subscription_type().await?)
            }
            "types" => {
                Ok(self.resolve_types().await?)
            }
            "directives" => {
                Ok(self.resolve_directives().await?)
            }
...
impl IntrospectionResolver {
    async fn resolve_schema(&self) -> Result<Value> {
        ...
        Ok(Value::ObjectValue(HashMap::from([
            ...
            ("mutationType".to_string(), self.resolve_mutation_type().await?),
            ("subscriptionType".to_string(), self.resolve_subscription_type().await?),
    ...
    async fn resolve_type(&self, type_name: &str) -> Result<Value> {
        if let Some(gql_type) = self.schema.get_type(type_name) {
            let type_introspection = TypeIntrospection::new(gql_type.clone(), Arc::clone(&self.schema));
            Ok(self.type_to_value(&type_introspection))
        } else {
            // Check built-in types
            let builtin_types = SchemaIntrospection::new(Arc::clone(&self.schema)).get_builtin_scalar_types();
    ...
    async fn resolve_query_type(&self) -> Result<Value> {
        if let Some(query_type) = SchemaIntrospection::new(Arc::clone(&self.schema)).get_query_type() {
    ...
    async fn resolve_mutation_type(&self) -> Result<Value> {
        if let Some(mutation_type) = SchemaIntrospection::new(Arc::clone(&self.schema)).get_mutation_type() {
    ...
    async fn resolve_subscription_type(&self) -> Result<Value> {
        if let Some(subscription_type) = SchemaIntrospection::new(Arc::clone(&self.schema)).get_subscription_type() {
    ...
    async fn resolve_types(&self) -> Result<Value> {
        ...
        let type_values: Vec<Value> = types.iter()
    ...
    async fn resolve_directives(&self) -> Result<Value> {
        ...
        let directive_values: Vec<Value> = directives.iter()
    ...
    fn type_to_value(&self, type_info: &TypeIntrospection) -> Value {
        ...
        obj.insert("kind".to_string(), Value::EnumValue(type_info.kind().as_str().to_string()));
        ...
        if let Some(fields) = type_info.fields(false) {
            let field_values: Vec<Value> = fields.iter()
        ...
        if let Some(interfaces) = type_info.interfaces() {
            let interface_values: Vec<Value> = interfaces.iter()
        ...
        if let Some(possible_types) = type_info.possible_types() {
            let possible_type_values: Vec<Value> = possible_types.iter()
                .map(|possible_type| self.type_to_value(possible_type))
                .collect();
            obj.insert("possibleTypes".to_string(), Value::ListValue(possible_type_values));
        } else {
        ...
        if let Some(enum_values) = type_info.enum_values(false) {
            let enum_value_values: Vec<Value> = enum_values.iter()
                .map(|enum_value| self.enum_value_to_value(enum_value))
                .collect();
            obj.insert("enumValues".to_string(), Value::ListValue(enum_value_values));
        } else {
        ...
        if let Some(input_fields) = type_info.input_fields() {
            let input_field_values: Vec<Value> = input_fields.iter()
                .map(|input_field| self.input_value_to_value(input_field))
                .collect();
            obj.insert("inputFields".to_string(), Value::ListValue(input_field_values));
        } else {
        ...
        if let Some(specified_by_url) = type_info.specified_by_url() {
            obj.insert("specifiedByURL".to_string(), Value::StringValue(specified_by_url));
        } else {
    ...
    fn field_to_value(&self, field: &FieldIntrospection) -> Value {
        ...
        let args: Vec<Value> = field.args().iter()
        ...
        obj.insert("isDeprecated".to_string(), Value::BooleanValue(field.is_deprecated()));
        
        if let Some(deprecation_reason) = field.deprecation_reason() {
            obj.insert("deprecationReason".to_string(), Value::StringValue(deprecation_reason));
        } else {
    ...
    fn input_value_to_value(&self, input_value: &InputValueIntrospection) -> Value {
        ...
        obj.insert("type".to_string(), self.type_to_value(&input_value.input_type()));
        
        if let Some(default_value) = input_value.default_value() {
            obj.insert("defaultValue".to_string(), Value::StringValue(default_value));
        } else {
    ...
    fn enum_value_to_value(&self, enum_value: &EnumValueIntrospection) -> Value {
        ...
        obj.insert("isDeprecated".to_string(), Value::BooleanValue(enum_value.is_deprecated()));
    ...
    fn directive_to_value(&self, directive: &DirectiveIntrospection) -> Value {
        ...
        let locations: Vec<Value> = directive.locations().iter()
            .map(|location| Value::EnumValue(location.as_str().to_string()))
            .collect();
        obj.insert("locations".to_string(), Value::ListValue(locations));
        
        let args: Vec<Value> = directive.args().iter()
            .map(|arg| self.input_value_to_value(arg))
            .collect();
        obj.insert("args".to_string(), Value::ListValue(args));
        
        obj.insert("isRepeatable".to_string(), Value::BooleanValue(directive.is_repeatable()));
...
mod tests {
    ...
    fn test_directive_location_string_conversion() {
        ...
        assert_eq!(DirectiveLocation::FragmentSpread.as_str(), "FRAGMENT_SPREAD");
        assert_eq!(DirectiveLocation::InlineFragment.as_str(), "INLINE_FRAGMENT");
    }
    ...
    async fn test_introspection_resolver() {
        ...
        // Test __schema field
        let schema_result = resolver.resolve_field("__schema", &HashMap::new(), &context).await;
        ...
        let mut args = HashMap::new();
        args.insert("name".to_string(), Value::StringValue("NonExistent".to_string()));
        let type_result = resolver.resolve_field("__type", &args, &context).await;
        assert!(type_result.is_ok());
    ...
    fn test_field_introspection() {
        ...
        assert_eq!(field_introspection.description(), Some("Test field".to_string()));
    ...
    fn test_input_value_introspection() {
        ...
        let input_value_introspection = InputValueIntrospection::new(arg, Arc::new(create_test_schema()));
        
        assert_eq!(input_value_introspection.name(), "test");
        assert_eq!(input_value_introspection.description(), Some("Test argument".to_string()));
        assert_eq!(input_value_introspection.default_value(), Some("\"default\"".to_string()));
    }
}