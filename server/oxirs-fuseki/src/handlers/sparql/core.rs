    ...
    match execute_sparql_query(&query_string, context, &state).await {
        Ok(result) => {
            ...
            if let Some(metrics) = &state.metrics_service {
                ...
                metrics.record_sparql_query(execution_time, true, query_type).await;
            }
        ...
        Err(e) => {
            ...
            if let Some(metrics) = &state.metrics_service {
                ...
                metrics.record_sparql_query(execution_time, false, query_type).await;
            }
    ...
    match execute_sparql_update(&params.update, context, &state).await {
        Ok(result) => {
            ...
            if let Some(metrics) = &state.metrics_service {
                ...
                metrics.record_sparql_update(execution_time, true, update_type).await;
            }
        ...
        Err(e) => {
            ...
            if let Some(metrics) = &state.metrics_service {
                ...
                metrics.record_sparql_update(execution_time, false, update_type).await;
            }
