...
impl FederationPlanner {
    ...
    pub async fn create_execution_plan(&self, query: &str) -> FusekiResult<FederatedQueryPlan> {
        ...
        let resource_requirements = self
            .estimate_resource_requirements(&execution_steps, &query_analysis)
            .await?;
        ...
        info!(
            "Created federated query plan: {} with {} steps",
            plan_id,
    ...
    fn extract_service_clauses(&self, query: &str) -> FusekiResult<Vec<ServiceClause>> {
        ...
        if let Ok(service_regex) = regex::Regex::new(r"SERVICE\s+<([^>]+)>\s*\{([^}]+)\}") {
            for captures in service_regex.captures_iter(query) {
                if let (Some(endpoint), Some(query_fragment)) = (captures.get(1), captures.get(2)) {
    ...
    async fn create_execution_steps(
        ...
        if service_clauses.len() > 1 {
            steps.push(ExecutionStep {
                ...
                dependencies: (0..service_clauses.len())
...
impl FederationPlanner {
    ...
    pub async fn register_endpoint(&self, endpoint: ServiceEndpoint) -> FusekiResult<()> {
        ...
        info!(
            "Registered federation endpoint: {} ({})",
    ...
    pub async fn plan_federated_query(
        ...
        if service_patterns.is_empty() {
            return Err(FusekiError::bad_request(
                "No SERVICE clauses found in query",
            ));
        }
        ...
        let plan = self
            .generate_execution_plan(query, service_patterns, query_analysis)
            .await?;
    ...
    pub async fn execute_federated_query(
        ...
        let result = if plan.parallelizable && self.config.enable_parallel_execution {
            self.execute_parallel_plan(plan.clone(), local_store)
                .await?
        } else {
            self.execute_sequential_plan(plan.clone(), local_store)
                .await?
        };
        ...
        self.update_endpoint_statistics(&result.endpoint_times)
            .await;
        ...
        info!(
            "Federated query executed in {:?}, {} results",
            execution_time, result.result_count
        );
                            ...
                            if brace_count == 0 {
                                ...
                                if let Some(endpoint) = current_service.take() {
                                    patterns.push(ServicePattern {
                                        ...
                                        estimated_complexity: self
                                            .estimate_pattern_complexity(&service_content),
                                    });
                                }
                                in_service = false;
                                service_content.clear();
        ...
        for (index, pattern) in service_patterns.iter().enumerate() {
            let stage = ExecutionStage {
                ...
                estimated_cardinality: self
                    .estimate_result_cardinality(&pattern.endpoint, &pattern.query_pattern)
                    .await,
    ...
    async fn execute_service_query(
        ...
        let endpoint = {
            ...
            endpoints_guard
                ...
                .cloned()
                .ok_or_else(|| {
                    FusekiError::not_found(format!("Endpoint not found: {}", stage.endpoint))
                })?
        };
        ...
        let response = request
            ...
            .await
            .map_err(|e| {
                FusekiError::service_unavailable(format!("Service request failed: {}", e))
            })?;
