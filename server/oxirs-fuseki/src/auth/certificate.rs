...
impl CertificateAuth {
    ...
    pub async fn authenticate_certificate(
        ...
        debug!("Successful certificate authentication for: {}", user.username);
    ...
    async fn validate_certificate_trust(&self, cert: &X509Certificate) -> FusekiResult<bool> {
        ...
        let trust_store_path = self.config.certificate.as_ref()
        ...
        if let Some(trusted_issuers) = self.config.certificate.as_ref()
    ...
    fn extract_username_from_dn(&self, dn: &str) -> FusekiResult<String> {
        ...
        Err(FusekiError::authentication("No username found in certificate DN"))
    }
    ...
    fn extract_email_from_dn(&self, dn: &str) -> FusekiResult<String> {
        for component in dn.split(',') {
            ...
            if component.starts_with("emailAddress=") || component.starts_with("E=") {
                let start_pos = if component.starts_with("emailAddress=") { 13 } else { 2 };
                return Ok(component[start_pos..].to_string());
            }
        }
        
        Err(FusekiError::authentication("No email found in certificate DN"))
    }
    ...
    fn extract_common_name_from_dn(&self, dn: &str) -> FusekiResult<String> {
        ...
        Err(FusekiError::authentication("No common name found in certificate DN"))
    }
}
