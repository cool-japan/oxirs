...
pub trait CostEstimator: Send + Sync {
    ...
    async fn estimate_result_size(&self, pattern: &str, endpoint: &ServiceEndpoint) -> Result<usize>;
            ...
            .await
        {
            Ok(response) => {
                ...
                Ok(ServiceHealth {
                    ...
                    error_message: if is_healthy { None } else { Some("HTTP error".to_string()) },
                })
            }
            Err(e) => {
                Ok(ServiceHealth {
            ...
            for request in batch {
                ...
                futures.push(async move {
                    match timeout_future.await {
                        Ok(result) => result,
                        Err(_) => Err(Error::TimeoutWithMessage("Service request timed out".to_string())),
                    }
                });
            }
...
impl QueryPlanner {
    ...
    pub async fn refresh_endpoints(&self) -> Result<()> {
        ...
        let health_futures: Vec<_> = discovered.iter()
            .map(|endpoint| async {
                let health = self.discovery_service.check_health(&endpoint.url).await
                    .unwrap_or_default();
                (endpoint.url.clone(), health)
            })
            .collect();
        
        let health_results = self.parallel_executor
    ...
    async fn create_execution_steps(
        ...
        if !service_patterns.is_empty() {
            ...
            self.create_service_delegation_steps(query, &service_patterns, endpoint_costs).await
        } else {
            // Query can potentially be decomposed based on data partitioning
            self.create_partitioned_execution_steps(query, endpoint_costs).await
        }
    }
    ...
    fn extract_service_patterns(&self, query: &Query) -> Result<Vec<ServicePattern>> {
        ...
        let service_regex = regex::Regex::new(r"SERVICE\s*<([^>]+)>\s*\{([^}]+)\}")
            .map_err(|e| Error::Parse { message: format!("Regex error: {}", e) })?;
    ...
    fn extract_variables_from_pattern(&self, pattern: &str) -> Vec<String> {
        ...
        var_regex.captures_iter(pattern)
    ...
    async fn create_service_delegation_steps(
        ...
        for (idx, pattern) in service_patterns.iter().enumerate() {
            ...
            let matching_endpoint = endpoint_costs.iter()
            ...
            let service_selection = ServiceSelection {
                ...
                score: 1.0 / (matching_endpoint.1.execution_time.unwrap_or_default().as_millis() as f64 + 1.0),
                is_primary: true,
            };
            ...
            steps.push(ExecutionStep {
                ...
                sub_query,
                dependencies: if idx > 0 { vec![format!("service_step_{}", idx - 1)] } else { vec![] },
    ...
    async fn create_partitioned_execution_steps(
        ...
        if self.is_complex_query(query) && endpoint_costs.len() > 1 {
            self.create_parallel_decomposition_steps(query, endpoint_costs).await
        } else {
    ...
    async fn create_single_step(
        ...
        let (best_endpoint, cost) = endpoint_costs.first()
            .ok_or_else(|| Error::ServiceUnavailable {
    ...
    async fn execute_parallel_plan(&self, plan: &FederatedQueryPlan) -> Result<Vec<QueryResult>> {
        let execution_futures: Vec<_> = plan.steps.iter()
            .map(|step| self.execute_step(step))
            .collect();
        
        let results = self.parallel_executor.execute_parallel(execution_futures).await;
    ...
    async fn execute_service_query(&self, service: &ServiceSelection, step: &ExecutionStep) -> Result<QueryResult> {
        ...
        let result = self.parallel_executor.execute_with_retry(|| {
        ...
        self.update_service_statistics(&service.service_id, execution_time, result.bindings.len()).await;
    ...
    async fn execute_with_fallback(&self, step: &ExecutionStep) -> Result<QueryResult> {
        ...
        sorted_services.sort_by(|a, b| b.score.partial_cmp(&a.score).unwrap_or(std::cmp::Ordering::Equal));
    ...
    fn parse_sparql_results(&self, json: serde_json::Value) -> Result<QueryResult> {
        let head = json.get("head")
            .ok_or_else(|| Error::Parse { message: "Missing 'head' in SPARQL results".to_string() })?;
        
        let vars = head.get("vars")
            .and_then(|v| v.as_array())
            .ok_or_else(|| Error::Parse { message: "Missing 'vars' in SPARQL results".to_string() })?;
        
        let variables: Vec<String> = vars.iter()
            .filter_map(|v| v.as_str())
            .map(|s| s.to_string())
            .collect();
        
        let results = json.get("results")
            .ok_or_else(|| Error::Parse { message: "Missing 'results' in SPARQL results".to_string() })?;
        
        let bindings_array = results.get("bindings")
            .and_then(|b| b.as_array())
            .ok_or_else(|| Error::Parse { message: "Missing 'bindings' in SPARQL results".to_string() })?;
    ...
    async fn update_service_statistics(&self, service_id: &str, execution_time: Duration, result_count: usize) {
        let mut stats = self.statistics.write().await;
        
        let service_stats = stats.service_stats.entry(service_id.to_string())
        ...
        let new_avg_size = (current_avg_size * (total_queries - 1.0) + result_count as f64) / total_queries;
        service_stats.average_result_size = new_avg_size as usize;
        
        // Update availability
        service_stats.availability = service_stats.successful_queries as f64 / service_stats.total_queries as f64;
