...
impl AggregateFunction for StdDevAggregate {
    ...
    fn get_result(&self) -> FusekiResult<AggregationResult> {
        ...
        let variance = self.values.iter()
            .map(|x| (x - mean).powi(2))
            .sum::<f64>() / if self.population { n } else { n - 1.0 };
    ...
    fn name(&self) -> &str {
        if self.population { "STDDEV_POP" } else { "STDDEV" }
    }
}
...
impl AggregateFunction for VarianceAggregate {
    ...
    fn name(&self) -> &str {
        if self.population { "VARIANCE_POP" } else { "VARIANCE" }
    }
}
        ...
        match value {
            Value::Number(n) => {
                if let Some(f) = n.as_f64() {
                    ...
                    Err(FusekiError::bad_request("PERCENTILE requires numeric values"))
                }
            }
            _ => Err(FusekiError::bad_request("PERCENTILE requires numeric values")),
        }
    }
...
impl AggregationFactory {
    ...
    pub fn create_aggregate(
        ...
        match name_upper.as_str() {
            "GROUP_CONCAT" => {
                let separator = args.get("separator")
                    .and_then(|v| v.as_str())
                    .map(|s| s.to_string());
                let distinct = args.get("distinct")
            ...
            "PERCENTILE" => {
                let percentile = args.get("percentile")
                    .and_then(|v| v.as_f64())
                    .unwrap_or(50.0);
    ...
    pub fn is_supported_aggregate(function_name: &str) -> bool {
        matches!(
            ...
            "STDDEV" | "STDEV" | "STDDEV_POP" | "STDEV_POP" |
            "VARIANCE" | "VAR" | "VARIANCE_POP" | "VAR_POP" |
