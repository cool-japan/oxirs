...
use oxirs_fuseki::handlers::saml::{
    ValidatedSamlAssertion, SamlMfaRequirement, AuthnStrength,
    EnhancedSamlConfig, SamlSessionConfig, SamlFederationConfig,
    SamlComplianceConfig, validate_mfa_requirements,
    validate_enhanced_saml_assertion, determine_authn_strength,
...
mod saml_authentication_tests {
    ...
    fn test_authn_strength_determination() {
        ...
        assert!(matches!(
            determine_authn_strength("urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport"),
            AuthnStrength::Low
        ));
        
        // Test medium strength
        assert!(matches!(
            determine_authn_strength("urn:oasis:names:tc:SAML:2.0:ac:classes:MobileOneFactorUnregistered"),
            AuthnStrength::Medium
        ));
        
        // Test high strength
        assert!(matches!(
            determine_authn_strength("urn:oasis:names:tc:SAML:2.0:ac:classes:MobileTwoFactorContract"),
            AuthnStrength::High
        ));
    ...
    fn test_strength_comparison() {
        ...
        assert!(meets_minimum_strength(&AuthnStrength::High, &AuthnStrength::Medium));
        assert!(meets_minimum_strength(&AuthnStrength::Highest, &AuthnStrength::High));
        assert!(meets_minimum_strength(&AuthnStrength::Medium, &AuthnStrength::Low));
        
        // Test equal strengths
        assert!(meets_minimum_strength(&AuthnStrength::Medium, &AuthnStrength::Medium));
        
        // Test that lower strengths don't meet higher requirements
        assert!(!meets_minimum_strength(&AuthnStrength::Low, &AuthnStrength::Medium));
        assert!(!meets_minimum_strength(&AuthnStrength::Medium, &AuthnStrength::High));
        assert!(!meets_minimum_strength(&AuthnStrength::High, &AuthnStrength::Highest));
    }
    ...
    fn test_mfa_validation_required_and_met() {
        let assertion = create_test_assertion(
            Some("urn:oasis:names:tc:SAML:2.0:ac:classes:MobileTwoFactorContract".to_string())
        );
    ...
    fn test_enhanced_saml_config_structure() {
        ...
        let compliance_config = SamlComplianceConfig {
            ...
            minimum_signature_algorithm: "http://www.w3.org/2001/04/xmldsig-more#rsa-sha256".to_string(),
    ...
    fn test_mfa_requirement_serialization() {
        ...
        let deserialized: Result<SamlMfaRequirement, _> = serde_json::from_str(&serialized.unwrap());
    ...
    fn test_complex_mfa_scenario() {
        ...
        let assertion_high = create_test_assertion(
            Some("urn:oasis:names:tc:SAML:2.0:ac:classes:Smartcard".to_string())
        );
        
        let assertion_low = create_test_assertion(
            Some("urn:oasis:names:tc:SAML:2.0:ac:classes:Password".to_string())
        );
    ...
    fn test_edge_case_empty_contexts() {
        let assertion = create_test_assertion(
            Some("urn:oasis:names:tc:SAML:2.0:ac:classes:SmartcardPKI".to_string())
        );
...
mod saml_integration_tests {
    ...
    fn test_enterprise_saml_configuration() {
        use oxirs_fuseki::handlers::saml::{SamlSpConfig, SamlIdpConfig, EndpointCapabilities};
        ...
        idp_configs.insert("corporate_idp".to_string(), SamlIdpConfig {
        ...
        let compliance_config = SamlComplianceConfig {
            audit_all_assertions: true,
            require_encryption: true,
            minimum_signature_algorithm: "http://www.w3.org/2001/04/xmldsig-more#rsa-sha384".to_string(),
            blacklisted_algorithms: vec![
                "http://www.w3.org/2000/09/xmldsig#rsa-sha1".to_string(),
        ...
        assert_eq!(enhanced_config.sp_config.entity_id, "https://fuseki.enterprise.com");
        ...
        assert_eq!(enhanced_config.session_config.max_session_duration_hours, 12);
        ...
        assert_eq!(enhanced_config.compliance_config.blacklisted_algorithms.len(), 2);
    }
