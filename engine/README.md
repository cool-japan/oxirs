...
# Full engine (includes all modules)
oxirs-engine = "0.1"
```
...
```rust
use oxirs_engine::{Engine, EngineConfig};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let config = EngineConfig::builder()
        ...
        .enable_rdf_star(true)
        .build();
    
    let engine = Engine::new(config).await?;
    
    // Load data
    engine.load_rdf_file("data.ttl").await?;
    engine.load_shapes_file("shapes.ttl").await?;
    ...
    let sparql = r#"
        ...
        SELECT ?entity ?similarity WHERE {
            ...
            ?entity vec:similar "AI researcher" .
    ...
    let results = engine.query(sparql).await?;
    
    // Validate with SHACL
    let validation_report = engine.validate().await?;
