...
use oxirs_rule::{
    ...
    performance::{RuleEngineProfiler, PerformanceTestHarness, PerformanceThresholds},
...
fn comprehensive_testing_example() -> anyhow::Result<()> {
    ...
    for (fact_count, rule_count, description) in test_cases {
        println!("\n🔬 Testing {}: {} facts, {} rules", description, fact_count, rule_count);
        ...
        println!("  Peak memory: {} bytes", metrics.memory_stats.peak_memory_usage);
...
fn memory_stress_testing_example() -> anyhow::Result<()> {
    ...
    for scale in stress_levels {
        ...
        println!("  Facts memory: {} KB", metrics.memory_stats.facts_memory / 1024);
        println!("  Rules memory: {} KB", metrics.memory_stats.rules_memory / 1024);
        
        if metrics.memory_stats.peak_memory_usage > 100 * 1024 * 1024 { // 100 MB
