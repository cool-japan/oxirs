    ...
    for fact_size in [50, 100, 200, 500].iter() {
        for rule_count in [5, 10, 15].iter() {
            ...
            group.bench_with_input(
                BenchmarkId::new("rule_execution", format!("{}facts_{}rules", fact_size, rule_count)),
                &(facts, rules),
                |b, (facts, rules)| {
                    b.iter(|| {
                    ...
                    let rule = SwrlRule {
                        ...
                        body: vec![
                            SwrlAtom::Builtin {
                                ...
                                arguments: args.iter().map(|&s| SwrlArgument::Literal(s.to_string())).collect(),
                            },
                        ],
                        head: vec![
                            SwrlAtom::Class {
