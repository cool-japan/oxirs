...
impl NTriplesParser {
    ...
    pub fn parse_line(&self, line: &str, line_number: usize) -> TurtleResult<Option<Triple>> {
        ...
        if tokens.len() != 3 {
            return Err(TurtleParseError::syntax(TurtleSyntaxError::Generic {
                message: format!("Expected 3 tokens (subject predicate object), found {}", tokens.len()),
    ...
    fn parse_subject(&self, token: &str, line_number: usize) -> TurtleResult<Subject> {
        if token.starts_with('<') && token.ends_with('>') {
            ...
            let named_node = NamedNode::new(iri)
                .map_err(|e| TurtleParseError::model(e))?;
                ...
                match chars.next() {
                    ...
                    Some('u') => {
                        ...
                        let code_point = u32::from_str_radix(&code, 16)
                            .map_err(|_| TurtleParseError::syntax(TurtleSyntaxError::InvalidEscape {
                                sequence: format!("u{}", code),
                                position: TextPosition::start(),
                            }))?;
                        let unicode_char = char::from_u32(code_point)
                            .ok_or_else(|| TurtleParseError::syntax(TurtleSyntaxError::InvalidUnicode {
...
impl Parser<Triple> for NTriplesParser {
    fn parse<R: Read>(&self, reader: R) -> TurtleResult<Vec<Triple>> {
        ...
        if !errors.is_empty() && self.lenient {
            eprintln!("Warning: {} errors encountered during parsing", errors.len());
        }
        
        Ok(triples)
    }
    
    fn for_reader<R: BufRead + 'static>(&self, reader: R) -> Box<dyn Iterator<Item = TurtleResult<Triple>>> {
...
impl NTriplesSerializer {
    ...
    /// Serialize a subject
    fn serialize_subject<W: Write>(&self, subject: &Subject, writer: &mut FormattedWriter<W>) -> TurtleResult<()> {
        match subject {
            Subject::NamedNode(nn) => {
                writer.write_str(&format!("<{}>", nn.as_str())).map_err(TurtleParseError::io)?;
            }
    ...
    fn serialize_predicate<W: Write>(&self, predicate: &Predicate, writer: &mut FormattedWriter<W>) -> TurtleResult<()> {
        match predicate {
    ...
    /// Serialize an object
    fn serialize_object<W: Write>(&self, object: &Object, writer: &mut FormattedWriter<W>) -> TurtleResult<()> {
        match object {
            Object::NamedNode(nn) => {
                writer.write_str(&format!("<{}>", nn.as_str())).map_err(TurtleParseError::io)?;
            }
            ...
            Object::Literal(literal) => {
                ...
                if let Some(language) = literal.language() {
                    ...
                    writer.write_str(&format!("^^<{}>", literal.datatype().as_str())).map_err(TurtleParseError::io)?;
                }
            }
            Object::Variable(var) => {
                writer.write_str(&format!("?{}", var.as_str())).map_err(TurtleParseError::io)?;
