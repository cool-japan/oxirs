...
use oxirs_core::model::{Triple, Quad};
    ...
    StreamingParser<R, T, P> 
{
    ...
    pub fn new(tokenizer: crate::toolkit::lexer::StreamingTokenizer<R, T>, rule_recognizer: P, context: P::Context) -> Self {
        Self {
            tokenizer,
            rule_recognizer,
            context,
    ...
    fn next(&mut self) -> Option<Self::Item> {
        loop {
            match self.tokenizer.next() {
                ...
                Some(Err(e)) => return Some(Err(TurtleParseError::syntax(
...
pub trait Parser<Output> {
    ...
    fn for_reader<R: BufRead + 'static>(&self, reader: R) -> Box<dyn Iterator<Item = TurtleResult<Output>>>;
}
