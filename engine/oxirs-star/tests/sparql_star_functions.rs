...
use oxirs_star::{
    StarStore, StarTriple, StarTerm, StarGraph,
    query::{QueryExecutor, BasicGraphPattern, TriplePattern, TermPattern},
    functions::{Expression, ExpressionEvaluator, StarFunction, FunctionEvaluator},
};
...
fn test_subject_predicate_object_functions() {
    ...
    assert_eq!(subject_result, StarTerm::iri("http://example.org/paris").unwrap());
    ...
    assert_eq!(predicate_result, StarTerm::iri("http://example.org/capitalOf").unwrap());
    ...
    assert_eq!(object_result, StarTerm::iri("http://example.org/france").unwrap());
}
...
fn test_is_triple_function() {
    ...
    bindings.insert("y".to_string(), StarTerm::iri("http://example.org/not_a_triple").unwrap());
...
fn test_nested_quoted_triple_functions() {
    ...
    let subject = FunctionEvaluator::evaluate(
    ...
    let alice = FunctionEvaluator::evaluate(
        StarFunction::Subject,
        &[inner_subject],
    ).unwrap();
...
fn test_triple_construction_and_comparison() {
    let mut bindings = HashMap::new();
    bindings.insert("s".to_string(), StarTerm::iri("http://example.org/subject").unwrap());
    bindings.insert("p".to_string(), StarTerm::iri("http://example.org/predicate").unwrap());
...
fn test_sparql_star_end_to_end() {
    ...
    for binding in &bindings {
        if let Some(fact) = binding.get("fact") {
            ...
            assert_eq!(triple.subject, StarTerm::iri("http://example.org/alice").unwrap());
        }
    }
}