...
fn query_pattern_strategy() -> impl Strategy<Value = (Option<StarTerm>, Option<StarTerm>, Option<StarTerm>)> {
    (
        prop::option::of(prop::string::string_regex("http://example.org/[a-z]+[0-9]*").unwrap()
            .prop_map(|s| StarTerm::iri(&s).unwrap())),
        prop::option::of(prop::string::string_regex("http://example.org/[a-z]+").unwrap()
            .prop_map(|s| StarTerm::iri(&s).unwrap())),
