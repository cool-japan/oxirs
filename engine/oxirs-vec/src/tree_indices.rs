...
impl BallTree {
    ...
    pub fn build(&mut self) -> Result<()> {
        ...
        let points: Vec<Vec<f32>> = self.data
            .iter()
            .map(|(_, v)| v.as_f32())
            .collect();
    ...
    fn build_node(&self, points: &[Vec<f32>], indices: Vec<usize>) -> Result<BallNode> {
        ...
        let radius = left_node.radius.max(right_node.radius) + 
                    self.config.distance_metric.distance(&center, &left_node.center);
    ...
    fn partition_indices(&self, points: &[Vec<f32>], indices: &[usize], dim: usize) -> (Vec<usize>, Vec<usize>) {
        let mut values: Vec<(f32, usize)> = indices.iter()
            .map(|&idx| (points[idx][dim], idx))
            .collect();
    ...
    fn search_node(
        ...
        if node.indices.is_empty() {
            ...
            for &idx in &node.indices {
                ...
                if heap.len() < k {
                    heap.push(SearchResult { index: idx, distance: dist });
...
impl KdTree {
    ...
    fn build_node(&self, points: &[Vec<f32>], indices: Vec<usize>, depth: usize) -> Result<KdNode> {
        ...
        let left_indices: Vec<usize> = values[..median_idx]
            .iter()
            .map(|(_, idx)| *idx)
            .collect();
        
        let right_indices: Vec<usize> = values[median_idx..]
            .iter()
            .map(|(_, idx)| *idx)
            .collect();
        
        let left = if !left_indices.is_empty() {
            Some(Box::new(self.build_node(points, left_indices, depth + 1)?))
        } else {
            None
        };
        
        let right = if !right_indices.is_empty() {
            Some(Box::new(self.build_node(points, right_indices, depth + 1)?))
        } else {
            None
        };
...
impl VpTree {
    ...
    pub fn search(&self, query: &[f32], k: usize) -> Vec<(usize, f32)> {
        ...
        let mut heap = BinaryHeap::new();
        self.search_node(self.root.as_ref().unwrap(), query, k, &mut heap, f32::INFINITY);
    ...
    fn search_node(
        ...
        if dist_to_vp < tau {
            if heap.len() < k {
                heap.push(SearchResult { index: node.vantage_point, distance: dist_to_vp });
...
impl TreeIndex {
    pub fn new(config: TreeIndexConfig) -> Self {
        ...
        let (ball_tree, kd_tree, vp_tree, cover_tree, rp_tree) = match tree_type {
            ...
            TreeType::RandomProjectionTree => (None, None, None, None, Some(RandomProjectionTree::new(config))),
        };
