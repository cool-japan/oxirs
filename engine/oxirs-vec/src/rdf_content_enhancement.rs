...
impl RdfContentProcessor {
    ...
    pub fn new(
        ...
        let embedding_manager = Arc::new(RwLock::new(EmbeddingManager::new(embedding_strategy, 1000)?));
    ...
    pub fn generate_entity_embedding(&mut self, entity: &RdfEntity) -> Result<Vector> {
        ...
        if self.config.enable_property_aggregation && !entity.properties.is_empty() {
            let prop_embedding = self.property_aggregator.aggregate_properties(&entity.properties)?;
        ...
        // Cache the result
        self.entity_cache.insert(entity.uri.clone(), final_embedding.clone());
    ...
    pub fn generate_property_path_embedding(&mut self, path: &PropertyPath) -> Result<Vector> {
        ...
        // Cache the result
        self.relationship_cache.insert(path_key, path_embedding.clone());
    ...
    fn generate_description_embedding(&self, descriptions: &HashMap<String, String>) -> Result<Vector> {
        let preferred_lang = &self.config.default_language;
...
impl PropertyAggregator {
    ...
    pub fn aggregate_properties(&self, properties: &HashMap<String, Vec<RdfValue>>) -> Result<Vector> {
        ...
        for (property_uri, values) in properties {
            ...
            for value in values {
                match value {
                    ...
                    RdfValue::Date(d) | RdfValue::DateTime(d) => property_text.push_str(&format!(" {}", d)),
                }
            }
        ...
        match self.aggregation_strategy {
            ...
            AggregationStrategy::WeightedMean => self.weighted_mean_aggregation(property_embeddings),
...
mod tests {
    ...
    fn test_property_path() {
        let path = PropertyPath {
            ...
            constraints: vec![PathConstraint::TypeFilter("http://example.org/Person".to_string())],
        };
