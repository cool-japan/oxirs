...
impl MinHashFunction {
    fn new(num_hashes: usize, seed: u64) -> Self {
        ...
        let a: Vec<u64> = (0..num_hashes)
            .map(|_| rng.gen_range(1..prime))
            .collect();
        let b: Vec<u64> = (0..num_hashes)
            .map(|_| rng.gen_range(0..prime))
            .collect();
...
impl HashFunction for MinHashFunction {
    fn hash(&self, vector: &[f32]) -> u64 {
        ...
        let threshold = 0.0;
        let set_elements: Vec<u32> = vector.iter()
            .enumerate()
...
impl PStableHash {
    fn new(dimensions: usize, num_projections: usize, width: f32, p: f32, seed: u64) -> Self {
        ...
        let projections: Vec<Vec<f32>> = if (p - 1.0).abs() < 0.1 {
            ...
            (0..num_projections)
                .map(|_| {
                    (0..dimensions)
                        .map(|_| {
                            let u: f32 = rng.gen_range(-std::f32::consts::PI/2.0..std::f32::consts::PI/2.0);
                            u.tan()
                        })
                        .collect()
                })
        ...
        Self { projections, offsets, width, p }
    }
}
        ...
        for table_idx in 0..config.num_tables {
            ...
            let hash_function: Box<dyn HashFunction> = match config.lsh_family {
                LshFamily::RandomProjection => {
                    Box::new(RandomProjectionHash::new(
                        dimensions,
                ...
                LshFamily::SimHash => {
                    Box::new(SimHashFunction::new(dimensions, seed))
                }
                LshFamily::PStable(p) => {
    ...
    pub fn stats(&self) -> LshStats {
        let avg_bucket_size = if self.tables.is_empty() {
            ...
            let total_buckets: usize = self.tables.iter()
                .map(|t| t.buckets.len())
                .sum();
            let total_items: usize = self.tables.iter()
    ...
    fn estimate_memory_usage(&self) -> usize {
        ...
        let table_memory: usize = self.tables.iter()
            .map(|t| {
                ...
                t.buckets.values().map(|v| v.len() * std::mem::size_of::<usize>()).sum::<usize>()
            })
            .sum();
                ...
                self.vectors.get(id).map(|(_, vec)| {
                    ...
                    let distance = match self.config.lsh_family {
                        ...
                        LshFamily::MinHash => {
                            // Jaccard distance
                            let threshold = 0.0;
                            let set1: HashSet<usize> = query_f32.iter()
                                .enumerate()
                                .filter(|(_, &v)| v > threshold)
                                .map(|(i, _)| i)
                                .collect();
                            let set2: HashSet<usize> = vec_f32.iter()
                                .enumerate()
                                .filter(|(_, &v)| v > threshold)
                                .map(|(i, _)| i)
                                .collect();
...
mod tests {
    ...
    fn test_random_projection_lsh() {
        ...
        index.insert("v_similar".to_string(), v_similar.clone()).unwrap();
        ...
        assert!(results.iter().any(|(uri, _)| uri == "v1" || uri == "v_similar"));
    }
    ...
    fn test_minhash_lsh() {
        ...
        v1[0] = 1.0; v1[10] = 1.0; v1[20] = 1.0;
        
        let mut v2 = vec![0.0; 100];
        v2[0] = 1.0; v2[10] = 1.0; v2[30] = 1.0; // 2/4 overlap with v1
        
        let mut v3 = vec![0.0; 100];
        v3[50] = 1.0; v3[60] = 1.0; v3[70] = 1.0; // No overlap with v1
        
        index.insert("v1".to_string(), Vector::new(v1.clone())).unwrap();
