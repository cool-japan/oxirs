...
impl RealTimeEmbeddingPipeline {
    ...
    pub fn new(config: PipelineConfig) -> PipelineResult<Self> {
        ...
        let update_coordinator = Arc::new(
            UpdateCoordinator::new(&config)
        ...
        let performance_monitor = Arc::new(
            PipelinePerformanceMonitor::new(config.monitoring_config.clone())
        ...
        let version_manager = Arc::new(
            VersionManager::new(config.version_control.clone())
        ...
        let consistency_manager = Arc::new(
            ConsistencyManager::new(config.consistency_level.clone())
    ...
    pub async fn stop(&self) -> PipelineResult<()> {
        ...
        {
            let processors = self.stream_processors.read()
    ...
    pub fn add_embedding_generator(
        ...
        let mut generators = self.embedding_generators.write()
            .map_err(|_| PipelineError::CoordinationError { 
    ...
    pub async fn create_stream(&self, config: StreamConfig) -> PipelineResult<String> {
        ...
        {
            let mut processors = self.stream_processors.write()
                .map_err(|_| PipelineError::CoordinationError { 
                    message: "Failed to acquire stream processors lock".to_string() 
                })?;
    ...
    pub async fn health_check(&self) -> PipelineResult<crate::real_time_embedding_pipeline::types::HealthCheckResult> {
        ...
        components.insert(
            ...
            self.consistency_manager.health_check().await
                .map_err(|e| PipelineError::ConsistencyError { 
        ...
        let overall_status = if components.values().all(|status| {
            matches!(status, crate::real_time_embedding_pipeline::traits::HealthStatus::Healthy)
        }) {
            crate::real_time_embedding_pipeline::traits::HealthStatus::Healthy
        } else if components.values().any(|status| {
            matches!(status, crate::real_time_embedding_pipeline::traits::HealthStatus::Unhealthy { .. })
        }) {
        ...
        Ok(crate::real_time_embedding_pipeline::types::HealthCheckResult {
    ...
    pub fn list_embedding_generators(&self) -> PipelineResult<Vec<String>> {
        let generators = self.embedding_generators.read()
            .map_err(|_| PipelineError::CoordinationError { 
                message: "Failed to acquire generators lock".to_string() 
            })?;
    ...
    async fn start_update_coordinator(&self) -> PipelineResult<()> {
        self.update_coordinator.start().await
...
mod tests {
    ...
    fn test_pipeline_configuration() {
        ...
        assert_eq!(pipeline.get_config().consistency_level, ConsistencyLevel::Strong);
    }
}