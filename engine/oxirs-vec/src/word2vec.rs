...
use crate::{Vector, embeddings::{EmbeddableContent, EmbeddingConfig, EmbeddingGenerator}};
...
impl Word2VecEmbeddingGenerator {
        ...
        for (word, embedding) in &self.embeddings {
            ...
            for subword in subwords {
                *subword_counts.entry(subword.clone()).or_insert(0) += 1;
                ...
                let sum = subword_sums.entry(subword).or_insert_with(|| vec![0.0; self.config.dimensions]);
    ...
    fn aggregate_embeddings(&self, word_embeddings: &[(String, Vec<f32>)]) -> Vec<f32> {
        ...
        match self.config.aggregation {
            ...
            AggregationMethod::MeanMax => {
                ...
                let mean = self.aggregate_embeddings_with_method(word_embeddings, AggregationMethod::Mean);
                let max = self.aggregate_embeddings_with_method(word_embeddings, AggregationMethod::Max);
            ...
            AggregationMethod::TfIdfWeighted => {
                ...
                if self.doc_frequencies.is_empty() {
                    ...
                    return self.aggregate_embeddings_with_method(word_embeddings, AggregationMethod::Mean);
                }
                ...
                for (word, embedding) in word_embeddings {
                    let tf = word_embeddings.iter().filter(|(w, _)| w == word).count() as f32 / word_embeddings.len() as f32;
    ...
    fn aggregate_embeddings_with_method(&self, word_embeddings: &[(String, Vec<f32>)], method: AggregationMethod) -> Vec<f32> {
...
mod tests {
    ...
    fn test_word2vec_generator() {
        ...
        generator.embeddings.insert("hello".to_string(), vec![0.1; 100]);
        generator.embeddings.insert("world".to_string(), vec![0.2; 100]);
    ...
    fn test_subword_generation() {
        ...
        let generator = Word2VecEmbeddingGenerator::new(config, EmbeddingConfig::default()).unwrap();
    ...
    fn test_aggregation_methods() {
        ...
        for method in [
            ...
            let mut generator = Word2VecEmbeddingGenerator::new(config.clone(), embedding_config.clone()).unwrap();
            
            generator.embeddings.insert("a".to_string(), vec![1.0, 2.0, 3.0]);
            generator.embeddings.insert("b".to_string(), vec![4.0, 5.0, 6.0]);
