...
impl HnswIndex {
    ...
    pub fn update_cache_stats(&self, hits: u64, misses: u64) {
        self.stats.cache_hits.fetch_add(hits, std::sync::atomic::Ordering::Relaxed);
        self.stats.cache_misses.fetch_add(misses, std::sync::atomic::Ordering::Relaxed);
    }

    /// Update SIMD operation statistics
    pub fn update_simd_stats(&self, operations: u64) {
        self.stats.simd_operations.fetch_add(operations, std::sync::atomic::Ordering::Relaxed);
    }

    /// Update prefetch statistics
    pub fn update_prefetch_stats(&self, operations: u64) {
        self.stats.prefetch_operations.fetch_add(operations, std::sync::atomic::Ordering::Relaxed);
    }
    ...
    pub fn get_optimal_thread_count(&self, work_size: usize) -> usize {
        ...
        let optimal = (work_size / 1000).max(1).min(max_threads);
        optimal
    }
}