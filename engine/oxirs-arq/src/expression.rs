...
use crate::algebra::{
    Aggregate, BinaryOperator, Expression as AlgebraExpression, UnaryOperator,
};
use crate::extensions::{ExtensionRegistry, ExecutionContext as ExtContext};
...
impl ExpressionEvaluator {
    ...
    pub fn evaluate(&self, expr: &AlgebraExpression) -> Result<Term> {
        match expr {
            ...
            AlgebraExpression::Literal(lit) => {
                Ok(Term::from_algebra_term(&crate::algebra::Term::Literal(lit.clone())))
            }
            ...
            AlgebraExpression::Conditional { condition, then_expr, else_expr } => {
            ...
            AlgebraExpression::Bound(var) => {
                Ok(Term::typed_literal(
                    if self.binding_context.is_bound(var) { "true" } else { "false" },
                    xsd::BOOLEAN
                )?)
            }
    ...
    fn evaluate_function(&self, name: &str, args: &[AlgebraExpression]) -> Result<Term> {
        // Evaluate arguments
        let arg_values: Vec<Term> = args.iter()
    ...
    fn builtin_substr(&self, args: &[Term]) -> Result<Term> {
        ...
        if let (Term::Literal(str_lit), Term::Literal(start_lit)) = (str_term, start) {
            let start_pos = start_lit.lexical_form.parse::<usize>()
            ...
            let result = if args.len() == 3 {
                ...
                if let Term::Literal(len_lit) = len {
                    let length = len_lit.lexical_form.parse::<usize>()
    ...
    fn builtin_replace(&self, args: &[Term]) -> Result<Term> {
        ...
        if let (Term::Literal(input_lit), Term::Literal(pattern_lit), Term::Literal(repl_lit)) = 
            ...
            let result = input_lit.lexical_form.replace(&pattern_lit.lexical_form, &repl_lit.lexical_form);
            Ok(Term::literal(&result))
        } else {
            unreachable!()
        }
    ...
    fn builtin_strlang(&self, args: &[Term]) -> Result<Term> {
        ...
        if let (Term::Literal(str_lit), Term::Literal(lang_lit)) = (str_val, lang_val) {
            Ok(Term::lang_literal(&str_lit.lexical_form, &lang_lit.lexical_form))
        } else {
            unreachable!()
        }
    }
...
mod tests {
    ...
    fn test_expression_evaluation() {
        ...
        evaluator.binding_context_mut().bind("y", Term::typed_literal("3.14", xsd::DOUBLE).unwrap());
        
        // Test variable evaluation
        let expr = AlgebraExpression::Variable("x".to_string());
        ...
        let add_expr = AlgebraExpression::Binary {
            ...
            left: Box::new(AlgebraExpression::Variable("x".to_string())),
            right: Box::new(AlgebraExpression::Literal(crate::algebra::Literal {
                value: "8".to_string(),
                language: None,
                datatype: Some(crate::algebra::Iri(xsd::INTEGER.to_string())),
            })),
        };
