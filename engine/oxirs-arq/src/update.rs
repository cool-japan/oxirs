...
impl<'a> UpdateExecutor<'a> {
    ...
    pub fn execute(&mut self, operation: &UpdateOperation) -> Result<UpdateResult, OxirsError> {
        match operation {
            ...
            UpdateOperation::Load { source, graph, silent } => {
    ...
    /// Execute CLEAR
    fn execute_clear(&mut self, target: &GraphTarget, silent: bool) -> Result<UpdateResult, OxirsError> {
        let mut result = UpdateResult::default();
        ...
        match target {
            ...
            GraphTarget::Graph(graph_ref) => {
                ...
                match self.store.clear_graph(Some(&GraphName::NamedNode(graph_name))) {
    ...
    /// Execute DROP
    fn execute_drop(&mut self, target: &GraphTarget, silent: bool) -> Result<UpdateResult, OxirsError> {
        let mut result = UpdateResult::default();
        ...
        match target {
            GraphTarget::All => {
                ...
                for graph in graphs {
                    self.store.drop_graph(Some(&GraphName::NamedNode(graph.clone())))?;
            ...
            GraphTarget::Graph(graph_ref) => {
                let graph_name = self.graph_ref_to_named_node(graph_ref)?;
                match self.store.drop_graph(Some(&GraphName::NamedNode(graph_name.clone()))) {
    ...
    /// Execute CREATE
    fn execute_create(&mut self, graph: &GraphReference, silent: bool) -> Result<UpdateResult, OxirsError> {
        let mut result = UpdateResult::default();
    ...
    /// Execute COPY
    fn execute_copy(&mut self, from: &GraphTarget, to: &GraphTarget, silent: bool) -> Result<UpdateResult, OxirsError> {
    ...
    /// Execute MOVE
    fn execute_move(&mut self, from: &GraphTarget, to: &GraphTarget, silent: bool) -> Result<UpdateResult, OxirsError> {
        // First copy
    ...
    /// Execute ADD
    fn execute_add(&mut self, from: &GraphTarget, to: &GraphTarget, _silent: bool) -> Result<UpdateResult, OxirsError> {
        let mut result = UpdateResult::default();
        ...
        let target_graph = match to {
            ...
            _ => return Err(OxirsError::Query("Invalid target for ADD operation".to_string())),
        };
    ...
    /// Execute LOAD
    fn execute_load(&mut self, source: &str, graph: Option<&GraphReference>, silent: bool) -> Result<UpdateResult, OxirsError> {
        let mut result = UpdateResult::default();
    ...
    fn pattern_to_quad(&self, pattern: &QuadPattern) -> Result<Quad, OxirsError> {
        ...
        let graph_name = pattern.graph.as_ref()
            .map(|g| self.graph_ref_to_named_node(g))
            .transpose()?
    ...
    fn term_to_object(&self, term: &Term) -> Result<oxirs_core::model::Object, OxirsError> {
        match term {
            ...
            Term::Variable(_) => Err(OxirsError::Query("Variables not allowed in concrete data".to_string())),
        }
    }
    ...
    fn graph_ref_to_named_node(&self, graph_ref: &GraphReference) -> Result<NamedNode, OxirsError> {
        match graph_ref {
            ...
            GraphReference::Default => Err(OxirsError::Query("DEFAULT is not a valid graph IRI".to_string())),
        }
    }
    ...
    fn instantiate_template(
        ...
        let graph_name = template.graph.as_ref()
            .map(|g| self.graph_ref_to_named_node(g))
            .transpose()?
            .map(GraphName::NamedNode)
            .unwrap_or(GraphName::DefaultGraph);
...
mod tests {
    ...
    fn test_quad_pattern() {
        let pattern = QuadPattern {
            ...
            predicate: Term::Iri(crate::algebra::Iri("http://example.org/pred".to_string())),
        ...
        assert_eq!(pattern.subject, Term::Variable("s".to_string()));
    }
}