...
pub enum ExecutionStrategy {
    ...
    Streaming,
    // /// Use vectorized SIMD execution for high performance
    // Vectorized, // Temporarily disabled - requires scirs2-core beta.4
...
pub struct QueryExecutor {
    ...
    // vectorized_context: Option<vectorized::VectorizedExecutionContext>, // Temporarily disabled
    #[allow(dead_code)]
...
impl QueryExecutor {
    ...
    pub fn new() -> Self {
        ...
        // // Initialize vectorized execution context - Temporarily disabled
        // let vectorized_context = match vectorized::VectorizedExecutionContext::new(
        ...
        Self {
            ...
            parallel_executor,
            // vectorized_context, // Temporarily disabled
    ...
    pub fn execute(
        ...
        let result = match strategy {
            ...
            //             ExecutionStrategy::Vectorized => self.execute_vectorized(algebra, dataset),
            ExecutionStrategy::Adaptive => {
                // Adaptive strategy chooses best approach based on query characteristics
                //                 if self.should_use_vectorized(algebra) {
                //                     self.execute_vectorized(algebra, dataset)
                // } else
    ...
    //         self.is_vectorizable_algebra(algebra) &&
    //         self.estimate_cardinality(algebra) > 1000 &&
    ...
    //         match algebra {
    //             Algebra::Bgp(_) => true,  // BGP patterns are highly vectorizable
    //             Algebra::Join { left, right } => {
    //                 self.is_vectorizable_algebra(left) && self.is_vectorizable_algebra(right)
    //             }
    //             Algebra::Union { left, right } => {
    //                 self.is_vectorizable_algebra(left) && self.is_vectorizable_algebra(right)
    //             }
    //             Algebra::Filter { pattern, .. } => self.is_vectorizable_algebra(pattern),
    //             Algebra::Project { pattern, .. } => self.is_vectorizable_algebra(pattern),
    //             Algebra::Distinct { pattern } => self.is_vectorizable_algebra(pattern),
    ...
    //         } else {
    //             // Fall back to serial execution if vectorized context not available
    //             self.execute_serial(algebra, dataset)
    //         }
    //     }
    ...
    //         let all_triples_pattern = TriplePattern {
    ...
    //             // Try to initialize vectorized context
    //             if let Ok(vc) = vectorized::VectorizedExecutionContext::new(
    //                 vectorized::VectorizedConfig::default()
    //             ) {
    //                 self.vectorized_context = Some(vc);
    //             }
    //         } else if !enabled {
