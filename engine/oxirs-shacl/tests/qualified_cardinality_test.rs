...
fn test_qualified_cardinality_validation() {
    ...
    store.insert_quad(Quad::new(alice.clone(), knows_pred.clone(), bob.clone(), GraphName::DefaultGraph)).unwrap();
    ...
    store.insert_quad(Quad::new(bob.clone(), type_pred.clone(), person_type.clone(), GraphName::DefaultGraph)).unwrap();
    store.insert_quad(Quad::new(bob.clone(), type_pred.clone(), friend_type.clone(), GraphName::DefaultGraph)).unwrap();
    ...
    assert!(report.conforms(), "Alice should conform to qualified cardinality constraint");
...
fn test_qualified_cardinality_min_violation() {
    ...
    assert!(!report.conforms(), "Alice should NOT conform - knows 0 Friends but needs at least 1");
...
fn test_qualified_cardinality_max_violation() {
    ...
    store.insert_quad(Quad::new(alice.clone(), knows_pred.clone(), bob.clone(), GraphName::DefaultGraph)).unwrap();
    store.insert_quad(Quad::new(alice.clone(), knows_pred.clone(), david.clone(), GraphName::DefaultGraph)).unwrap();
    
    // Type information
    store.insert_quad(Quad::new(alice.clone(), type_pred.clone(), person_type.clone(), GraphName::DefaultGraph)).unwrap();
    store.insert_quad(Quad::new(bob.clone(), type_pred.clone(), friend_type.clone(), GraphName::DefaultGraph)).unwrap();
    store.insert_quad(Quad::new(david.clone(), type_pred.clone(), friend_type.clone(), GraphName::DefaultGraph)).unwrap();
    ...
    assert!(!report.conforms(), "Alice should NOT conform - knows 2 Friends but max allowed is 1");
    assert!(report.violation_count() > 0);
}
...
fn test_qualified_cardinality_range_success() {
    ...
    assert!(report.conforms(), "Alice should conform - knows 2 Friends which is within range 1-3");
    assert_eq!(report.violation_count(), 0);
}