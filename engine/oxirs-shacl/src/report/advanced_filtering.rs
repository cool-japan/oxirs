...
use crate::{
    report::{ValidationReport, ReportFormat},
...
impl ReportFilterEngine {
    ...
    pub fn filter_report(&mut self, report: &ValidationReport) -> Result<FilteredReport> {
        ...
        if !self.filter_config.severity_filters.is_empty() {
            filtered_violations.retain(|v| {
                self.filter_config.severity_filters.contains(&v.result_severity)
            });
        }
        ...
        if !self.filter_config.shape_filters.is_empty() {
            filtered_violations.retain(|v| {
                self.filter_config.shape_filters.contains(&v.source_shape)
            });
        }
        ...
        if !self.filter_config.path_filters.is_empty() {
            filtered_violations.retain(|v| {
                if let Some(path) = &v.result_path {
                    self.filter_config.path_filters.iter().any(|filter_path| {
                        path.to_string().contains(filter_path)
                    })
                } else {
                    false
                }
        ...
        if !self.filter_config.component_filters.is_empty() {
            filtered_violations.retain(|v| {
                self.filter_config.component_filters.contains(&v.source_constraint_component)
            });
        }
        ...
        if !self.filter_config.focus_node_filters.is_empty() {
            filtered_violations.retain(|v| {
                self.filter_config.focus_node_filters.iter().any(|filter_node| {
                    v.focus_node.to_string().contains(filter_node)
                })
            });
        }
    ...
    fn apply_custom_filter(
        ...
        match &custom_filter.filter_function {
            FilterFunction::ContainsText(text) => {
                Ok(violations
                    .iter()
                    .filter(|v| {
                        v.result_message
            ...
            FilterFunction::RegexPattern { field, pattern } => {
                ...
                Ok(violations
                    ...
                    .filter(|v| {
                        match field {
                            FilterField::ViolationMessage => {
                                v.result_message
                                    .as_ref()
                                    .map(|msg| regex.is_match(msg))
                                    .unwrap_or(false)
                            }
                            FilterField::FocusNode => {
                                regex.is_match(&v.focus_node.to_string())
                            }
                            FilterField::ShapeId => {
                                regex.is_match(v.source_shape.as_str())
                            }
                            FilterField::PropertyPath => {
                                v.result_path
                                    .as_ref()
    ...
    fn apply_template(&self, report: &ValidationReport, template: &ReportTemplate) -> Result<String> {
    ...
    fn generate_html_from_template(&self, report: &ValidationReport, template: &ReportTemplate) -> Result<String> {
        ...
        // Add body
        html.push_str(&self.interpolate_template_variables(&template.template_content.body, report)?);
    ...
    fn generate_json_from_template(&self, report: &ValidationReport, _template: &ReportTemplate) -> Result<String> {
    ...
    fn generate_text_from_template(&self, report: &ValidationReport, template: &ReportTemplate) -> Result<String> {
        ...
        // Add body
        text.push_str(&self.interpolate_template_variables(&template.template_content.body, report)?);
    ...
    fn interpolate_template_variables(&self, template: &str, report: &ValidationReport) -> Result<String> {
        ...
        result = result.replace("{{TIMESTAMP}}", &Utc::now().format("%Y-%m-%d %H:%M:%S UTC").to_string());
        
        // Count violations by severity
        let error_count = report.violations.iter()
            .filter(|v| v.result_severity == Severity::Violation)
            .count();
        let warning_count = report.violations.iter()
...
mod tests {
    ...
    fn test_severity_filtering() {
        ...
        assert_eq!(filtered.filtered_violations[0].result_severity, Severity::Violation);
    }
