...
pub fn generate_turtle_report(report: &ValidationReport) -> Result<String> {
    ...
    writeln!(output, "    sh:conforms {} ;", if report.conforms() { "true" } else { "false" })?;
    ...
    if !report.violations().is_empty() {
        ...
        for (i, violation) in report.violations().iter().enumerate() {
            ...
            if let Some(focus_node) = &violation.focus_node {
                writeln!(output, "            sh:focusNode {} ;", format_term_turtle(focus_node))?;
            }
            
            // Result path
            if let Some(path) = &violation.result_path {
                writeln!(output, "            sh:resultPath {} ;", format_path_turtle(path))?;
            }
            
            // Value
            if let Some(value) = &violation.value {
                writeln!(output, "            sh:value {} ;", format_term_turtle(value))?;
            }
            
            // Source constraint component
            if let Some(component) = &violation.source_constraint_component {
                writeln!(output, "            sh:sourceConstraintComponent {} ;", format_iri_turtle(component.as_str()))?;
            }
            
            // Source shape
            if let Some(shape) = &violation.source_shape {
                writeln!(output, "            sh:sourceShape {} ;", format_iri_turtle(shape.as_str()))?;
            }
            ...
            if let Some(message) = &violation.result_message {
                writeln!(output, "            sh:resultMessage \"{}\" ;", escape_turtle_string(message))?;
            }
...
pub fn generate_jsonld_report(report: &ValidationReport) -> Result<String> {
    ...
    context.insert("sh".to_string(), Value::String("http://www.w3.org/ns/shacl#".to_string()));
    context.insert("rdf".to_string(), Value::String("http://www.w3.org/1999/02/22-rdf-syntax-ns#".to_string()));
    context.insert("conforms".to_string(), Value::String("sh:conforms".to_string()));
    ...
    // Type
    json_report.insert("@type".to_string(), Value::String("sh:ValidationReport".to_string()));
    ...
    // Results
    if !report.violations().is_empty() {
        let results: Vec<Value> = report.violations().iter().map(|violation| {
            let mut result = serde_json::Map::new();
            result.insert("@type".to_string(), Value::String("sh:ValidationResult".to_string()));
            ...
            if let Some(component) = &violation.source_constraint_component {
                result.insert("sh:sourceConstraintComponent".to_string(), 
                    Value::String(format!("sh:{}", component.as_str())));
            }
            
            if let Some(shape) = &violation.source_shape {
                result.insert("sh:sourceShape".to_string(), 
            ...
            result.insert("sh:resultSeverity".to_string(), Value::String(severity.to_string()));
            
            if let Some(message) = &violation.result_message {
                result.insert("sh:resultMessage".to_string(), Value::String(message.clone()));
            }
...
pub fn generate_rdfxml_report(report: &ValidationReport) -> Result<String> {
    ...
    writeln!(output, "    xmlns:rdf=\"http://www.w3.org/1999/02/22-rdf-syntax-ns#\"")?;
    writeln!(output, "    xmlns:sh=\"http://www.w3.org/ns/shacl#\"")?;
    writeln!(output, "    xmlns:xsd=\"http://www.w3.org/2001/XMLSchema#\">")?;
    
    // Validation report
    let report_id = format!("report_{}", generate_timestamp_id());
    writeln!(output, "  <sh:ValidationReport rdf:about=\"#{}\"_>", report_id)?;
    ...
    for (i, violation) in report.violations().iter().enumerate() {
        ...
        writeln!(output, "      <sh:ValidationResult rdf:about=\"#{}\">", result_id)?;
        
        if let Some(focus_node) = &violation.focus_node {
            writeln!(output, "        <sh:focusNode rdf:resource=\"{}\"/>", format_term_rdfxml(focus_node))?;
        }
        
        if let Some(path) = &violation.result_path {
            writeln!(output, "        <sh:resultPath rdf:resource=\"{}\"/>", format_path_rdfxml(path))?;
        }
        
        if let Some(value) = &violation.value {
            writeln!(output, "        <sh:value>{}</sh:value>", format_term_rdfxml_value(value))?;
        }
        ...
        if let Some(shape) = &violation.source_shape {
            writeln!(output, "        <sh:sourceShape rdf:resource=\"{}\"/>", shape.as_str())?;
        }
        ...
        writeln!(output, "        <sh:resultSeverity rdf:resource=\"{}\"/>", severity_iri)?;
        
        if let Some(message) = &violation.result_message {
            writeln!(output, "        <sh:resultMessage>{}</sh:resultMessage>", escape_xml_string(message))?;
        }
...
pub fn generate_ntriples_report(report: &ValidationReport) -> Result<String> {
    ...
    for (i, violation) in report.violations().iter().enumerate() {
        ...
        // Link to result
        writeln!(output, "{} <http://www.w3.org/ns/shacl#result> {} .", report_iri, result_iri)?;
        ...
        // Focus node
        if let Some(focus_node) = &violation.focus_node {
            writeln!(output, "{} <http://www.w3.org/ns/shacl#focusNode> {} .", result_iri, format_term_ntriples(focus_node))?;
        }
        
        // Result path
        if let Some(path) = &violation.result_path {
            writeln!(output, "{} <http://www.w3.org/ns/shacl#resultPath> {} .", result_iri, format_path_ntriples(path))?;
        }
        
        // Value
        if let Some(value) = &violation.value {
            writeln!(output, "{} <http://www.w3.org/ns/shacl#value> {} .", result_iri, format_term_ntriples(value))?;
        }
        ...
        // Source shape
        if let Some(shape) = &violation.source_shape {
            writeln!(output, "{} <http://www.w3.org/ns/shacl#sourceShape> <{}> .", result_iri, shape.as_str())?;
        }
        ...
        writeln!(output, "{} <http://www.w3.org/ns/shacl#resultSeverity> {} .", result_iri, severity_iri)?;
        
        // Result message
        if let Some(message) = &violation.result_message {
            writeln!(output, "{} <http://www.w3.org/ns/shacl#resultMessage> \"{}\" .", 
                result_iri, escape_ntriples_string(message))?;
        }
    }
...
pub fn generate_json_report(report: &ValidationReport) -> Result<String> {
    ...
    // Basic information
    json_report.insert("conforms".to_string(), Value::Bool(report.conforms()));
    json_report.insert("violationCount".to_string(), Value::Number(report.violations().len().into()));
    json_report.insert("timestamp".to_string(), Value::String(Utc::now().to_rfc3339()));
    
    // Violations
    let violations: Vec<Value> = report.violations().iter().map(|violation| {
        let mut v = serde_json::Map::new();
        
        if let Some(focus_node) = &violation.focus_node {
            v.insert("focusNode".to_string(), Value::String(format!("{:?}", focus_node)));
        }
        ...
        if let Some(component) = &violation.source_constraint_component {
            v.insert("sourceConstraintComponent".to_string(), Value::String(component.as_str().to_string()));
        }
        
        if let Some(shape) = &violation.source_shape {
            v.insert("sourceShape".to_string(), Value::String(shape.as_str().to_string()));
        }
        
        v.insert("severity".to_string(), Value::String(format!("{:?}", violation.result_severity)));
    ...
    for violation in report.violations() {
        *severity_counts.entry(&violation.result_severity).or_insert(0) += 1;
    }
    
    stats.insert("violations".to_string(), 
        Value::Number(severity_counts.get(&Severity::Violation).unwrap_or(&0).clone().into()));
    stats.insert("warnings".to_string(), 
        Value::Number(severity_counts.get(&Severity::Warning).unwrap_or(&0).clone().into()));
    stats.insert("info".to_string(), 
        Value::Number(severity_counts.get(&Severity::Info).unwrap_or(&0).clone().into()));
...
pub fn generate_html_report(report: &ValidationReport) -> Result<String> {
    ...
    writeln!(output, "                <p><strong>Generated:</strong> {}</p>", Utc::now().format("%Y-%m-%d %H:%M:%S UTC"))?;
    writeln!(output, "                <p><strong>Conforms:</strong> <span class=\"{}\">{}</span></p>", 
        if report.conforms() { "conforms-yes" } else { "conforms-no" },
        if report.conforms() { "✓ Yes" } else { "✗ No" })?;
    writeln!(output, "                <p><strong>Total Violations:</strong> {}</p>", report.violations().len())?;
    writeln!(output, "            </div>")?;
    ...
    if !report.violations().is_empty() {
        ...
        writeln!(output, "                <div class=\"stat-card violation\">")?;
        writeln!(output, "                    <h3>Violations</h3>")?;
        writeln!(output, "                    <span class=\"count\">{}</span>", 
        ...
        for violation in report.violations() {
            ...
            writeln!(output, "                    <tr class=\"{}\">", severity_class)?;
            writeln!(output, "                        <td><span class=\"severity-badge {}\">{:?}</span></td>", 
                severity_class, violation.result_severity)?;
            writeln!(output, "                        <td class=\"focus-node\">{}</td>", 
                violation.focus_node.as_ref().map(|n| format!("{:?}", n)).unwrap_or_else(|| "-".to_string()))?;
            writeln!(output, "                        <td class=\"path\">{}</td>", 
                violation.result_path.as_ref().map(|p| format!("{:?}", p)).unwrap_or_else(|| "-".to_string()))?;
            writeln!(output, "                        <td class=\"value\">{}</td>", 
                violation.value.as_ref().map(|v| format!("{:?}", v)).unwrap_or_else(|| "-".to_string()))?;
            writeln!(output, "                        <td class=\"message\">{}</td>", 
                violation.result_message.as_ref().unwrap_or(&"-".to_string()))?;
            writeln!(output, "                        <td class=\"shape\">{}</td>", 
                violation.source_shape.as_ref().map(|s| s.as_str()).unwrap_or("-"))?;
            writeln!(output, "                        <td class=\"constraint\">{}</td>", 
                violation.source_constraint_component.as_ref().map(|c| c.as_str()).unwrap_or("-"))?;
            writeln!(output, "                    </tr>")?;
        }
...
pub fn generate_csv_report(report: &ValidationReport) -> Result<String> {
    ...
    // CSV header
    writeln!(output, "Severity,Focus Node,Result Path,Value,Message,Source Shape,Source Constraint Component")?;
    
    // CSV rows
    for violation in report.violations() {
        let severity = format!("{:?}", violation.result_severity);
        let focus_node = violation.focus_node.as_ref().map(|n| format!("{:?}", n)).unwrap_or_else(|| "".to_string());
        let result_path = violation.result_path.as_ref().map(|p| format!("{:?}", p)).unwrap_or_else(|| "".to_string());
        let value = violation.value.as_ref().map(|v| format!("{:?}", v)).unwrap_or_else(|| "".to_string());
        let message = violation.result_message.as_ref().unwrap_or(&"".to_string()).clone();
        let source_shape = violation.source_shape.as_ref().map(|s| s.as_str()).unwrap_or("");
        let source_constraint = violation.source_constraint_component.as_ref().map(|c| c.as_str()).unwrap_or("");
...
pub fn generate_text_report(report: &ValidationReport) -> Result<String> {
    ...
    writeln!(output, "Conforms: {}", if report.conforms() { "YES" } else { "NO" })?;
    ...
    if !report.violations().is_empty() {
        ...
        writeln!(output, "  Violations: {}", severity_counts.get(&Severity::Violation).unwrap_or(&0))?;
        writeln!(output, "  Warnings:   {}", severity_counts.get(&Severity::Warning).unwrap_or(&0))?;
        writeln!(output, "  Info:       {}", severity_counts.get(&Severity::Info).unwrap_or(&0))?;
        writeln!(output)?;
        ...
        for (i, violation) in report.violations().iter().enumerate() {
            writeln!(output, "{}. [{:?}] {}", 
                ...
                violation.result_message.as_ref().unwrap_or(&"No message".to_string()))?;
...
pub fn generate_yaml_report(report: &ValidationReport) -> Result<String> {
    ...
    writeln!(output, "  conforms: {}", if report.conforms() { "true" } else { "false" })?;
    ...
    if !report.violations().is_empty() {
        ...
        for violation in report.violations() {
            ...
            if let Some(focus_node) = &violation.focus_node {
                writeln!(output, "      focusNode: \"{}\"", format!("{:?}", focus_node).replace("\"", "\\\""))?;
            }
            
            if let Some(path) = &violation.result_path {
                writeln!(output, "      resultPath: \"{}\"", format!("{:?}", path).replace("\"", "\\\""))?;
            }
            
            if let Some(value) = &violation.value {
                writeln!(output, "      value: \"{}\"", format!("{:?}", value).replace("\"", "\\\""))?;
            }
            
            if let Some(message) = &violation.result_message {
                writeln!(output, "      message: \"{}\"", message.replace("\"", "\\\""))?;
            }
            ...
            if let Some(component) = &violation.source_constraint_component {
                writeln!(output, "      sourceConstraintComponent: \"{}\"", component.as_str())?;
            }
        }
    } else {
        writeln!(output, "  violations: []")?;
