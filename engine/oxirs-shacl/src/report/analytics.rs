...
use crate::{
    ...
    ConstraintComponentId, PropertyPath, Result, ShaclError, Severity, ShapeId,
};
...
impl ValidationReportAnalytics {
    ...
    pub fn generate_analytics(&mut self) -> Result<AnalyticsResult> {
        ...
        if self.config.enable_caching {
            if let Some(cached) = self.analytics_cache.get(&cache_key) {
                if cached.cached_at.elapsed().unwrap_or(Duration::MAX).as_secs() < self.config.cache_ttl_seconds {
        ...
        if self.config.enable_caching {
            self.analytics_cache.insert(cache_key, CachedAnalytics {
                result: result.clone(),
    ...
    {
        self.report_history.iter().filter(|report| predicate(report)).collect()
    }
    ...
    pub fn get_reports_in_range(
        ...
        self.filter_reports(|report| {
            report.analyzed_at >= start && report.analyzed_at <= end
        })
    }
    ...
    pub fn get_reports_above_threshold(&self, threshold: usize) -> Vec<&AnalyzedReport> {
        self.filter_reports(|report| {
            report.metrics.total_violations > threshold
        })
    }
    ...
    pub fn get_shape_statistics(&self, shape_id: &ShapeId) -> Result<ShapeStatistics> {
        ...
        for report in &self.report_history {
            let shape_violations = report.metrics.violations_by_shape
                .get(shape_id)
                .copied()
                .unwrap_or(0);
    ...
    pub fn export_analytics(&self, format: ReportFormat) -> Result<String> {
        ...
        match format {
            ...
            _ => Err(ShaclError::UnsupportedOperation(
                format!("Export format {:?} not supported for analytics", format)
            )),
        }
    }
    ...
    fn compute_metrics(&self, report: &ValidationReport) -> Result<ReportMetrics> {
        ...
        for violation in &report.violations {
            // Count by severity
            *violations_by_severity.entry(violation.result_severity.clone()).or_insert(0) += 1;
            
            // Count by shape
            *violations_by_shape.entry(violation.source_shape.clone()).or_insert(0) += 1;
            
            // Count by constraint component
            *violations_by_component.entry(violation.source_constraint_component.clone()).or_insert(0) += 1;
        ...
        let quality_score = self.calculate_quality_score(total_violations, unique_focus_nodes_count);
        
        // Calculate complexity score based on violation patterns
        let complexity_score = self.calculate_complexity_score(&violations_by_component, &violations_by_shape);
    ...
    fn detect_patterns(&self, report: &ValidationReport, metrics: &ReportMetrics) -> Result<Vec<ViolationPattern>> {
        ...
        let missing_prop_violations = report.violations.iter()
        ...
        if missing_prop_violations > 0 {
            patterns.push(ViolationPattern {
                ...
                description: format!("{} missing required property violations", missing_prop_violations),
        ...
        let datatype_violations = report.violations.iter()
    ...
    fn compute_performance_metrics(&self, report: &ValidationReport, duration: Duration) -> Result<PerformanceMetrics> {
    ...
    fn calculate_complexity_score(&self, violations_by_component: &HashMap<ConstraintComponentId, usize>, violations_by_shape: &HashMap<ShapeId, usize>) -> f64 {
    ...
    fn compute_summary(&self) -> Result<AnalyticsSummary> {
        ...
        let total_violations: usize = self.report_history.iter()
        ...
        let quality_scores: Vec<f64> = self.report_history.iter()
    ...
    fn compute_trends(&self) -> Result<TrendAnalysis> {
        let quality_trend: Vec<_> = self.report_history.iter()
            .map(|r| (r.analyzed_at, r.metrics.quality_score))
            .collect();
        
        let violation_trend: Vec<_> = self.report_history.iter()
            .map(|r| (r.analyzed_at, r.metrics.total_violations))
            .collect();
        
        let performance_trend: Vec<_> = self.report_history.iter()
            .filter_map(|r| {
                r.performance.validation_duration.map(|d| (r.analyzed_at, d.as_secs_f64()))
            })
            .collect();
    ...
    fn assess_quality(&self) -> Result<QualityAssessment> {
        ...
        let avg_quality = self.report_history.iter()
            .map(|r| r.metrics.quality_score)
            .sum::<f64>() / self.report_history.len() as f64;
    ...
    fn identify_quality_issues(&self) -> Result<Vec<QualityIssue>> {
        ...
        if missing_data_count > 0 {
            issues.push(QualityIssue {
                ...
                severity: if missing_data_count > 100 { Severity::Violation } else { Severity::Warning },
        ...
        if invalid_values_count > 0 {
            issues.push(QualityIssue {
                ...
                severity: if invalid_values_count > 50 { Severity::Violation } else { Severity::Warning },
    ...
    fn generate_recommendations(&self, summary: &AnalyticsSummary, quality: &QualityAssessment) -> Result<Vec<Recommendation>> {
        ...
        if matches!(quality.quality_rating, QualityRating::Poor | QualityRating::Critical) {
            recommendations.push(Recommendation {
    ...
    fn determine_quality_trend(&self) -> QualityTrend {
        ...
        let recent_quality: f64 = self.report_history.iter()
            .rev()
            .take(3)
            .map(|r| r.metrics.quality_score)
            .sum::<f64>() / 3.0;
        
        let older_quality: f64 = self.report_history.iter()
            .take(3)
            .map(|r| r.metrics.quality_score)
            .sum::<f64>() / 3.0;
    ...
    fn export_to_csv(&self, analytics: &AnalyticsResult) -> Result<String> {
        ...
        csv.push_str(&format!("Total Reports,{}\n", analytics.summary.total_reports));
        csv.push_str(&format!("Average Violations Per Report,{:.2}\n", analytics.summary.avg_violations_per_report));
        csv.push_str(&format!("Overall Quality Score,{:.2}\n", analytics.summary.overall_quality_score));
    ...
    fn export_to_html(&self, analytics: &AnalyticsResult) -> Result<String> {
        ...
        html.push_str(&format!("<tr><td>Total Reports</td><td>{}</td></tr>", analytics.summary.total_reports));
        html.push_str(&format!("<tr><td>Average Violations Per Report</td><td>{:.2}</td></tr>", analytics.summary.avg_violations_per_report));
        html.push_str(&format!("<tr><td>Overall Quality Score</td><td>{:.2}</td></tr>", analytics.summary.overall_quality_score));
        html.push_str(&format!("<tr><td>Quality Trend</td><td>{:?}</td></tr>", analytics.summary.quality_trend));
        html.push_str("</table>");
        
        // Quality assessment
        html.push_str("<h2>Quality Assessment</h2>");
        html.push_str("<table>");
        html.push_str(&format!("<tr><td>Quality Rating</td><td>{:?}</td></tr>", analytics.quality_assessment.quality_rating));
        html.push_str(&format!("<tr><td>Risk Level</td><td>{:?}</td></tr>", analytics.quality_assessment.risk_level));
        html.push_str("</table>");
        ...
        if !analytics.recommendations.is_empty() {
            ...
            for rec in &analytics.recommendations {
                html.push_str(&format!("<li><strong>{}</strong>: {} (Priority: {:?})</li>", 
    ...
    fn export_to_text(&self, analytics: &AnalyticsResult) -> Result<String> {
        ...
        text.push_str(&format!("  Total Reports: {}\n", analytics.summary.total_reports));
        text.push_str(&format!("  Average Violations Per Report: {:.2}\n", analytics.summary.avg_violations_per_report));
        text.push_str(&format!("  Overall Quality Score: {:.2}\n", analytics.summary.overall_quality_score));
        text.push_str(&format!("  Quality Trend: {:?}\n\n", analytics.summary.quality_trend));
