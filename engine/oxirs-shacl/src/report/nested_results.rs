...
impl NestedValidationViolation {
    ...
    pub fn analyze_root_causes(&self) -> Vec<RootCause> {
        ...
        if self.nested_results.as_ref().map_or(true, |nr| nr.child_violations.is_empty()) {
            root_causes.push(RootCause {
                description: format!("Root constraint violation: {}", self.violation.source_constraint_component),
    ...
    fn suggest_remediation(&self) -> String {
        match self.violation.source_constraint_component.as_str() {
            "MinCountConstraintComponent" => "Add more values to satisfy minimum cardinality".to_string(),
            "MaxCountConstraintComponent" => "Remove excess values to satisfy maximum cardinality".to_string(),
            "DatatypeConstraintComponent" => "Ensure values match the required datatype".to_string(),
            "ClassConstraintComponent" => "Ensure resources are instances of the required class".to_string(),
            "NodeKindConstraintComponent" => "Ensure values are of the correct node kind (IRI, Literal, etc.)".to_string(),
...
mod tests {
    use super::*;
    use crate::{ShapeId, ConstraintComponentId, Severity};
    ...
    fn test_nested_violation_builder() {
        ...
        let nested_violation = violation.to_nested()
    ...
    fn test_nested_violation_hierarchy() {
        ...
        let nested_child = child_violation.to_nested()
            .with_hierarchy_level(1)
            .build();
        ...
        let nested_root = root_violation.to_nested()
