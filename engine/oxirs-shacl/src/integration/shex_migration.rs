...
use crate::{
    Constraint, ConstraintComponentId, NodeKindConstraint, PropertyConstraint, PropertyPath, Result,
    ShaclError, Shape, ShapeId, ShapeType, Target,
};
...
pub enum ValueSetValue {
    ...
    IriStem { stem: String },
    /// IRI stem range
    IriStemRange { stem: String, exclusions: Vec<String> },
    /// Language stem
...
impl ShexMigrationEngine {
    ...
    pub fn migrate(&mut self, schema: &ShexSchema) -> Result<MigrationResult> {
        ...
        for shex_shape in &schema.shapes {
            match self.convert_shape(shex_shape, &mut report) {
                Ok(shacl_shape) => {
    ...
    fn parse_start_line(&self, line: &str) -> Option<String> {
        ...
        if let Some(pos) = line.find('=') {
            ...
            Some(shape_ref.trim_start_matches('@').trim_matches(&['<', '>'][..]).to_string())
        } else {
            None
        }
    }
    ...
    fn parse_shape_text(&self, text: &str) -> Result<ShexShape> {
        ...
        let id = if let Some(id_end) = text.find('{') {
            ...
            return Err(ShaclError::ParseError("Invalid shape syntax".to_string()));
        };
    ...
    fn extract_annotations(&self, text: &str) -> Vec<ShexAnnotation> {
        ...
        for line in text.lines() {
            ...
            if trimmed.starts_with("//") {
                let parts: Vec<&str> = trimmed[2..].trim().splitn(2, ' ').collect();
                if parts.len() == 2 {
    ...
    fn parse_triple_constraint(&self, text: &str) -> Result<ShexExpression> {
        ...
        if trimmed.starts_with('@') {
            let shape_ref = trimmed.trim_start_matches('@')
                .trim_matches(&['<', '>'][..])
                .to_string();
    ...
    fn parse_value_and_cardinality(&self, text: &str) -> Result<(Option<Box<ShexExpression>>, usize, i64)> {
        let trimmed = text.trim();
    ...
    fn extract_cardinality(&self, text: &str) -> (&str, usize, i64) {
        let trimmed = text.trim();

        // Check for cardinality suffixes
        if trimmed.ends_with('*') {
            (&trimmed[..trimmed.len()-1].trim(), 0, -1)
        } else if trimmed.ends_with('+') {
            (&trimmed[..trimmed.len()-1].trim(), 1, -1)
        } else if trimmed.ends_with('?') {
            (&trimmed[..trimmed.len()-1].trim(), 0, 1)
    ...
    fn convert_shape(&self, shex_shape: &ShexShape, report: &mut MigrationStatistics) -> Result<Shape> {
        let shape_id = ShapeId(shex_shape.id.clone());

        // Create target definition - if not specified, no automatic target
        let target = Target::Class(
            NamedNode::new(&shex_shape.id).unwrap_or_else(|_| {
        ...
        shape.description = if description.is_empty() { None } else { Some(description) };
    ...
    fn convert_expression(
        ...
        match expr {
            ...
            ShexExpression::ShapeRef(shape_ref) => {
                ...
                constraints.push(Constraint::Node(ShapeId(shape_ref.clone())));
            }
    ...
    fn convert_triple_constraint(
        ...
        let path = if tc.inverse {
            PropertyPath::Inverse(Box::new(PropertyPath::Predicate(
                NamedNode::new(&tc.predicate).unwrap_or_else(|_| {
        ...
        let mut prop_constraint = PropertyConstraint::new(path);
        ...
        if tc.min > 0 {
            prop_constraint = prop_constraint.with_min_count(tc.min);
        }
        if tc.max >= 0 {
            prop_constraint = prop_constraint.with_max_count(tc.max as usize);
        }

        // Handle value expression
        if let Some(value_expr) = &tc.value_expr {
            self.apply_value_constraint(&mut prop_constraint, value_expr, stats)?;
        }

        constraints.push(Constraint::Property(prop_constraint));
    ...
    fn apply_value_constraint(
        &self,
        prop_constraint: &mut PropertyConstraint,
        expr: &ShexExpression,
        stats: &mut MigrationStatistics,
    ) -> Result<()> {
        match expr {
            ShexExpression::ShapeRef(shape_ref) => {
                // Add sh:node
                prop_constraint.node = Some(ShapeId(shape_ref.clone()));
            }
            ShexExpression::NodeConstraint(nc) => {
                // Apply node constraint facets
                if let Some(node_kind) = &nc.node_kind {
                    let shacl_kind = match node_kind.as_str() {
                        "IRI" => NodeKindConstraint::Iri,
                        "BNode" => NodeKindConstraint::BlankNode,
                        "Literal" => NodeKindConstraint::Literal,
                        "NonLiteral" => NodeKindConstraint::BlankNodeOrIri,
                        _ => NodeKindConstraint::Iri,
                    };
                    prop_constraint.node_kind = Some(shacl_kind);
                }

                if let Some(datatype) = &nc.datatype {
                    prop_constraint.datatype = Some(
                        NamedNode::new(datatype).unwrap_or_else(|_| {
                ...
                // Apply string facets
                if let Some(min_len) = nc.string_facets.min_length {
                    prop_constraint.min_length = Some(min_len);
                }
                if let Some(max_len) = nc.string_facets.max_length {
                    prop_constraint.max_length = Some(max_len);
                }
                if let Some(pattern) = &nc.string_facets.pattern {
                    prop_constraint.pattern = Some(pattern.clone());
                }

                // Apply numeric facets
                if let Some(min) = nc.numeric_facets.min_inclusive {
                    prop_constraint.min_inclusive = Some(min);
                }
                if let Some(max) = nc.numeric_facets.max_inclusive {
                    prop_constraint.max_inclusive = Some(max);
                }
                if let Some(min) = nc.numeric_facets.min_exclusive {
                    prop_constraint.min_exclusive = Some(min);
                }
                if let Some(max) = nc.numeric_facets.max_exclusive {
                    prop_constraint.max_exclusive = Some(max);
                }
                ...
                if !nc.values.is_empty() {
                    ...
                    prop_constraint.in_values = Some(in_values);
                }
            }
            _ => {
                // For complex expressions, we'd need to create nested shapes
            }
        }
        Ok(())
    }
    ...
    fn convert_value_set(&self, values: &[ValueSetValue]) -> Vec<Term> {
        values.iter().filter_map(|v| {
            match v {
                ...
                ValueSetValue::Literal { value, datatype: _, language: _ } => {
    ...
    pub fn generate_report(&self, result: &MigrationResult) -> String {
        ...
        report.push_str(&format!("- Shapes converted: {}\n", result.report.shapes_converted));
        report.push_str(&format!("- Constraints converted: {}\n", result.report.constraints_converted));
...
"#;
        ...
        assert_eq!(schema.prefixes.get("ex"), Some(&"http://example.org/".to_string()));
    }
    ...
    fn test_value_set_parsing() {
        let engine = ShexMigrationEngine::new();
        let values = engine.parse_value_set("[<http://example.org/a> <http://example.org/b>]").unwrap();
