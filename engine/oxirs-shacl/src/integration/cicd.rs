...
impl CiCdEngine {
    ...
    fn evaluate_thresholds(&self, report: &ValidationReport) -> ThresholdResults {
        ...
        let total_count = report.validation_stats().targets_validated;
    ...
    fn generate_summary(
        ...
        lines.push(format!(
            "Targets validated: {}",
            report.validation_stats().targets_validated
        ));
        lines.push(format!(
            "Violations found: {}",
    ...
    fn generate_junit(&self, result: &CiCdResult) -> String {
        ...
        xml.push_str(&format!(
            ...
            result.report.validation_stats().targets_validated,
            errors,
            failures,
    ...
    fn generate_tap(&self, result: &CiCdResult) -> String {
        ...
        let total = result.report.validation_stats().targets_validated.max(1);
    ...
    fn generate_sarif(&self, result: &CiCdResult) -> String {
        let sarif = serde_json::json!({
            ...
            "runs": [{
                ...
                "results": result.report.violations().iter().enumerate().map(|(i, v)| {
                    serde_json::json!({
                        "ruleId": v.source_constraint.as_deref().unwrap_or("unknown"),
    ...
    fn generate_github_output(&self, result: &CiCdResult) -> String {
        ...
        for violation in result.report.violations() {
            ...
            output.push_str(&format!(
                "::{} title={}::{}\n",
                level, violation.source_shape, violation.message
            ));
        }
        ...
        if self.config.github_summary {
            ...
            output.push_str(&result.summary.replace('\n', "\n"));
        }
