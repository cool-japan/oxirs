...
use crate::{
    ConstraintComponentId, PropertyPath, Result, ShaclError, ShapeId,
};
...
impl StringInterner {
    ...
    fn update_avg_length(&mut self, new_length: usize) {
        let total_length = self.stats.avg_string_length * (self.stats.total_strings - 1) as f64 + new_length as f64;
...
impl MemoryPool {
    ...
    pub fn deallocate(&mut self, ptr: *mut u8) {
        ...
        for pool in [&mut self.small_pool, &mut self.medium_pool, &mut self.large_pool] {
            for obj in pool.iter_mut() {
    ...
    pub fn compact(&mut self, max_age: Duration) {
        ...
        if removed > 0 {
            self.stats.total_memory_managed = self.small_pool.iter()
...
impl MemoryMonitor {
    ...
    pub fn record_sample(&mut self, memory_usage: usize, max_memory: usize) {
        ...
        self.stats.avg_memory_usage = (self.stats.avg_memory_usage * (self.stats.total_samples - 1) as f64 + memory_usage as f64) / self.stats.total_samples as f64;
        
        if memory_usage > self.stats.peak_memory_usage {
            self.stats.peak_memory_usage = memory_usage;
        }
        
        if matches!(self.pressure_level, MemoryPressureLevel::Warning) && !matches!(old_level, MemoryPressureLevel::Warning) {
            self.stats.pressure_warnings += 1;
        }
        
        if matches!(self.pressure_level, MemoryPressureLevel::Critical | MemoryPressureLevel::Emergency) && !matches!(old_level, MemoryPressureLevel::Critical | MemoryPressureLevel::Emergency) {
            self.stats.critical_pressure_events += 1;
        }
        
        // Update high pressure duration
        if matches!(self.pressure_level, MemoryPressureLevel::Critical | MemoryPressureLevel::Emergency) {
    ...
    pub fn memory_trend(&self) -> MemoryTrend {
        ...
        let recent_avg = self.samples.iter()
            .rev()
            .take(5)
            .map(|s| s.memory_usage)
            .sum::<usize>() as f64 / 5.0;
        
        let older_avg = self.samples.iter()
            .rev()
            .skip(5)
            .take(5)
            .map(|s| s.memory_usage)
...
impl MemoryOptimizer {
    ...
    pub fn record_memory_usage(&mut self, memory_usage: usize) {
        if self.config.enable_monitoring {
            self.memory_monitor.record_sample(memory_usage, self.config.max_memory_threshold);
        }
    }
    ...
    pub fn optimize(&mut self) -> Result<OptimizationResult> {
        ...
        if self.config.enable_memory_pooling {
            ...
            result.pool_objects_freed = pool.stats().total_allocations - pool.stats().active_objects;
        }
...
pub mod compact {
    ...
    impl CompactConverter {
        ...
        pub fn compact_violation(&self, violation: &crate::validation::ValidationViolation) -> Result<CompactViolation> {
            ...
            let constraint_component = interner.intern(&violation.source_constraint_component.to_string());
            ...
            let message = violation.result_message.as_ref()
        ...
        pub fn expand_violation(&self, compact: &CompactViolation) -> Result<crate::validation::ValidationViolation> {
            let interner = self.interner.read().unwrap();
            
            let focus_node_str = interner.get(compact.focus_node)
                .ok_or_else(|| ShaclError::MemoryOptimization("Invalid focus node reference".to_string()))?;
            
            let shape_id_str = interner.get(compact.shape_id)
                .ok_or_else(|| ShaclError::MemoryOptimization("Invalid shape ID reference".to_string()))?;
            
            let constraint_component_str = interner.get(compact.constraint_component)
                .ok_or_else(|| ShaclError::MemoryOptimization("Invalid constraint component reference".to_string()))?;
...
mod tests {
    ...
    fn test_memory_optimizer_creation() {
        ...
        assert_eq!(optimizer.memory_pressure_level(), &MemoryPressureLevel::Normal);
