...
#[cfg(feature = "async")]
use oxirs_core::{
    model::{GraphName, NamedNode, Term, Triple, Quad},
    Store,
};
...
impl MultiGraphValidationEngine {
    ...
    pub async fn validate_multi_graph(
        ...
        let graph_results = self.validate_graphs_concurrent(&selected_graphs, stores).await?;
        
        // Evaluate cross-graph constraints if enabled
        let cross_graph_violations = if self.config.enable_cross_graph_constraints {
            self.evaluate_cross_graph_constraints(stores, &graph_results).await?
        } else {
            Vec::new()
        };
        ...
        Ok(MultiGraphValidationResult {
            graph_reports: graph_results.iter()
                .map(|(g, r)| (g.clone(), r.report.clone()))
                .collect(),
            aggregated_report,
            cross_graph_violations,
            graph_stats: graph_results.iter()
    ...
    async fn select_graphs_for_validation(
        ...
        match &self.config.graph_strategy {
            GraphSelectionStrategy::All => {
                Ok(stores.keys().cloned().collect())
            }
            GraphSelectionStrategy::Explicit(graphs) => {
                Ok(graphs.intersection(&stores.keys().cloned().collect()).cloned().collect())
            }
            ...
            if let Some(store) = stores.get(graph_name) {
                ...
                let task = tokio::spawn(async move {
                    ...
                    match result {
                        ...
                        Ok(Err(e)) => Err(anyhow::anyhow!("Validation error for graph {}: {}", graph_name, e)),
                        Err(_) => Err(anyhow::anyhow!("Validation timeout for graph {}", graph_name)),
                    }
                });
    ...
    async fn evaluate_cross_graph_constraints(
        ...
        tracing::info!("Evaluating cross-graph constraints across {} graphs", stores.len());
        
        // Identify shapes with cross-graph constraints
        let cross_graph_shapes: Vec<_> = shapes.values()
            .filter(|shape| self.has_cross_graph_constraints(shape))
            .collect();
            
        tracing::debug!("Found {} shapes with cross-graph constraints", cross_graph_shapes.len());
        
        for shape in cross_graph_shapes {
            let shape_violations = self.evaluate_shape_cross_graph_constraints(
                shape, stores, graph_results
            ).await?;
    ...
    fn has_cross_graph_constraints(&self, shape: &Shape) -> bool {
        ...
        shape.constraints.values().any(|constraint| {
            match constraint {
            ...
            for focus_node in focus_nodes {
                ...
                for (constraint_id, constraint) in &shape.constraints {
                    if let crate::constraints::Constraint::Sparql(sparql_constraint) = constraint {
                        if sparql_constraint.sparql.contains("GRAPH") || sparql_constraint.sparql.contains("SERVICE") {
                            let violation_result = self.evaluate_cross_graph_sparql_constraint(
                                &focus_node,
                                constraint_id,
                                sparql_constraint,
                                stores,
            ...
            for quad in store.iter() {
                ...
                for (other_graph_name, other_store) in stores {
                    if graph_name != other_graph_name {
                        ...
                        let has_different_value = other_store.iter()
    ...
    async fn evaluate_cross_graph_disjointness(
        ...
        for (graph_name, store) in stores {
            for quad in store.iter() {
                ...
                for (other_graph_name, other_store) in stores {
                    if graph_name != other_graph_name {
                        let exists_in_other = other_store.iter()
    ...
    fn build_federated_sparql_query(
        ...
        for (i, graph_name) in graph_names.iter().enumerate() {
            ...
            federated_query.push_str(&format!("{{ GRAPH <{}> {{ {} }} }}", graph_name, query_with_focus));
        }
    ...
    fn collect_graph_errors(
        ...
        for (graph_name, result) in graph_results {
            if !result.stats.validation_successful {
                let errors = result.report.violations()
                    .iter()
                    .map(|v| v.message.clone().unwrap_or_else(|| "Unknown validation error".to_string()))
                    .collect();
    ...
    fn calculate_average_cpu_usage(
        ...
        let total_validation_time: Duration = graph_results.values()
            .map(|r| r.stats.validation_duration)
            .sum();
        
        let total_constraints: usize = graph_results.values()
        ...
        if total_validation_time.as_millis() > 0 {
            ...
            let cpu_factor = (total_constraints as f64) / (total_validation_time.as_millis() as f64);
    ...
    fn calculate_network_bandwidth(
        ...
        let total_bytes_transferred: usize = graph_results.values()
            .filter_map(|r| r.stats.remote_stats.as_ref())
            .map(|s| s.bytes_transferred)
            .sum();
        
        let total_time: Duration = graph_results.values()
            .filter_map(|r| r.stats.remote_stats.as_ref())
            .map(|s| s.network_time)
            .sum();
        
        if total_time.as_secs() > 0 {
            (total_bytes_transferred as f64) / (total_time.as_secs() as f64) / 1_000_000.0 // Convert to Mbps
        } else {
            0.0
        }
    }
