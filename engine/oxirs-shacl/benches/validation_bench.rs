...
fn bench_constraint_validation(c: &mut Criterion) {
    ...
    let constraints = vec![
        ("class", Constraint::Class(ClassConstraint {
            class_iri: NamedNode::new("http://example.org/Person").unwrap()
        })),
        ("min_count", Constraint::MinCount(MinCountConstraint { min_count: 1 })),
        ("max_count", Constraint::MaxCount(MaxCountConstraint { max_count: 10 })),
        ("min_length", Constraint::MinLength(MinLengthConstraint { min_length: 1 })),
        ("max_length", Constraint::MaxLength(MaxLengthConstraint { max_length: 100 })),
        ("pattern", Constraint::Pattern(PatternConstraint {
            pattern: "^[A-Za-z ]+$".to_string(),
            flags: None,
            message: None,
        })),
        ("node_kind", Constraint::NodeKind(NodeKindConstraint {
            node_kind: NodeKind::Literal
        })),
    ];
    
    for (name, constraint) in constraints {
        group.bench_with_input(BenchmarkId::new("constraint", name), &constraint, |b, constraint| {
            b.iter(|| {
...
fn bench_property_path_evaluation(c: &mut Criterion) {
    ...
    let paths = vec![
        ("simple", PropertyPath::predicate(NamedNode::new("http://example.org/name").unwrap())),
        ("inverse", PropertyPath::inverse(Box::new(
            PropertyPath::predicate(NamedNode::new("http://example.org/knows").unwrap())
        ))),
        ("sequence", PropertyPath::sequence(vec![
        ...
        ("alternative", PropertyPath::alternative(vec![
            PropertyPath::predicate(NamedNode::new("http://example.org/name").unwrap()),
...
fn bench_validation_config_impact(c: &mut Criterion) {
    ...
    let configs = vec![
        ...
        ("fail_fast", ValidationConfig {
            fail_fast: true,
            ..ValidationConfig::default()
        }),
        ("max_violations_10", ValidationConfig {
            max_violations: 10,
            ..ValidationConfig::default()
        }),
        ("exclude_warnings", ValidationConfig {
            include_warnings: false,
            ..ValidationConfig::default()
        }),
        ("parallel", ValidationConfig {
            parallel: true,
            ..ValidationConfig::default()
        }),
    ];
