...
mod tests {
    ...
    async fn test_parse_from_string_basic() {
        ...
        assert!(result.is_ok(), "Failed to parse basic Turtle: {:?}", result.err());
    ...
    async fn test_parse_from_string_no_aspect() {
        ...
        if let Err(err) = result {
            ...
            assert!(err_str.contains("No Aspect found"), "Expected 'No Aspect found' error");
        }
    }
    ...
    async fn test_parse_aspect_with_multiple_properties() {
        ...
        let result = parse_aspect_from_string(ttl_content, "urn:samm:org.example:1.0.0#").await;
        assert!(result.is_ok(), "Failed to parse multi-property aspect: {:?}", result.err());
    ...
    async fn test_parse_aspect_with_operations() {
        ...
        let result = parse_aspect_from_string(ttl_content, "urn:samm:org.example:1.0.0#").await;
        assert!(result.is_ok(), "Failed to parse aspect with operations: {:?}", result.err());
    ...
    async fn test_model_resolver_creation() {
        ...
        assert!(resolver.get_cache_stats().total_requests == 0);
    }
