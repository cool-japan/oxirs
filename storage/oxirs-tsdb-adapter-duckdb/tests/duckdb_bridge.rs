...
//!
//! These tests live in the `oxirs-tsdb-adapter-duckdb` quarantine crate, which
//! always compiles against the embedded (bundled) DuckDB engine. Build with:
//!
//! ```bash
//! cargo nextest run -p oxirs-tsdb-adapter-duckdb
//! ```
//!
//! The C build of `libduckdb` is bundled and built from source, so a C
//! toolchain must be available on the host.

use chrono::{Duration, TimeZone, Utc};
use oxirs_tsdb::{DataPoint, TimeChunk};
use oxirs_tsdb_adapter_duckdb::duckdb_bridge::{SERIES_COL, TS_COL, VAL_COL};
use oxirs_tsdb_adapter_duckdb::{
    chunk_to_record_batch, duckdb_open_in_memory, duckdb_run_sql, points_to_record_batch,
    read_into_chunk, register_tsdb_chunk, DuckDbReadOptions, DuckDbRegisterOptions,
};
