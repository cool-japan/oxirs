...
impl RaftStateMachine {
    ...
    async fn run_follower(&self) {
        ...
        loop {
            ...
            if elapsed >= timeout_duration {
                info!("Node {} election timeout expired, becoming candidate", self.node_id);
    ...
    async fn run_candidate(&self) {
        ...
        loop {
            ...
            if start_time.elapsed() >= timeout_duration {
                info!("Node {} election timeout expired, restarting election", self.node_id);
                self.handle_event(RaftEvent::ElectionTimeout).await;
                break;
            }
    ...
    async fn run_leader(&self) {
        ...
        let mut heartbeat_interval = interval(Duration::from_millis(self.config.heartbeat_interval));
    ...
    async fn start_election(&self) -> Result<()> {
        ...
        for peer in peers {
            ...
            let delay_ms = thread_rng().gen_range(10..50);
            let vote_granted = thread_rng().gen_bool(0.5);
            ...
            match timeout(
                ...
                Ok(Some((peer_id, response))) => {
                    if response.term > current_term {
                        self.handle_event(RaftEvent::HigherTermDiscovered(response.term)).await;
        ...
        if votes_received >= votes_needed {
            info!("Node {} won election with {} votes", self.node_id, votes_received);
            self.handle_event(RaftEvent::ElectionWon).await;
            Ok(())
        } else {
            info!("Node {} lost election with {} votes", self.node_id, votes_received);
            self.handle_event(RaftEvent::ElectionLost).await;
            ...
            if let Some(entry) = log.get((new_commit_index - 1) as usize) {
                if entry.term == current_term {
                    *self.commit_index.write().await = new_commit_index;
                    debug!("Leader {} updated commit index to {}", self.node_id, new_commit_index);
                }
            }
        }
    }
    ...
    pub async fn handle_append_entries(&self, request: AppendEntriesRequest) -> AppendEntriesResponse {
        ...
        *self.last_heartbeat.lock().await = Instant::now();
        ...
        if *self.state.read().await == NodeState::Candidate {
            self.handle_event(RaftEvent::LeaderDiscovered(request.leader_id)).await;
        }
    ...
    fn random_election_timeout(&self) -> u64 {
        thread_rng().gen_range(self.config.election_timeout_min..=self.config.election_timeout_max)
    }
    ...
    async fn get_prev_log_index_for_peer(&self, peer: OxirsNodeId) -> LogIndex {
        self.next_index.read().await.get(&peer).cloned().unwrap_or(1).saturating_sub(1)
    }
...
mod tests {
    ...
    async fn test_node_state_transitions() {
        ...
        state_machine.handle_event(RaftEvent::HigherTermDiscovered(10)).await;
        assert_eq!(state_machine.get_state().await, NodeState::Follower);
        assert!(!state_machine.is_leader().await);
    }
    ...
    async fn test_term_updates() {
        ...
        state_machine.handle_event(RaftEvent::HigherTermDiscovered(5)).await;
