...
mod tests {
    ...
    fn test_discovery_error_display() {
        let err = DiscoveryError::DnsResolution { message: "failed".to_string() };
        assert!(err.to_string().contains("DNS resolution failed: failed"));
        
        let err = DiscoveryError::Network { message: "timeout".to_string() };
        assert!(err.to_string().contains("Network error: timeout"));
        
        let err = DiscoveryError::Configuration { message: "invalid".to_string() };
        assert!(err.to_string().contains("Configuration error: invalid"));
        
        let err = DiscoveryError::Timeout { message: "5s".to_string() };
        assert!(err.to_string().contains("Timeout: 5s"));
        
        let err = DiscoveryError::Serialization { message: "json".to_string() };
