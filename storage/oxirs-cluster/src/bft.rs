...
impl MessageLog {
    ...
    fn is_prepared(&self, view: u64, sequence: u64, required_votes: usize) -> bool {
        self.prepares.get(&(view, sequence))
    ...
    fn is_committed(&self, view: u64, sequence: u64, required_votes: usize) -> bool {
        self.commits.get(&(view, sequence))
            .map(|votes| votes.len() >= required_votes)
            .unwrap_or(false)
    }
}
...
impl ByzantineTracker {
    ...
    fn report_invalid(&mut self, node_id: &str) {
        let count = self.invalid_messages.entry(node_id.to_string()).or_insert(0);
        *count += 1;
...
impl BftConsensus {
    ...
    pub fn register_node(&self, node_id: String, public_key: PublicKey) -> Result<()> {
        let mut keys = self.node_keys.write()
    ...
    fn verify_signature(&self, node_id: &str, message: &[u8], signature: &[u8]) -> Result<bool> {
        ...
        let public_key = keys.get(node_id)
    ...
    pub fn handle_message(&self, message: BftMessage, from_node: &str) -> Result<()> {
        ...
        let tracker = self.byzantine_tracker.read()
            .map_err(|e| ClusterError::Lock(e.to_string()))?;
        if tracker.is_byzantine(from_node) {
            return Err(ClusterError::Network(format!("Byzantine node: {}", from_node)));
        }
        drop(tracker);
        
        match message {
            BftMessage::PrePrepare { view, sequence, digest, request, primary_signature } => {
                self.handle_pre_prepare(view, sequence, digest, *request, primary_signature, from_node)?;
            }
            BftMessage::Prepare { view, sequence, digest, node_id, signature } => {
                self.handle_prepare(view, sequence, digest, node_id, signature)?;
            }
            BftMessage::Commit { view, sequence, digest, node_id, signature } => {
                self.handle_commit(view, sequence, digest, node_id, signature)?;
            }
            BftMessage::ViewChange { new_view, node_id, prepared_messages, signature } => {
    ...
    fn handle_pre_prepare(
        ...
        // Store pre-prepare
        self.store_pre_prepare(view, sequence, BftMessage::PrePrepare {
            view,
            sequence,
            digest: digest.clone(),
            request: Box::new(request),
    ...
    fn handle_prepare(
        ...
        log.prepares
            ...
            .insert(node_id.clone(), BftMessage::Prepare {
                view,
                sequence,
                digest: digest.clone(),
                node_id,
    ...
    fn handle_commit(
        ...
        log.commits
            .entry((view, sequence))
            .or_insert_with(HashMap::new)
            .insert(node_id.clone(), BftMessage::Commit {
                view,
                sequence,
                digest: digest.clone(),
                node_id,
        ...
        if log.is_committed(view, sequence, self.config.required_votes()) {
            ...
            if let Some(pre_prepare) = log.pre_prepares.get(&(view, sequence)) {
                if let BftMessage::PrePrepare { request, .. } = pre_prepare {
                    if let BftMessage::Request { operation, client_id, timestamp, .. } = &**request {
                        // Mark as completed
    ...
    fn report_byzantine(&self, node_id: &str) -> Result<()> {
        let mut tracker = self.byzantine_tracker.write()
            .map_err(|e| ClusterError::Lock(e.to_string()))?;
    ...
    pub fn start_view_timer(&self) -> Result<()> {
        let mut timer = self.view_timer.write()
            .map_err(|e| ClusterError::Lock(e.to_string()))?;
