...
impl QueryRouter {
    ...
    pub async fn plan_query(
        ...
        let shard_ids = self.shard_router
        ...
        // Update statistics
        {
            ...
            stats.avg_shards_per_query = 
                (stats.avg_shards_per_query * (stats.total_queries - 1) as f64 
                + shard_ids.len() as f64) / stats.total_queries as f64;
        }
        ...
        for &shard_id in &shard_ids {
            if let Some(metadata) = self.shard_router.get_shard_metadata(shard_id).await {
                ...
                let target = ShardTarget {
                    ...
                    alternative_nodes: metadata.node_ids.iter()
    ...
    pub async fn plan_federated_query(
        ...
        let local_plan = self.plan_query(
            local_pattern.0,
            local_pattern.1,
            local_pattern.2,
            hints.clone(),
    ...
    pub async fn optimize_plan(&self, plan: &mut QueryPlan) -> Result<()> {
            ...
            if let Some(metadata) = self.shard_router.get_shard_metadata(target.shard_id).await {
                ...
                if metadata.state != crate::shard::ShardState::Active {
                    if let Some(alt) = target.alternative_nodes.first() {
                        ...
                        warn!("Shard {} primary offline, using alternative node {}", 
                            target.shard_id, alt);
                    }
                }
            }
        ...
        if plan.shard_targets.len() > 1 && plan.optimization_hints.parallel_execution {
            debug!("Enabling parallel execution for {} shards", plan.shard_targets.len());
        }
    ...
    pub async fn execute_plan(&self, plan: QueryPlan) -> Result<Vec<Triple>> {
        ...
        if plan.optimization_hints.parallel_execution && plan.shard_targets.len() > 1 {
            ...
            for handle in handles {
                let results = handle.await.map_err(|e| 
                    ClusterError::Runtime(format!("Query execution failed: {}", e)))?;
        ...
        let latency_ms = start_time.elapsed().as_millis() as f64;
        {
            ...
            stats.avg_latency_ms = (stats.avg_latency_ms * (stats.total_queries - 1) as f64 
                + latency_ms) / stats.total_queries as f64;
        }
    ...
    async fn cache_plan(&self, query_id: String, plan: QueryPlan) {
        ...
        if cache.entries.len() >= cache.max_size {
            // Simple LRU eviction
            if let Some((evict_id, _)) = cache.entries.iter()
        ...
        cache.entries.insert(query_id, CacheEntry {
            plan,
...
mod tests {
    ...
    async fn test_query_planning() {
        ...
        let plan = query_router.plan_query(
    ...
    fn test_selectivity_estimation() {
        ...
        assert_eq!(query_router.estimate_selectivity(Some("s"), Some("p"), Some("o")), 0.01);
        assert_eq!(query_router.estimate_selectivity(Some("s"), Some("p"), None), 0.1);
        assert_eq!(query_router.estimate_selectivity(Some("s"), None, None), 0.5);
