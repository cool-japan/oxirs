...
mod shard_integration_tests {
    ...
    use oxirs_cluster::shard_routing::{QueryRouter, QueryOptimizationHints};
    ...
    async fn test_end_to_end_sharding() {
        ...
        let manager = Arc::new(ShardManager::new(1, router.clone(), config, storage, network));
        ...
        // Query triples
        let results = manager.query_triples(None, Some("http://example.org/knows"), None).await.unwrap();
    ...
    async fn test_namespace_sharding() {
        ...
        let strategy = ShardingStrategy::Namespace { namespace_mapping: namespace_mapping.clone() };
    ...
    async fn test_query_routing() {
        ...
        let plan1 = query_router.plan_query(
        ...
        let plan2 = query_router.plan_query(
            None,
