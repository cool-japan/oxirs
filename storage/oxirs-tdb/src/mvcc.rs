    ...
    V: Clone + Default,
{
    ...
    pub fn begin_transaction(&self, read_only: bool) -> Result<TransactionId> {
        let mut tx_counter = self.transaction_counter.write()
        ...
        *tx_counter += 1;
        ...
        let mut active_txs = self.active_transactions.write()
    ...
    pub fn commit_transaction(&self, tx_id: TransactionId) -> Result<Version> {
        ...
        let transaction = active_txs.remove(&tx_id)
        ...
        *current_version += 1;
        ...
        debug!("Committed transaction {} at version {}", tx_id, commit_version);
        self.update_stats_on_commit();
    ...
    pub fn vacuum(&self) -> Result<usize> {
        ...
        data.retain(|_key, versions| {
            let has_live_data = versions.iter().any(|v| {
                !v.is_deleted && (v.version == 0 || v.version >= oldest_active_version)
            });
    ...
    pub fn get_active_transactions(&self) -> Result<Vec<(TransactionId, Duration)>> {
        let active_txs = self.active_transactions.read()
            .map_err(|_| anyhow!("Failed to acquire active transactions lock"))?;
        
        Ok(active_txs.values()
    ...
    fn get_oldest_active_version(&self) -> Result<Version> {
        ...
        let oldest = active_txs.values()
            .map(|tx| tx.start_version)
            .min()
            .unwrap_or_else(|| {
                *self.current_version.read().unwrap_or_else(|_| {
                    panic!("Failed to acquire version lock")
                })
            });
    ...
    fn update_storage_stats(&self, stats: &mut MvccStats, data: &HashMap<K, Vec<VersionedValue<V>>>) {
        stats.total_keys = data.len();
        stats.total_versions = data.values().map(|v| v.len()).sum();
        stats.deleted_versions = data.values()
