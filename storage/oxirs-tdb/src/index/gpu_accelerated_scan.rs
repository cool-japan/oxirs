...
impl GpuIndexScanner {
    ...
    fn bulk_scan_patterns_gpu(
        ...
        // For now, fallback to sequential GPU scans
        // TODO: Implement true bulk GPU kernel for multiple patterns
        patterns
            .iter()
            .map(|p| self.scan_pattern_gpu(triples, p))
            .collect()
    }
    ...
    fn count_pattern_gpu(&self, triples: &[Triple], pattern: &TriplePattern) -> Result<u64> {
        // For now, fallback to scan and count
        // TODO: Implement dedicated GPU count kernel
        Ok(self.scan_pattern_cpu(triples, pattern).len() as u64)
    }
