...
impl CommandBus {
    ...
    pub fn new(config: CQRSConfig, event_store: Arc<dyn EventStoreTrait>) -> Self {
        ...
        Self {
            command_semaphore: Arc::new(tokio::sync::Semaphore::new(config.max_concurrent_commands)),
            config,
            event_store,
        ...
        C: Command + 'static,
    {
        ...
        debug!("Executing command {} for aggregate {:?}", command_id, command.aggregate_id());
        ...
        loop {
            match self.execute_once(command.clone()).await {
                ...
                Err(e) if attempt >= self.config.command_retry_config.max_retries => {
                    error!("Command {} failed after {} attempts: {}", command.command_id(), attempt + 1, e);
                    return Err(e);
                }
                Err(e) => {
                    warn!("Command {} failed on attempt {}: {}", command.command_id(), attempt + 1, e);
                    attempt += 1;
                    
                    tokio::time::sleep(delay).await;
                    delay = Duration::from_millis(
                        (delay.as_millis() as f64 * self.config.command_retry_config.backoff_multiplier) as u64
                    ).min(Duration::from_millis(self.config.command_retry_config.max_delay_ms));
                }
            }
        }
    }
        ...
        C: Command + 'static,
    {
        ...
        // Execute handler
        let events = handler.handle(command.clone()).await
        ...
        let version = if events.is_empty() {
            ...
            self.event_store.append_events(&aggregate_id, &events, command.expected_version()).await?
        };
...
impl QueryBus {
    ...
    pub fn new(config: CQRSConfig) -> Self {
        Self {
            ...
            cache: Arc::new(RwLock::new(QueryCache::new(config.query_cache_config.clone()))),
            config,
            handlers: Arc::new(RwLock::new(HashMap::new())),
        ...
        Q::Result: Clone + Serialize + for<'de> Deserialize<'de>,
    {
        ...
        let timeout = Duration::from_millis(
            query.timeout_ms().unwrap_or(self.config.query_timeout_ms)
        );
        
        let result = tokio::time::timeout(timeout, handler.handle(query.clone())).await
...
impl ReadModelManager {
    ...
    async fn process_projection(&self, name: &str, projection: &dyn ReadModelProjection<Event = StreamEvent>) -> Result<()> {
        let position = {
            let positions = self.projection_positions.read().await;
            positions.get(name).copied().unwrap_or(0)
        };
        
        let events = self.event_stream.read_events_from_position(position, 1000).await?;
...
impl QueryCache {
    ...
    fn evict_if_needed(&mut self) {
        ...
        if total_size > max_size {
            let mut entries: Vec<_> = self.entries.iter().map(|(k, v)| (k.clone(), v.created_at, v.size_bytes)).collect();
...
impl CQRSSystem {
    ...
    pub async fn health_check(&self) -> CQRSHealthStatus {
        ...
        CQRSHealthStatus {
            command_bus_healthy: command_metrics.active_commands < self.config.max_concurrent_commands as u64,
            query_bus_healthy: query_metrics.active_queries < self.config.max_concurrent_queries as u64,
