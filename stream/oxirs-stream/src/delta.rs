...
use crate::{StreamEvent, RdfPatch, PatchOperation};
...
impl DeltaComputer {
    ...
    pub fn delta_to_patch(&self, events: &[StreamEvent]) -> Result<RdfPatch> {
        ...
        debug!("Converted {} events to RDF patch with {} operations", 
               events.len(), patch.operations.len());
        Ok(patch)
    }
    ...
    fn process_update_operation(&mut self, operation: &UpdateOperation) -> Result<Vec<StreamEvent>> {
        let mut events = Vec::new();
...
enum UpdateOperation {
    InsertData { triples: Vec<Triple> },
    DeleteData { triples: Vec<Triple> },
    InsertWhere { insert: Vec<Triple>, where_clause: String },
    DeleteWhere { patterns: Vec<Triple> },
    DeleteInsert { delete: Vec<Triple>, insert: Vec<Triple>, where_clause: Option<String> },
    ClearAll,
    ClearDefault,
    ClearGraph { graph: Option<String> },
    DropGraph { graph: Option<String> },
    CreateGraph { graph: Option<String> },
    Load { url: String, graph: Option<String> },
}
...
impl BatchDeltaProcessor {
    ...
    pub async fn process_updates(&mut self, updates: &[String]) -> Result<Vec<StreamEvent>> {
        ...
        for update in updates {
            ...
            let events = self.processors[processor_idx].process_update(update).await?;
            all_events.extend(events);
        }
