...
async fn test_service_creation() {
    ...
    assert!(sparql.capabilities.contains(&ServiceCapability::SparqlQuery));
    assert!(sparql.capabilities.contains(&ServiceCapability::SparqlUpdate));
    ...
    assert!(graphql.capabilities.contains(&ServiceCapability::GraphQLQuery));
    assert!(graphql.capabilities.contains(&ServiceCapability::GraphQLMutation));
    assert!(graphql.capabilities.contains(&ServiceCapability::GraphQLSubscription));
}
...
async fn test_capability_filtering() {
    ...
    sparql.capabilities.insert(ServiceCapability::FullTextSearch);
    ...
    let fulltext_services = registry.get_services_with_capability(&ServiceCapability::FullTextSearch);
...
async fn test_service_performance_settings() {
    ...
    assert_eq!(
        service.performance.rate_limit.as_ref().unwrap().requests_per_minute,
        1000
    );
}
...
async fn test_registry_statistics() {
    ...
    assert!(stats.capabilities_distribution.contains_key(&ServiceCapability::SparqlQuery));
    assert_eq!(
        *stats.capabilities_distribution.get(&ServiceCapability::SparqlQuery).unwrap(),
        3
    );
}
...
async fn test_extended_metadata_operations() {
    ...
    assert!(registry.enable_extended_metadata("metadata-test").await.is_ok());
    
    // Should not error if already enabled
    assert!(registry.enable_extended_metadata("metadata-test").await.is_ok());
    
    // Should error for non-existent service
    assert!(registry.enable_extended_metadata("non-existent").await.is_err());
