...
async fn test_query_type_detection() {
    ...
    let test_cases = vec![
        ...
        ("SELECT ?name WHERE { ?s foaf:name ?name }", QueryType::SparqlSelect),
        ("CONSTRUCT { ?s ?p ?o } WHERE { ?s ?p ?o }", QueryType::SparqlConstruct),
        ("ASK { ?s ?p ?o }", QueryType::SparqlAsk),
        ("DESCRIBE <http://example.org/resource>", QueryType::SparqlDescribe),
    ...
    for (query, expected_type) in test_cases {
        ...
        assert_eq!(result.query_type, expected_type, "Failed for query: {}", query);
    }
}
...
async fn test_service_clause_parsing() {
    ...
    assert_eq!(result.service_clauses[0].service_url, "http://people.example.org/sparql");
    assert!(!result.service_clauses[0].silent);
    
    assert_eq!(result.service_clauses[1].service_url, "http://products.example.org/sparql");
...
async fn test_complexity_calculation() {
    ...
    assert!(matches!(medium_result.complexity, QueryComplexity::Low | QueryComplexity::Medium));
    ...
    assert!(matches!(complex_result.complexity, QueryComplexity::Medium | QueryComplexity::High));
}
...
async fn test_execution_plan_optimization() {
    ...
    let parallel_count: usize = plan.steps
        .iter()
...
async fn test_service_selection_strategies() {
    ...
    geo_service.capabilities.insert(ServiceCapability::Geospatial);
    ...
    text_service.capabilities.insert(ServiceCapability::FullTextSearch);
    ...
    let query_info = QueryInfo {
        ...
        service_clauses: vec![],
        filters: vec![],
        variables: ["?location", "?lat"].iter().map(|s| s.to_string()).collect(),
        complexity: QueryComplexity::Low,
        estimated_cost: 10,
    };
    ...
    assert!(plan.steps.iter().any(|step| {
        step.service_id.as_ref().map(|id| id == "geo-service").unwrap_or(false)
    }));
}
...
async fn test_advanced_query_decomposition() {
    ...
    let plan = planner.plan_sparql_advanced(&query_info, &registry).await.unwrap();
