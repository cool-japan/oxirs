...
impl DistributedJoinOptimizer {
        ...
        for node in &graph.nodes {
            ...
            if connections.len() >= 3 {
                ...
                let star_pattern = StarJoinPattern {
                    ...
                    connected_nodes,
                    estimated_benefit: self
                        .calculate_star_join_benefit_from_edges(node, &connections)
                        .await?,
    ...
    async fn detect_chain_joins(&self, graph: &JoinGraph) -> Result<Vec<ChainJoinPattern>> {
        ...
        for start_node in &graph.nodes {
            ...
            let chain = self
                .find_longest_chain(graph, &start_node.id, &mut visited)
                .await?;
    ...
    async fn optimize_bushy_tree(
        ...
        Ok(JoinPlan {
            ...
            parallelization_opportunities: self
                .identify_parallelization_opportunities(&tree)
                .await?,
                    ...
                    if submask != mask && dp.contains_key(&submask) {
                        ...
                        if complement > 0 && dp.contains_key(&complement) {
                            ...
                            let join_cost = self
                                ...
                                .await?;
                            let total_cost =
                                left_tree.total_cost + right_tree.total_cost + join_cost;
    ...
    pub async fn collect_runtime_statistics(
        ...
        strategy_stats
            .cardinality_samples
            .push(execution_result.result_cardinality);
        ...
        strategy_stats
            ...
            .push(execution_result.memory_used);
    ...
    pub async fn recommend_execution_strategy(
        ...
        for (strategy, performance) in &stats.strategy_performance {
            let cardinality_match = self
                .calculate_cardinality_match_score(
                    estimated_cardinality,
                    &performance.cardinality_samples,
                )
                .await?;
        ...
        if best_score == 0.0 {
            best_strategy = self
                .heuristic_strategy_selection(patterns, estimated_cardinality)
                .await?;
        }
    ...
    async fn build_join_graph_from_patterns(
        &self,
        patterns: &[TriplePattern],
    ) -> Result<JoinGraph> {
        let mut graph = JoinGraph {
    ...
    async fn detect_special_patterns_compatibility(
        &self,
        graph: &JoinGraph,
    ) -> Result<SpecialJoinPatterns> {
    ...
    async fn calculate_graph_complexity(&self, graph: &JoinGraph) -> Result<f64> {
        ...
        let all_variables: HashSet<String> = graph
            .nodes
            .iter()
    ...
    async fn optimize_star_join(
        ...
        let expected_parallelism = self
            .calculate_parallelization_opportunities_for_operations(&operations)
            .await?;
        ...
        for edge in &graph.edges {
            ...
            if let (Some(from), Some(to)) = (from_node, to_node) {
                options.push(JoinOrderOption {
                    ...
                    estimated_cost: self
                        .estimate_join_cost(from.estimated_cardinality, to.estimated_cardinality)
                        .await?,
    ...
    async fn calculate_parallelization_opportunities_for_operations(
        ...
        let parallelizable_ops: Vec<_> = operations.iter().filter(|op| op.parallelizable).collect();

        if !parallelizable_ops.is_empty() {
            let op_ids: Vec<String> = parallelizable_ops.iter().map(|op| op.id.clone()).collect();
    ...
    async fn analyze_filter_pushdown(
        ...
        let filter_selectivity = self
            .estimate_filter_selectivity_from_expression(filter)
            .await?;
    ...
    async fn calculate_star_join_benefit(
        ...
        for node in connected_nodes {
            ...
            current_cardinality = self
                .estimate_join_result_cardinality(current_cardinality, node.estimated_cardinality)
                .await?;
        }
        ...
        let total_join_cost: f64 = connected_nodes
            ...
            .map(|node| {
                ...
                let join_cost = futures::executor::block_on(self.estimate_join_cost(
                    center_node.estimated_cardinality,
                    node.estimated_cardinality,
                ));
    ...
    fn extract_filter_variables_from_expression(
        &self,
        _filter: &PlanningFilterExpression,
    ) -> HashSet<String> {
    ...
    async fn estimate_filter_selectivity_from_expression(
        &self,
        _filter: &PlanningFilterExpression,
    ) -> Result<f64> {
    ...
    async fn calculate_star_join_benefit_from_edges(
        ...
        for _ in &connected_node_ids {
            // Sequential join cost
            let join_cost = self
                .estimate_join_cost(current_cardinality, estimated_connected_cardinality)
                .await?;
    ...
    async fn estimate_filter_selectivity(&self, filter: &str) -> Result<f64> {
        ...
        debug!(
            "Estimated filter selectivity for '{}': {:.2}",
            filter, selectivity
        );
        Ok(selectivity)
    }
    ...
    async fn tree_to_execution_operations(
        &self,
        tree: &BushyTreeNode,
    ) -> Result<Vec<JoinOperation>> {
        let mut operations = Vec::new();
        let mut counter = 0;
        self.tree_to_operations_recursive(tree, &mut operations, &mut counter)
            .await?;
        Ok(operations)
    }
            ...
            match &node.node_type {
                ...
                BushyNodeType::InnerJoin => {
                    if let (Some(left), Some(right)) = (&node.left_child, &node.right_child) {
                        let left_id = self
                            .tree_to_operations_recursive(left, operations, counter)
                            .await?;
                        let right_id = self
                            .tree_to_operations_recursive(right, operations, counter)
                            .await?;
    ...
    async fn identify_parallelization_opportunities(
        &self,
        tree: &BushyTreeNode,
    ) -> Result<Vec<ParallelizationOpportunity>> {
        let mut opportunities = Vec::new();
        self.collect_parallelization_opportunities(tree, &mut opportunities)
            .await?;
        Ok(opportunities)
    }
            ...
            match &node.node_type {
                ...
                BushyNodeType::InnerJoin => {
                    ...
                    if let Some(left) = &node.left_child {
                        self.collect_parallelization_opportunities(left, opportunities)
                            .await?;
                    }
                    if let Some(right) = &node.right_child {
                        self.collect_parallelization_opportunities(right, opportunities)
                            .await?;
                    }
    ...
    async fn estimate_bushy_join_cost(
        &self,
        left: &BushyTreeNode,
        right: &BushyTreeNode,
    ...
    async fn calculate_parallelization_factor(
        &self,
        left: &BushyTreeNode,
        right: &BushyTreeNode,
    ) -> Result<f64> {
    ...
    async fn heuristic_strategy_selection(
        &self,
        patterns: &[TriplePattern],
        cardinality: u64,
    ) -> Result<JoinExecutionStrategy> {
