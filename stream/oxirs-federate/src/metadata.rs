...
impl ExtendedServiceMetadata {
    ...
    fn recalculate_health_metrics(&mut self) {
        ...
        self.health_metrics.availability_percentage = 
            (successful as f64 / total as f64) * 100.0;
        
        let response_times: Vec<Duration> = recent
            .iter()
            .filter_map(|c| c.response_time)
            .collect();
        
        if !response_times.is_empty() {
            let total_millis: u64 = response_times
                .iter()
        ...
        self.health_metrics.health_score = 
            (self.health_metrics.availability_percentage * 0.5) +
            ((100.0 - self.health_metrics.error_rate.min(100.0)) * 0.3) +
            ((1000.0 / (self.health_metrics.avg_response_time.as_millis() as f64 + 1.0)).min(100.0) * 0.2);
