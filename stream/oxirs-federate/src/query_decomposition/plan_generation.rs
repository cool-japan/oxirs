...
impl QueryDecomposer {
        ...
        for (_, pattern) in &component.patterns {
            if let Some(ref predicate) = pattern.predicate {
                if !predicate.starts_with('?') {
                    let best_service =
                        self.find_best_service_for_predicate(predicate, services);
    ...
    pub fn distribute_star_join_optimized(
        ...
        debug!("Using star join optimization algorithm for {} patterns", component.patterns.len());
        ...
        let central_variable = join_analysis.central_variable
            .as_ref()
            .ok_or_else(|| anyhow!("Star join pattern detected but no central variable found"))?;
        
        debug!("Star join optimization with central variable: {}", central_variable);
        ...
        let center_service = self.find_best_service_for_center_patterns(&center_patterns, services)?;
        ...
        if !center_patterns.is_empty() {
            let cost = self.cost_estimator.estimate_pattern_cost(&center_service, &center_patterns);
            total_cost += cost;
    ...
    pub fn distribute_maximize_parallel_advanced(
        ...
        let parallel_plan = self.construct_bushy_tree_plan(
            &independent_groups,
            &component.patterns,
            services
        )?;
    ...
    pub fn pattern_binds_variable(&self, pattern: &crate::planner::TriplePattern, variable: &str) -> bool {
    ...
    pub fn optimize_satellite_distribution(
        ...
        if available_services.is_empty() {
            ...
            let patterns_per_service = (sorted_patterns.len() + available_services.len() - 1) / available_services.len();
    ...
    pub fn construct_bushy_tree_plan(
        ...
        for (group_idx, group) in independent_groups.iter().enumerate() {
            ...
            if !group_patterns.is_empty() {
                ...
                let cost = self.cost_estimator.estimate_pattern_cost(service, &group_patterns);
                total_cost += cost;
