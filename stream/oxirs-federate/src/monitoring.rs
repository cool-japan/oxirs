...
impl FederationMonitor {
    ...
    pub async fn record_query_execution(&self, query_type: &str, duration: Duration, success: bool) {
        ...
        let type_metrics = metrics.query_type_metrics.entry(query_type.to_string()).or_insert_with(QueryTypeMetrics::new);
        ...
        self.update_response_time_histogram(&mut metrics, duration).await;
        ...
        debug!("Recorded {} query execution: {}ms, success: {}", query_type, duration.as_millis(), success);
    }

    /// Record service interaction metrics
    pub async fn record_service_interaction(&self, service_id: &str, duration: Duration, success: bool, response_size: Option<usize>) {
        let mut metrics = self.metrics.write().await;

        let service_metrics = metrics.service_metrics.entry(service_id.to_string()).or_insert_with(ServiceMetrics::new);
        
        service_metrics.total_requests += 1;
        service_metrics.total_duration += duration;
        service_metrics.avg_duration = service_metrics.total_duration / service_metrics.total_requests as u32;
        ...
        if let Some(size) = response_size {
            ...
            service_metrics.avg_response_size = service_metrics.total_response_size / service_metrics.successful_requests.max(1) as u64;
        }

        // Update last seen timestamp
        service_metrics.last_seen = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();

        debug!("Recorded service interaction for {}: {}ms, success: {}", service_id, duration.as_millis(), success);
    }
    ...
    pub async fn record_cache_hit(&self, cache_type: &str, hit: bool) {
        ...
        let cache_metrics = metrics.cache_metrics.entry(cache_type.to_string()).or_insert_with(CacheMetrics::new);
        ...
        debug!("Cache {} for {}: hit rate {:.2}%", if hit { "hit" } else { "miss" }, cache_type, cache_metrics.hit_rate * 100.0);
    }
    ...
    pub async fn export_prometheus_metrics(&self) -> String {
        ...
        output.push_str("# HELP oxirs_federation_queries_total Total number of federated queries\n");
        output.push_str("# TYPE oxirs_federation_queries_total counter\n");
        output.push_str(&format!("oxirs_federation_queries_total {}\n", metrics.total_queries));
        ...
        output.push_str(&format!("oxirs_federation_success_rate {:.4}\n", success_rate));
        ...
        for (query_type, type_metrics) in &metrics.query_type_metrics {
            output.push_str(&format!("oxirs_federation_queries_by_type_total{{type=\"{}\"}} {}\n", query_type, type_metrics.total_count));
        }
        ...
        for (query_type, type_metrics) in &metrics.query_type_metrics {
            output.push_str(&format!("oxirs_federation_avg_response_time_seconds{{type=\"{}\"}} {:.6}\n", 
                query_type, type_metrics.avg_duration.as_secs_f64()));
        }
        ...
        for (service_id, service_metrics) in &metrics.service_metrics {
            output.push_str(&format!("oxirs_federation_service_requests_total{{service=\"{}\"}} {}\n", service_id, service_metrics.total_requests));
        }
        ...
        for (cache_type, cache_metrics) in &metrics.cache_metrics {
            output.push_str(&format!("oxirs_federation_cache_hit_rate{{cache=\"{}\"}} {:.4}\n", cache_type, cache_metrics.hit_rate));
        }
    ...
    /// Update response time histogram
    async fn update_response_time_histogram(&self, metrics: &mut FederationMetrics, duration: Duration) {
        ...
        *metrics.response_time_histogram.entry(bucket.to_string()).or_insert(0) += 1;
    }
    ...
    async fn calculate_health_indicators(&self, metrics: &FederationMetrics) -> HealthIndicators {
        ...
        let avg_response_time = if !metrics.query_type_metrics.is_empty() {
            let total_duration: Duration = metrics.query_type_metrics.values()
                ...
                .sum();
            let total_count: u64 = metrics.query_type_metrics.values()
                .map(|m| m.total_count)
                .sum();
        ...
        let recent_error_count = metrics.recent_queries.iter()
            .filter(|q| !q.success)
            .count();

        let cache_hit_rate = if !metrics.cache_metrics.is_empty() {
            let total_hit_rate: f64 = metrics.cache_metrics.values()
                .map(|c| c.hit_rate)
                .sum();
        ...
        for (service_id, service_metrics) in &metrics.service_metrics {
            ...
            let health = if service_error_rate > 0.2 {
                HealthStatus::Unhealthy
            } else if service_error_rate > 0.1 || service_metrics.avg_duration > Duration::from_secs(3) {
                HealthStatus::Degraded
            } else {
                HealthStatus::Healthy
            };
    ...
    async fn get_top_errors(&self, metrics: &FederationMetrics) -> Vec<ErrorSummary> {
        ...
        for event in &metrics.federation_events {
            if matches!(event.event_type, FederationEventType::Error | FederationEventType::ServiceFailure) {
                *error_counts.entry(event.details.clone()).or_insert(0) += 1;
            }
        }

        let mut errors: Vec<ErrorSummary> = error_counts.into_iter()
    ...
    async fn get_performance_summary(&self, metrics: &FederationMetrics) -> PerformanceSummary {
        ...
        let healthy_services = metrics.service_metrics.values()
            .filter(|s| {
...
mod tests {
    ...
    async fn test_query_recording() {
        ...
        monitor.record_query_execution("sparql", Duration::from_millis(100), true).await;
        monitor.record_query_execution("graphql", Duration::from_millis(200), false).await;
    ...
    async fn test_service_metrics() {
        ...
        monitor.record_service_interaction("service1", Duration::from_millis(150), true, Some(1024)).await;
        monitor.record_service_interaction("service1", Duration::from_millis(250), false, None).await;
    ...
    async fn test_event_recording() {
        ...
        monitor.record_federation_event(FederationEventType::ServiceRegistered, "New service added").await;
        monitor.record_federation_event(FederationEventType::Error, "Query failed").await;
        
        let stats = monitor.get_stats().await;
        assert_eq!(stats.recent_events_count, 2);
        assert_eq!(stats.event_type_counts[&FederationEventType::ServiceRegistered], 1);
    ...
    async fn test_health_metrics() {
        ...
        monitor.record_query_execution("sparql", Duration::from_millis(100), false).await;
