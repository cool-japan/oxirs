...
//!
//! Enhanced with scirs2-core optimizations for improved numerical computations,
//! random number generation, and vectorized operations.
...
// scirs2 integration for optimized numerical operations
...
impl NeuralNetworkModel {
    ...
    pub fn new(input_size: usize, hidden_size: usize, learning_rate: f64) -> Self {
        ...
        // Pre-allocate output weights for better memory efficiency
    ...
    fn backpropagate(&mut self, features: &[f64], output_error: f64, _target: f64) {
        ...
        let output_gradient = output_error.clamp(-1.0, 1.0); // Gradient clipping

        // Update output layer weights
        for (i, &hidden_val) in hidden_activations.iter().enumerate() {
            if i < self.weights_hidden_output.len() {
                let weight_update = self.learning_rate * output_gradient * hidden_val;
                let clipped_update = weight_update.clamp(-0.5, 0.5); // Clip weight updates
...
impl MLOptimizer {
    ...
    fn generate_heuristic_orders(&self, items: &[String]) -> Vec<Vec<String>> {
        ...
        vec![
            ...
            }, // Reversed order
            {
                ...
                // scirs2-optimized Fisher-Yates shuffle for proper randomization
                let mut rng = Random::default();
                Self::shuffle_optimized(&mut random, &mut rng);
                random
            }, // Properly randomized order using scirs2
        ]
    }
    ...
    /// scirs2-optimized Fisher-Yates shuffle for proper randomization
    /// This provides much better randomization than the simple swap previously used
