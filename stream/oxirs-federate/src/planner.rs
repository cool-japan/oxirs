...
impl QueryPlanner {
    ...
    pub fn analyze_join_graph(
        ...
        analysis.star_joins = self.detect_star_joins(patterns, &analysis.join_variables)?;
    ...
    /// Detect star join patterns (one central pattern joined to multiple others)
    fn detect_star_joins(
        &self,
        patterns: &[TriplePattern],
        ...
        // For each pattern, check if it's the center of a star
        for (center_idx, center_pattern) in patterns.iter().enumerate() {
            if used_patterns.contains(&center_idx) {
                continue;
            }

            let center_vars = self.extract_pattern_variables(&[center_pattern.clone()]);
            let mut connected_patterns = Vec::new();

            // Find patterns that share exactly one variable with center
            for (other_idx, other_pattern) in patterns.iter().enumerate() {
                if other_idx == center_idx || used_patterns.contains(&other_idx) {
                    continue;
                }

                let other_vars = self.extract_pattern_variables(&[other_pattern.clone()]);
                let shared_vars: Vec<_> = center_vars.intersection(&other_vars).collect();

                // Star join: exactly one shared variable
                if shared_vars.len() == 1 {
                    connected_patterns.push(JoinPattern {
                        pattern_index: other_idx,
                        join_variable: shared_vars[0].clone(),
            ...
            // If we have 2+ connected patterns, it's a star join
            if connected_patterns.len() >= 2 {
                star_joins.push(StarJoin {
                    center_pattern_index: center_idx,
                    connected_patterns: connected_patterns.clone(),
                    estimated_cost: self.estimate_star_join_cost(
                        center_pattern,
                        &connected_patterns,
                        patterns,
                    ),
                ...
                used_patterns.insert(center_idx);
                for jp in &connected_patterns {
                    used_patterns.insert(jp.pattern_index);
                }
            }
        }
    ...
    fn estimate_pattern_selectivity(&self, pattern: &TriplePattern) -> f64 {
        ...
        if !pattern.subject.starts_with('?') {
            selectivity *= 0.001; // Specific subject is very selective
        }
        if !pattern.predicate.starts_with('?') {
            selectivity *= 0.1; // Specific predicate is moderately selective
        }
        if !pattern.object.starts_with('?') {
            selectivity *= 0.01; // Specific object is quite selective
        }

        selectivity.max(0.001).min(0.9)
    }
    ...
    /// Calculate pattern complexity for join optimization
    fn calculate_pattern_complexity(&self, pattern: &TriplePattern) -> PatternComplexity {
        ...
        let has_complex_features = pattern.pattern_string.contains("REGEX")
            || pattern.pattern_string.contains("PropertyPath")
            || pattern.pattern_string.contains("FILTER");

        if var_count == 0 || (var_count == 1 && !has_complex_features) {
            PatternComplexity::Simple
        } else if var_count <= 2 && !has_complex_features {
            PatternComplexity::Medium
        } else {
            PatternComplexity::Complex
        }
...
impl QueryPlanner {
    ...
    fn create_subgraph(
        ...
        let internal_joins = Vec::new(); // TODO: Extract from join_graph
    ...
    pub async fn optimize_with_feedback(
        ...
        let historical_analysis = self.analyze_execution_history(execution_history).await?;

        // Extract performance insights
        let performance_insights = self.extract_performance_insights(&historical_analysis)?;

        // Adjust plan based on feedback
        let mut optimized_plan = base_plan.clone();

        // Apply join order adjustments
        if let Some(join_order_feedback) = performance_insights.join_order_feedback {
            self.apply_join_order_feedback(
                &mut optimized_plan,
                &join_order_feedback,
                feedback_weight,
            )
        ...
        // Apply algorithm selection feedback
        if let Some(algorithm_feedback) = performance_insights.algorithm_feedback {
            self.apply_algorithm_feedback(
                &mut optimized_plan,
                &algorithm_feedback,
                feedback_weight,
            )
        ...
        if let Some(resource_feedback) = performance_insights.resource_feedback {
            self.apply_resource_feedback(&mut optimized_plan, &resource_feedback, feedback_weight)
                .await?;
        }

        // Validate optimized plan
        let validation_result = self
            .validate_optimized_plan(&optimized_plan, base_plan)
            .await?;

        Ok(FeedbackOptimizedPlan {
            optimized_plan,
            feedback_applied: performance_insights,
            improvement_estimate: validation_result.estimated_improvement,
    ...
    pub async fn adapt_resource_usage(
        ...
        let current_usage = self
            .analyze_current_resource_usage(current_plan, system_state)
            .await?;

        // Identify resource bottlenecks
        let bottlenecks =
            self.identify_resource_bottlenecks(&current_usage, resource_constraints)?;

        // Generate adaptation strategies
        let adaptation_strategies = self
            .generate_adaptation_strategies(&bottlenecks, system_state)
            .await?;

        // Select optimal adaptation strategy
        let optimal_strategy =
            self.select_optimal_adaptation_strategy(&adaptation_strategies, resource_constraints)?;
        ...
        let adaptation_plan = ResourceAdaptationPlan {
            ...
            resource_adjustments: self
                .calculate_resource_adjustments(&optimal_strategy, current_plan)
                .await?,
            expected_impact: self
                .estimate_adaptation_impact(&optimal_strategy, &current_usage)
                .await?,
            implementation_steps: self
                .generate_implementation_steps(&optimal_strategy)
                .await?,
        };
    ...
    fn create_subgraph(
        ...
        let subgraph_patterns = component
            .patterns
            .iter()
        ...
        Ok(JoinSubgraph {
            id: format!("subgraph_{}", uuid::Uuid::new_v4()),
            patterns: subgraph_patterns,
            estimated_cost: self.estimate_subgraph_cost(component, patterns)?,
            parallel_safe: component.parallel_safe,
        })
    }
    ...
    fn build_optimal_bushy_tree_dp(
        &self,
        pattern_indices: &[usize],
        _join_variables: &HashMap<String, Vec<usize>>,
    ) -> Result<BushyJoinTree> {
        // Simple implementation - would be more sophisticated in practice
        if pattern_indices.len() == 1 {
            Ok(BushyJoinTree {
                pattern_index: Some(pattern_indices[0]),
                left: None,
                right: None,
                estimated_cost: Some(1.0),
            })
        } else {
            let mid = pattern_indices.len() / 2;
            let left =
                self.build_optimal_bushy_tree_dp(&pattern_indices[..mid], _join_variables)?;
            let right =
                self.build_optimal_bushy_tree_dp(&pattern_indices[mid..], _join_variables)?;
            let cost = left.estimated_cost.unwrap_or(1.0) + right.estimated_cost.unwrap_or(1.0);

            Ok(BushyJoinTree {
                pattern_index: None,
                left: Some(Box::new(left)),
                right: Some(Box::new(right)),
                estimated_cost: Some(cost * 1.5), // Join cost multiplier
            })
        }
    }

    fn extract_join_order(&self, tree: &BushyJoinTree) -> Result<Vec<usize>> {
        let mut order = Vec::new();
        self.collect_order_from_bushy_tree(tree, &mut order);
        Ok(order)
    }

    fn collect_order_from_bushy_tree(&self, tree: &BushyJoinTree, order: &mut Vec<usize>) {
        if let Some(pattern_index) = tree.pattern_index {
            order.push(pattern_index);
        } else {
            if let Some(ref left) = tree.left {
                self.collect_order_from_bushy_tree(left, order);
            }
            if let Some(ref right) = tree.right {
                self.collect_order_from_bushy_tree(right, order);
            }
        }
    }
}
...
impl QueryPlanner {
    ...
    fn analyze_schema_alignment_coverage(
        ...
        if let Some(ref metadata) = service.extended_metadata {
            if let Some(ref known_vocabularies) = metadata.known_vocabularies {
                ...
                // Check for schema mappings
                if let Some(ref schema_mappings) = metadata.schema_mappings {
                    if schema_mappings.contains_key(&pattern_vocabulary) {
                        alignment_score += 0.4;
                    }
                }
            }
        ...
        if let Some(ref metadata) = service.extended_metadata {
            if let Some(ref performance_history) = metadata.performance_history {
                ...
                for (signature, performance) in performance_history {
                    let similarity =
                        self.calculate_pattern_signature_similarity(&pattern_signature, signature)?;
                    if similarity > 0.7 {
    ...
    fn calculate_pattern_similarity_score(
        ...
        if let Some(ref metadata) = service.extended_metadata {
            if let Some(ref successful_patterns) = metadata.successful_query_patterns {
                let pattern_features = self.extract_pattern_features(pattern)?;

                for successful_pattern in successful_patterns {
                    let pattern_similarity =
                        self.calculate_feature_similarity(&pattern_features, successful_pattern)?;
    ...
    fn predict_service_performance(
        ...
        prediction_score += service_features.reliability_score * 0.3;
        prediction_score += service_features.performance_score * 0.4;
    ...
    fn estimate_service_specific_selectivity(
        ...
        if let Some(ref metadata) = service.extended_metadata {
            ...
            if metadata.estimated_triple_count.unwrap_or(0) > 10_000_000 {
                selectivity *= 0.5;
            }
    ...
    fn apply_multi_objective_optimization(
        ...
        for (pattern_idx, pattern) in patterns.iter().enumerate() {
            ...
            for service in services {
                ...
                let cost_score = 1.0 - self.estimate_pattern_cost(pattern, service)?;
