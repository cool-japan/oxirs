...
impl QueryPlanOptimizer {
    ...
    pub async fn optimize_query_plan(&self, 
        query_hash: &str, 
        ...
        if let Some(cached_plan) = self.plan_cache.read().await.get(query_hash) {
            self.optimization_stats.cache_hits.fetch_add(1, Ordering::Relaxed);
            return Ok(cached_plan.clone());
        }
        
        self.optimization_stats.cache_misses.fetch_add(1, Ordering::Relaxed);
        
        let model = self.plan_model.read().await;
        
        if model.training_samples < 10 {
            // Not enough training data, use heuristics
            return self.generate_heuristic_plan(complexity, available_services).await;
        }
        
        // Generate candidate plans
        let candidate_plans = self.generate_candidate_plans(complexity, available_services).await;
        ...
        for plan_type in candidate_plans {
            ...
            if score > best_score {
                ...
                best_plan = Some(OptimalPlan {
                    ...
                    service_execution_order: self.optimize_service_order(available_services, &plan_type),
        ...
        // Cache the result
        self.plan_cache.write().await.insert(query_hash.to_string(), optimal_plan.clone());
        
        self.optimization_stats.total_optimizations.fetch_add(1, Ordering::Relaxed);
    ...
    async fn retrain_model(&self) -> Result<()> {
        ...
        model.execution_time_weights = self.train_linear_regression(&features_matrix, &execution_time_targets);
        model.memory_usage_weights = self.train_linear_regression(&features_matrix, &memory_usage_targets);
        model.success_rate_weights = self.train_linear_regression(&features_matrix, &success_targets);
        
        // Calculate accuracy metrics
        model.model_accuracy.execution_time_r_squared = 
            self.calculate_r_squared(&features_matrix, &execution_time_targets, &model.execution_time_weights);
        model.model_accuracy.memory_usage_r_squared = 
            self.calculate_r_squared(&features_matrix, &memory_usage_targets, &model.memory_usage_weights);
        model.model_accuracy.success_rate_accuracy = 
            self.calculate_classification_accuracy(&features_matrix, &success_targets, &model.success_rate_weights);
        
        model.model_accuracy.last_training = SystemTime::now();
        model.training_samples = historical_plans.len();
        
        self.optimization_stats.model_retrainings.fetch_add(1, Ordering::Relaxed);
    ...
    async fn generate_heuristic_plan(&self, complexity: &QueryComplexity, available_services: &[String]) -> Result<OptimalPlan> {
        ...
        Ok(OptimalPlan {
            plan_type,
            predicted_execution_time: Duration::from_millis((complexity.complexity_score * 100.0) as u64),
    ...
    async fn generate_candidate_plans(&self, complexity: &QueryComplexity, _services: &[String]) -> Vec<PlanType> {
    ...
    fn extract_plan_features(&self, complexity: &QueryComplexity, plan_type: &PlanType, services: &[String]) -> Vec<f64> {
        vec![
            ...
            if complexity.optional_patterns > 0 { 1.0 } else { 0.0 },
    ...
    fn calculate_classification_accuracy(&self, features: &[Vec<f64>], targets: &[f64], weights: &[f64]) -> f64 {
        if features.is_empty() || targets.is_empty() {
            return 0.0;
        }
    ...
    fn determine_parallel_groups(&self, services: &[String], plan_type: &PlanType) -> Vec<Vec<String>> {
        match plan_type {
            PlanType::Parallel => {
    ...
    fn calculate_adaptive_timeout(&self, predicted_time: f64) -> Duration {
        ...
        let timeout_ms = (predicted_time * buffer_factor).max(1000.0).min(300000.0); // 1s to 5min
        Duration::from_millis(timeout_ms as u64)
    }

    /// Recommend cache strategy based on query characteristics
    fn recommend_cache_strategy(&self, complexity: &QueryComplexity, predicted_time: f64) -> CacheStrategy {
        if predicted_time > 10000.0 { // > 10 seconds
            CacheStrategy::AggressiveCache
        } else if complexity.service_count > 3 {
            CacheStrategy::IntermediateCache
        } else if predicted_time > 1000.0 { // > 1 second
    ...
    pub async fn get_optimization_stats(&self) -> QueryOptimizationStats {
        QueryOptimizationStats {
            total_optimizations: AtomicU64::new(self.optimization_stats.total_optimizations.load(Ordering::Relaxed)),
            successful_predictions: AtomicU64::new(self.optimization_stats.successful_predictions.load(Ordering::Relaxed)),
            model_retrainings: AtomicU64::new(self.optimization_stats.model_retrainings.load(Ordering::Relaxed)),
            cache_hits: AtomicU64::new(self.optimization_stats.cache_hits.load(Ordering::Relaxed)),
            cache_misses: AtomicU64::new(self.optimization_stats.cache_misses.load(Ordering::Relaxed)),
            average_improvement: Arc::new(RwLock::new(*self.optimization_stats.average_improvement.read().await)),
            total_time_saved: Arc::new(RwLock::new(*self.optimization_stats.total_time_saved.read().await)),
        }
    }
...
mod query_optimizer_tests {
    ...
    async fn test_heuristic_plan_generation() {
        ...
        let plan = optimizer.optimize_query_plan("test_query", &complexity, &services).await.unwrap();
