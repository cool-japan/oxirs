...
use anyhow::Result;
use oxirs_federate::{
    AuthConfig, AuthType, AuthCredentials, AutoDiscovery, AutoDiscoveryConfig,
    FederatedService, ServiceDiscovery, ServiceRegistry, ServiceType
};
...
async fn demo_service_registry() -> Result<()> {
    ...
    info!("  - Capabilities distribution: {:?}", stats.capabilities_distribution);
...
async fn demo_service_discovery() -> Result<()> {
    ...
    info!("Discovering services from {} endpoints...", test_endpoints.len());
...
async fn demo_auto_discovery() -> Result<()> {
    ...
    match auto_discovery.start().await {
        Ok(mut receiver) => {
            ...
            info!("📈 Total services discovered: {}", discovered_services.len());
        }
