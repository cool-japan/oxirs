...
use oxirs_core::concurrent::{ConcurrentGraph, pin};
use oxirs_core::model::{BlankNode, Literal, NamedNode, Subject, Object, Predicate, Triple};
...
fn test_stress_with_mixed_types() {
                    ...
                    match i % 3 {
                        0 => {
                            ...
                            let triple = Triple::new(
                                Subject::NamedNode(
                                    NamedNode::new(&format!("http://thread{}/s{}", thread_id, i)).unwrap()
                                ),
    ...
    let literal_matches = graph.iter()
        .filter(|t| matches!(t.object(), Object::Literal(_)))
        .count();
    let blank_matches = graph.iter()
            ...
            thread::spawn(move || {
                ...
                while start.elapsed() < duration {
                    for i in 0..10 {
                        ...
                        let triple = Triple::new(
                            Subject::NamedNode(
                                NamedNode::new(&format!("http://s{}", id)).unwrap()
                            ),
                ...
                for batch_id in 0..batches_per_thread {
                    ...
                    let batch: Vec<_> = (0..batch_size)
                        .map(|i| {
                            Triple::new(
                                Subject::NamedNode(
                                    NamedNode::new(&format!("http://batch/s{}", base_id + i)).unwrap()
                                ),
                                Predicate::NamedNode(NamedNode::new("http://batch/pred").unwrap()),
                                Object::Literal(
                                    Literal::new_simple_literal(&format!("batch_{}", base_id + i))
                                ),
                            )
                        })
                        .collect();
