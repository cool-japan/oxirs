...
use oxirs_core::model::{Triple, Subject, Predicate, Object, NamedNode, BlankNode, Literal};
...
fn test_multiple_triples() {
    ...
    let triples = vec![
        create_triple("http://example.org/alice", "http://example.org/knows", "Bob"),
        create_triple("http://example.org/alice", "http://example.org/age", "30"),
        create_triple("http://example.org/bob", "http://example.org/knows", "Charlie"),
...
fn test_subject_queries() {
    ...
    graph.insert(&create_triple("http://example.org/alice", "http://example.org/p1", "o1"));
    graph.insert(&create_triple("http://example.org/alice", "http://example.org/p2", "o2"));
    graph.insert(&create_triple("http://example.org/bob", "http://example.org/p1", "o3"));
...
fn test_predicate_queries() {
    ...
    graph.insert(&create_triple("http://example.org/s1", "http://example.org/p1", "o1"));
    graph.insert(&create_triple("http://example.org/s2", "http://example.org/p1", "o2"));
    graph.insert(&create_triple("http://example.org/s3", "http://example.org/p2", "o3"));
...
fn test_object_queries() {
    ...
    graph.insert(&create_triple("http://example.org/s1", "http://example.org/p1", "object1"));
    graph.insert(&create_triple("http://example.org/s2", "http://example.org/p2", "object1"));
    graph.insert(&create_triple("http://example.org/s3", "http://example.org/p3", "object2"));
...
fn test_compound_queries() {
    ...
    graph.insert(&create_triple("http://example.org/alice", "http://example.org/knows", "Bob"));
    graph.insert(&create_triple("http://example.org/alice", "http://example.org/age", "30"));
    graph.insert(&create_triple("http://example.org/bob", "http://example.org/knows", "Charlie"));
    graph.insert(&create_triple("http://example.org/bob", "http://example.org/age", "25"));
    ...
    assert_eq!(who_is_30[0].subject().to_string(), "<http://example.org/alice>");
}
...
fn test_language_tagged_literals() {
    ...
    let lang_literal = Object::Literal(Literal::new_language_tagged_literal("Hello", "en").unwrap());
...
fn test_clear_and_reuse() {
    ...
    // Insert and clear
    graph.insert(&create_triple("http://example.org/s", "http://example.org/p", "o"));
    assert_eq!(graph.len(), 1);
    ...
    // Reuse after clear
    graph.insert(&create_triple("http://example.org/s2", "http://example.org/p2", "o2"));
    assert_eq!(graph.len(), 1);
}
...
fn test_shared_interner() {
    ...
    // Insert in graph1
    graph1.insert(&create_triple("http://example.org/s", "http://example.org/p", "o"));
    
    // Insert in graph2
    graph2.insert(&create_triple("http://example.org/s", "http://example.org/p2", "o2"));
...
fn test_concurrent_operations() {
    ...
    for i in 0..10 {
        let graph_clone = Arc::clone(&graph);
        let handle = thread::spawn(move || {
            let p = Predicate::NamedNode(NamedNode::new(&format!("http://example.org/p{}", i)).unwrap());
