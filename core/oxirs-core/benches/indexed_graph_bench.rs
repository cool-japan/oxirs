...
fn generate_test_triples(count: usize) -> Vec<Triple> {
    ...
    (0..count)
        .map(|_| {
            create_triple(
                &format!("http://example.org/s{}", rng.gen_range(0..1000)),
                &format!("http://example.org/p{}", rng.gen_range(0..10)),
                &format!("object{}", rng.gen_range(0..10000)),
            )
        })
        .collect()
}
...
fn bench_query_patterns(c: &mut Criterion) {
    ...
    group.bench_function("query_by_subject", |b| {
        ...
        b.iter(|| {
            graph.query(Some(black_box(&subject)), None, None)
        });
    });
    ...
    group.bench_function("query_by_predicate", |b| {
        ...
        b.iter(|| {
            graph.query(None, Some(black_box(&predicate)), None)
        });
    });
    ...
    group.bench_function("query_by_object", |b| {
        ...
        b.iter(|| {
            graph.query(None, None, Some(black_box(&object)))
        });
    });
    ...
    group.bench_function("query_by_sp", |b| {
        ...
        let predicate = Predicate::NamedNode(NamedNode::new("http://example.org/p5").unwrap());
        b.iter(|| {
            graph.query(Some(black_box(&subject)), Some(black_box(&predicate)), None)
        });
    });
    ...
    group.bench_function("query_by_po", |b| {
        ...
        let object = Object::Literal(Literal::new("object5"));
        b.iter(|| {
            graph.query(None, Some(black_box(&predicate)), Some(black_box(&object)))
        });
    });
    ...
    group.bench_function("query_by_so", |b| {
        ...
        let object = Object::Literal(Literal::new("object5"));
        b.iter(|| {
            graph.query(Some(black_box(&subject)), None, Some(black_box(&object)))
        });
    });
            ...
            |graph| {
                ...
                for i in 0..1000 {
                    let triple = create_triple(
                        ...
                        &format!("This is object number {} with some text", i),
                    );
                    graph.insert(&triple);
                }
