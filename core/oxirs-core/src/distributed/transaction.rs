...
impl TransactionCoordinator {
    ...
    pub async fn add_operation(
        ...
        // Update transaction properties
        {
            ...
            if matches!(operation, TransactionOp::Insert(_) | TransactionOp::Remove(_)) {
        ...
        *transaction.state.write() = TransactionState::Preparing;
        ...
        for shard_id in participants {
            ...
            prepare_futures.push(async move {
                sim_rx.await.unwrap_or(Vote::No(AbortReason::NodeFailure))
            });
        }
    ...
    async fn commit_single_shard(&self, transaction_id: TransactionId) -> Result<()> {
        ...
        let shard_id = *transaction.participants
            .read()
            .iter()
            .next()
    ...
    fn filter_operations_for_shard(
        ...
        operations
            .iter()
            .filter(|op| {
                match self.get_affected_shards(op) {
    ...
    fn simulate_participant_vote(
        &self,
        _shard_id: ShardId,
        _operations: &[TransactionOp],
    ) -> Vote {
...
mod tests {
    ...
    fn test_lock_manager() {
        ...
        assert!(lock_manager.acquire_lock(tx1, lock1.clone(), LockType::Exclusive).is_ok());
        
        // TX2 tries to acquire, should fail
        assert!(lock_manager.acquire_lock(tx2, lock1.clone(), LockType::Shared).is_err());
        
        // Release TX1's locks
        lock_manager.release_transaction_locks(tx1);
        
        // Now TX2 should be able to acquire
        assert!(lock_manager.acquire_lock(tx2, lock1, LockType::Shared).is_ok());
    }
