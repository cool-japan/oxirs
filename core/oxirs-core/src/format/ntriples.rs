...
use crate::model::{Triple, TripleRef, NamedNode, BlankNode, Literal};
use crate::model::term::{Subject, Predicate, Object};
...
impl NTriplesParser {
    ...
    fn parse_term(&self, input: &str, start_pos: usize, line_number: usize) -> ParseResult<(NTriplesTerm, usize)> {
    ...
    /// Parse an IRI term <...>
    fn parse_iri(&self, input: &str, start_pos: usize, line_number: usize) -> ParseResult<(NTriplesTerm, usize)> {
    ...
    fn parse_blank_node(&self, input: &str, start_pos: usize, line_number: usize) -> ParseResult<(NTriplesTerm, usize)> {
    ...
    fn parse_literal(&self, input: &str, start_pos: usize, line_number: usize) -> ParseResult<(NTriplesTerm, usize)> {
        ...
        if remaining.starts_with('@') {
            ...
            Ok((NTriplesTerm::LanguageLiteral(literal_value, language), pos_after_quote))
            ...
            if let Some(datatype_end) = remaining[3..].find('>') {
                ...
                Ok((NTriplesTerm::TypedLiteral(literal_value, datatype), pos_after_quote))
            } else {
                Err(RdfParseError::syntax_at(
    ...
    fn term_to_subject(&self, term: &NTriplesTerm, line_number: usize) -> ParseResult<crate::model::term::Subject> {
        match term {
            NTriplesTerm::Iri(iri) => {
                let named_node = NamedNode::new(iri)
                    .map_err(|e| RdfParseError::syntax_at(
            ...
            NTriplesTerm::BlankNode(id) => {
                let blank_node = BlankNode::new(id)
                    .map_err(|e| RdfParseError::syntax_at(
    ...
    fn term_to_predicate(&self, term: &NTriplesTerm, line_number: usize) -> ParseResult<crate::model::term::Predicate> {
        match term {
            NTriplesTerm::Iri(iri) => {
                let named_node = NamedNode::new(iri)
                    .map_err(|e| RdfParseError::syntax_at(
    ...
    fn term_to_object(&self, term: &NTriplesTerm, line_number: usize) -> ParseResult<crate::model::term::Object> {
        match term {
            NTriplesTerm::Iri(iri) => {
                let named_node = NamedNode::new(iri)
                    .map_err(|e| RdfParseError::syntax_at(
...
impl<W: Write> WriterNTriplesSerializer<W> {
    ...
    fn serialize_subject(&mut self, subject: crate::model::triple::SubjectRef<'_>) -> SerializeResult<()> {
    ...
    fn serialize_predicate(&mut self, predicate: crate::model::triple::PredicateRef<'_>) -> SerializeResult<()> {
    ...
    fn serialize_object(&mut self, object: crate::model::triple::ObjectRef<'_>) -> SerializeResult<()> {
    ...
    fn serialize_literal(&mut self, literal: &Literal) -> SerializeResult<()> {
        ...
        if let Some(language) = literal.language() {
            ...
            write!(self.writer, "^^<{}>", self.escape_iri(literal.datatype().as_str()))?;
        }
...
mod tests {
    ...
    fn test_ntriples_line_validation() {
        ...
        let result = parser.parse_triple_line("<http://example.org/s> <http://example.org/p> <http://example.org/o>", 1);
