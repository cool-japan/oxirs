...
impl AsyncStreamingParser {
        ...
        Fut: Future<Output = Result<()>>,
    {
        ...
        loop {
            ...
            if matches!(self.format, RdfFormat::NTriples | RdfFormat::NQuads) {
                self.process_lines_async(&mut accumulated_data, &mut line_buffer, &mut handler).await?;
            }
        ...
        if !accumulated_data.is_empty() {
            match self.format {
                RdfFormat::NTriples | RdfFormat::NQuads => {
                    ...
                    self.process_lines_async(&mut accumulated_data, &mut String::new(), &mut handler).await?;
                }
                _ => {
    ...
    fn parse_line(&self, line: &str) -> Result<Option<Quad>> {
        ...
        match self.format {
            ...
            _ => Err(OxirsError::Parse("Unsupported format for line parsing".to_string()))
        }
    }
