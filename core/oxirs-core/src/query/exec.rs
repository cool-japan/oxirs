...
impl<'a> QueryExecutor<'a> {
    ...
    fn execute_plan(&self, plan: &ExecutionPlan) -> Result<Vec<Solution>, OxirsError> {
        match plan {
            ExecutionPlan::TripleScan { pattern } => {
                self.execute_triple_scan(pattern)
            }
            ...
            ExecutionPlan::Filter { input, condition } => {
                self.execute_filter(input, condition)
            }
            ExecutionPlan::Project { input, vars } => {
                self.execute_project(input, vars)
            }
            ExecutionPlan::Sort { input, order_by } => {
                self.execute_sort(input, order_by)
            }
            ExecutionPlan::Limit { input, limit, offset } => {
    ...
    fn execute_filter(
        ...
        Ok(solutions
            .into_iter()
            .filter(|solution| self.evaluate_expression(condition, solution).unwrap_or(false))
            .collect())
    }
