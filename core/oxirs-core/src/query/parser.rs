...
impl SparqlParser {
    ...
    pub fn parse_update(&self, update: &str) -> Result<Update, OxirsError> {
        ...
        if update.to_uppercase().starts_with("INSERT DATA") {
            ...
            Err(OxirsError::Parse(format!("Unsupported update operation")))
        }
    }
    ...
    fn parse_construct_query(&self, _query: &str) -> Result<Query, OxirsError> {
        ...
        Err(OxirsError::Parse(
            "CONSTRUCT queries not yet implemented".to_string(),
        ))
    }
    ...
    fn parse_describe_query(&self, _query: &str) -> Result<Query, OxirsError> {
        // Placeholder
        Err(OxirsError::Parse(
            "DESCRIBE queries not yet implemented".to_string(),
        ))
    }
    ...
    fn parse_insert_data(&self, update: &str) -> Result<Update, OxirsError> {
        // Extract data block
        let data_start = update
            .to_uppercase()
            .find("INSERT DATA")
            .ok_or_else(|| OxirsError::Parse("Invalid INSERT DATA".to_string()))?
            + 11;

        let data_text = update[data_start..].trim();
        if !data_text.starts_with('{') || !data_text.ends_with('}') {
            return Err(OxirsError::Parse(
                "INSERT DATA must have data in {}".to_string(),
            ));
        }
    ...
    fn parse_delete_data(&self, update: &str) -> Result<Update, OxirsError> {
        ...
        let data_start = update
            .to_uppercase()
            .find("DELETE DATA")
            .ok_or_else(|| OxirsError::Parse("Invalid DELETE DATA".to_string()))?
            + 11;

        let data_text = update[data_start..].trim();
        if !data_text.starts_with('{') || !data_text.ends_with('}') {
            return Err(OxirsError::Parse(
                "DELETE DATA must have data in {}".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_simple_select_query() {
        ...
        if let Ok(Query {
            form: QueryForm::Select { variables, .. },
            ..
        }) = result
        {
            match variables {
