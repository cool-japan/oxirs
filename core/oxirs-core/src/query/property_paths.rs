...
impl PropertyPath {
    ...
    pub fn max_length(&self) -> Option<usize> {
        match self {
            ...
            PropertyPath::Sequence(l, r) => {
                match (l.max_length(), r.max_length()) {
    ...
    fn collect_predicates<'a>(&'a self, predicates: &mut HashSet<&'a NamedNode>) {
        match self {
            ...
            PropertyPath::FixedLength(p, _) |
            PropertyPath::RangeLength(p, _, _) => p.collect_predicates(predicates),
...
impl PropertyPathOptimizer {
    ...
    fn optimize_recursive(&self, path: PropertyPath) -> PropertyPath {
        match path {
            ...
            // Optimize p? | p+ to p*
            PropertyPath::Alternative(ref l, ref r) => {
            ...
            PropertyPath::NegatedPropertySet(_) => path,
        }
    }
}
