...
use crate::model::{Triple, NamedNode, BlankNode, Literal, Subject, Predicate, Object};
...
impl VersionChain {
    ...
    fn add_version(&mut self, version: Version) {
        ...
        let pos = self.versions
            .binary_search_by_key(&std::cmp::Reverse(version.timestamp), |v| std::cmp::Reverse(v.timestamp))
...
impl MvccStore {
    ...
    pub fn insert(
        ...
        // Add to write set
        tx.write_set.insert(key.clone(), WriteOperation::Insert(triple.clone()));
    ...
    fn get_active_transaction(&self, tx_id: TransactionId) -> Result<dashmap::mapref::one::RefMut<TransactionId, TransactionState>> {
    ...
    fn update_indexes_for_insert(&self, key: &TripleKey) {
        self.indexes.subject_index
            .entry(key.subject.clone())
            .or_default()
            .insert(key.clone());
        
        self.indexes.predicate_index
    ...
    fn create_snapshot(&self, timestamp: Timestamp) -> Result<()> {
        let active_txs: HashSet<TransactionId> = self.transactions
            .iter()
    ...
    fn run_gc_internal(
        ...
        for mut entry in versions.iter_mut() {
            entry.value_mut().gc_versions(min_timestamp, config.max_versions_per_triple);
        }
    }
    ...
    fn calculate_min_timestamp(&self) -> Timestamp {
        ...
        let min_active = self.transactions
            .iter()
            .filter(|entry| entry.value().status == TransactionStatus::Active)
        ...
        let min_snapshot = self.snapshots
            .read()
            .keys()
            .next()
            .copied()
    ...
    pub fn get_stats(&self) -> MvccStats {
        let total_versions = self.versions
            .iter()
        ...
        MvccStats {
            ...
            total_versions,
            active_transactions: self.transactions
                .iter()
                .filter(|entry| entry.value().status == TransactionStatus::Active)
                .count(),
