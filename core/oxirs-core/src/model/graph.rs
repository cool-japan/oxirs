...
use crate::model::{Triple, TripleRef, Subject, Predicate, Object};
...
mod tests {
    ...
    fn test_graph_pattern_matching() {
        ...
        // Find by subject
        let by_subject: Vec<_> = graph.triples_for_subject(&Subject::NamedNode(subject.clone())).cloned().collect();
        assert_eq!(by_subject.len(), 2);
        
        // Find by predicate
        let by_predicate: Vec<_> = graph.triples_for_predicate(&Predicate::NamedNode(predicate1)).cloned().collect();
