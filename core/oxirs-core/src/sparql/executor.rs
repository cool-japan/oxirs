...
mod tests {
    ...
    fn make_populated_backend() -> StorageBackend {
        ...
        // Insert: <http://example.org/alice> <http://xmlns.com/foaf/0.1/name> "Alice"
    ...
    fn test_select_with_specific_subject() {
        ...
        // Alice has: name, knows, age = 3 triples
        assert_eq!(
            result.len(),
            3,
    ...
    fn test_select_with_specific_predicate() {
        ...
        // Two people have names: alice and bob
        assert_eq!(
            result.len(),
            2,
    ...
    fn test_select_with_specific_object_iri() {
        ...
        let executor = QueryExecutor::new(&backend);
        let query =
            "SELECT ?s WHERE { ?s <http://xmlns.com/foaf/0.1/knows> <http://example.org/bob> }";
        let result = executor
            .execute(query)
    ...
    fn test_select_with_offset_and_limit() {
        ...
        // LIMIT 3 should yield exactly 3 results (since total >= 3)
        if total >= 3 {
            assert_eq!(
                limited_results.len(),
                3,
    ...
    fn test_ask_returns_true_when_matching() {
        let backend = make_populated_backend();
        let executor = QueryExecutor::new(&backend);
        // Use WHERE keyword so the executor's extract_triple_patterns can find the pattern
        let result = executor
            .execute("ASK WHERE { ?s ?p ?o }")
            .expect("ASK WHERE should succeed");
        // Result should be a boolean true
    ...
    fn test_describe_query_specific_resource() {
        ...
        // DESCRIBE should return triples about alice
        // At minimum we should not error out
        let _ = result.len(); // Just verify it runs without panic
    }
    ...
    fn test_prefix_expansion_in_select() {
        ...
        // Should find both alice and bob's names
        assert_eq!(
            result.len(),
            2,
    ...
    fn test_select_distinct_reduces_duplicates() {
        ...
        // DISTINCT should return <= the total (possibly fewer due to deduplication)
        assert!(
