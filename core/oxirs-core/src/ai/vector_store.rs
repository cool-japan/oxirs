...
pub trait VectorStore: Send + Sync {
    ...
    async fn insert(&self, id: String, vector: Vec<f32>, metadata: Option<HashMap<String, String>>) -> Result<()>;
    ...
    /// Update vector
    async fn update(&self, id: String, vector: Vec<f32>, metadata: Option<HashMap<String, String>>) -> Result<()>;
...
impl VectorStore for InMemoryVectorStore {
    async fn insert(&self, id: String, vector: Vec<f32>, metadata: Option<HashMap<String, String>>) -> Result<()> {
        ...
        if let Some(index) = self.index.write().await.as_mut() {
            index.add(id, self.vectors.get(&id_for_lookup).unwrap().vector.clone()).await?;
        }
    ...
    async fn build_index(&self) -> Result<()> {
        ...
        let mut new_index: Box<dyn VectorIndex> = match &self.config.index_type {
            ...
            IndexType::HNSW { max_connections, ef_construction, ef_search } => {
                Box::new(HNSWIndex::new(*max_connections, *ef_construction, *ef_search))
            }
...
pub fn compute_similarity(a: &[f32], b: &[f32], metric: SimilarityMetric) -> Result<f32> {
    ...
    match metric {
        ...
        SimilarityMetric::Hamming => {
            ...
            let differences: usize = a_binary.iter()
                .zip(b_binary.iter())
                .map(|(x, y)| if x != y { 1 } else { 0 })
                .sum();
...
mod tests {
    ...
    async fn test_vector_insertion_and_retrieval() {
        ...
        let metadata = Some([("type".to_string(), "test".to_string())].iter().cloned().collect());
        
        store.insert("test1".to_string(), vector.clone(), metadata.clone()).await.unwrap();
    ...
    async fn test_similarity_search() {
        ...
        // Insert test vectors
        store.insert("vec1".to_string(), vec![1.0, 0.0, 0.0], None).await.unwrap();
        store.insert("vec2".to_string(), vec![0.9, 0.1, 0.0], None).await.unwrap();
        store.insert("vec3".to_string(), vec![0.0, 1.0, 0.0], None).await.unwrap();
    ...
    async fn test_index_building() {
        ...
        store.insert("vec1".to_string(), vec![1.0, 0.0, 0.0], None).await.unwrap();
        store.insert("vec2".to_string(), vec![0.0, 1.0, 0.0], None).await.unwrap();
