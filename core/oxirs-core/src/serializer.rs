...
use crate::{Result, model::{graph::Graph, dataset::Dataset, Quad, GraphName}, parser::RdfFormat};
...
impl TurtleSerializer {
    fn new() -> Self {
        ...
        prefixes.insert("http://purl.org/dc/elements/1.1/".to_string(), "dc".to_string());
    ...
    fn serialize_graph(&mut self, graph: &Graph) -> Result<String> {
        ...
        let mut subjects_map: std::collections::HashMap<crate::model::Subject, Vec<&crate::model::Triple>> = std::collections::HashMap::new();
        
        for triple in graph.iter() {
            subjects_map.entry(triple.subject().clone()).or_insert_with(Vec::new).push(triple);
        }
        ...
        for (i, (subject, triples)) in subject_entries.iter().enumerate() {
            ...
            let mut predicates_map: std::collections::HashMap<crate::model::Predicate, Vec<&crate::model::Object>> = std::collections::HashMap::new();
            
            for triple in triples.iter() {
                predicates_map.entry(triple.predicate().clone()).or_insert_with(Vec::new).push(triple.object());
            }
