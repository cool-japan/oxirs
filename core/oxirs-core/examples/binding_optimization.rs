...
use oxirs_core::query::binding_optimizer::{
    BindingMetadata, ValueConstraintType, RelationType,
};
...
fn value_constraint_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Age 25: {}", 
        if bindings.bind(var_age.clone(), valid_age, BindingMetadata::default()).is_ok() {
            "Valid"
        } else {
            "Invalid"
        }
    ...
    println!("  Email alice@example.com: {}",
        if bindings.bind(var_email.clone(), valid_email, BindingMetadata::default()).is_ok() {
            "Valid"
        } else {
            "Invalid"
        }
    ...
    println!("  Country USA: {}",
        if bindings.bind(var_country.clone(), valid_country, BindingMetadata::default()).is_ok() {
            "Valid"
        } else {
            "Invalid"
        }
    ...
    println!("  Age 200: {}",
        if bindings.bind(var_age, invalid_age, BindingMetadata::default()).is_ok() {
            "Valid"
        } else {
            "Invalid"
        }
    ...
    println!("  Email not-an-email: {}",
        if bindings.bind(var_email, invalid_email, BindingMetadata::default()).is_ok() {
            "Valid"
        } else {
            "Invalid"
        }
    ...
    println!("  Country Mars: {}",
        if bindings.bind(var_country, invalid_country, BindingMetadata::default()).is_ok() {
            "Valid"
        } else {
            "Invalid"
        }
...
fn relationship_constraint_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(binding) = iterator.next_valid() {
        if count < 5 {
            // Show first 5
            println!("  Start: {}, End: {}, Duration: {} days",
...
fn complex_query_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let test_data = vec![
        ("Valid person", vec![
            (var_person.clone(), Term::NamedNode(NamedNode::new("http://example.org/alice")?)),
            (var_name.clone(), Term::Literal(Literal::new("Alice Smith"))),
            (var_age.clone(), Term::Literal(Literal::new("30"))),
            (var_city.clone(), Term::NamedNode(NamedNode::new("http://example.org/NYC")?)),
        ]),
        ("Invalid age", vec![
            (var_person.clone(), Term::NamedNode(NamedNode::new("http://example.org/bob")?)),
            (var_name.clone(), Term::Literal(Literal::new("Bob Jones"))),
            (var_age.clone(), Term::Literal(Literal::new("70"))), // Too old
            (var_city.clone(), Term::NamedNode(NamedNode::new("http://example.org/LA")?)),
        ]),
        ("Invalid name", vec![
            (var_person.clone(), Term::NamedNode(NamedNode::new("http://example.org/charlie")?)),
            (var_name.clone(), Term::Literal(Literal::new("charlie"))), // Lowercase
            (var_age.clone(), Term::Literal(Literal::new("25"))),
            (var_city.clone(), Term::NamedNode(NamedNode::new("http://example.org/SF")?)),
        ]),
    ];
