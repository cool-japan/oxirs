...
use oxirs_core::format::{
    RdfFormat, FormatHandler, FormatDetection, RdfParser, RdfSerializer,
    TurtleParser, TurtleSerializer, NTriplesParser, NTriplesSerializer,
    JsonLdParser, JsonLdSerializer, JsonLdProfile, JsonLdProfileSet,
    RdfXmlParser, RdfXmlSerializer,
};
use oxirs_core::format::serializer::simple;
use oxirs_core::model::{NamedNode, Triple, Quad, Literal, BlankNode};
...
fn test_format_detection() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(RdfFormat::from_media_type("application/ld+json"), Some(RdfFormat::JsonLd { 
        profile: JsonLdProfileSet::empty() 
    }));
...
fn test_turtle_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     ✓ Base IRI: {}", serializer.base_iri().unwrap_or("None"));
...
fn test_jsonld_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("   Parser configuration:");
    println!("     ✓ Profile: {} entries", parser.profile().profiles().len());
    ...
    println!("   Serializer configuration:");
    println!("     ✓ Profile: {} entries", serializer.profile().profiles().len());
...
fn test_rdfxml_format() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("     ✓ Prefixes: {} defined", serializer.prefixes().len());
    println!("     ✓ Pretty formatting: {}", serializer.is_pretty());
    println!("     ✓ XML declaration: {}", serializer.has_xml_declaration());
    ...
    println!("     ✓ Common prefixes: {} available", common_prefixes.len());
