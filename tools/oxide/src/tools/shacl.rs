...
pub async fn run(
    ...
    println!("Validation completed in {}", utils::format_duration(validation_duration));
    ...
    stats.errors = if validation_report.conforms { 0 } else { validation_report.results.len() };
    stats.finish();
                    ...
                    if line.contains("sh:targetClass") {
                        if let Some(class_start) = line.find('<') {
                            if let Some(class_end) = line.find('>') {
                                shape.target_class = Some(line[class_start+1..class_end].to_string());
                            }
                        }
                    }
                }
    ...
    for shape in &shapes_graph.shapes {
        ...
        for target_node in target_nodes {
            ...
            for property_shape in &shape.property_shapes {
                let validation_result = validate_property_shape(
                    data_graph,
                    &target_node,
                    property_shape,
                    shape,
        ...
        if values.len() < min_count {
            return Some(ValidationResult {
                ...
                message: format!(
                    ...
                    property_shape.path, values.len(), min_count
                ),
        ...
        if values.len() > max_count {
            return Some(ValidationResult {
                ...
                message: format!(
                    ...
                    property_shape.path, values.len(), max_count
                ),
...
fn format_turtle_report(report: &ValidationReport) -> ToolResult<String> {
    ...
    if !report.results.is_empty() {
        ...
        for (i, result) in report.results.iter().enumerate() {
            ...
            output.push_str(&format!("  sh:sourceConstraintComponent sh:{} ;\n", 
...
fn format_json_report(report: &ValidationReport) -> ToolResult<String> {
    ...
    for (i, result) in report.results.iter().enumerate() {
        ...
        output.push_str(&format!("      \"focusNode\": \"{}\",\n", result.focus_node));
        if let Some(ref path) = result.result_path {
            output.push_str(&format!("      \"resultPath\": \"{}\",\n", path));
        }
        output.push_str(&format!("      \"message\": \"{}\",\n", result.message));
        output.push_str(&format!("      \"sourceConstraintComponent\": \"{}\",\n", result.source_constraint_component));
        output.push_str(&format!("      \"sourceShape\": \"{}\"\n", result.source_shape));
        output.push_str("    }");
        if i < report.results.len() - 1 {
            output.push_str(",");
        }
...
fn format_xml_report(report: &ValidationReport) -> ToolResult<String> {
    ...
    for result in &report.results {
        ...
        output.push_str(&format!("    <focusNode>{}</focusNode>\n", result.focus_node));
        if let Some(ref path) = result.result_path {
            output.push_str(&format!("    <resultPath>{}</resultPath>\n", path));
        }
        output.push_str(&format!("    <message>{}</message>\n", result.message));
        output.push_str(&format!("    <sourceConstraintComponent>{}</sourceConstraintComponent>\n", result.source_constraint_component));
        output.push_str(&format!("    <sourceShape>{}</sourceShape>\n", result.source_shape));
        output.push_str("  </result>\n");
    }
