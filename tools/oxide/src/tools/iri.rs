    ...
    if let Some(scheme_end) = iri.find(':') {
        ...
        if rest.starts_with("://") {
            ...
            if let Some(authority_end) = rest[3..].find('/') {
                ...
                Ok(format!("{}:{}//{}{}", scheme, &rest[..3], authority, path_and_rest))
                ...
                Ok(format!("{}:{}//{}?{}", scheme, &rest[..3], authority, query_and_rest))
                ...
                Ok(format!("{}:{}//{}#{}", scheme, &rest[..3], authority, fragment))
            } else {
    ...
    for (scheme, default_port) in &default_ports {
        if iri.starts_with(scheme) {
            ...
            if let Some(port_pos) = after_scheme.find(default_port) {
                ...
                if !before_port.contains('/') && !before_port.contains('?') && !before_port.contains('#') {
                    let after_port = &after_scheme[port_pos + default_port.len()..];
                    if after_port.is_empty() || after_port.starts_with('/') || 
                       after_port.starts_with('?') || after_port.starts_with('#') {
    ...
    if let Some(scheme_end) = iri.find(':') {
        ...
        if rest.starts_with("//") {
            ...
            if let Some(path_start) = rest[2..].find('/') {
                ...
                // Find end of path
                let path_end = path_and_rest.find('?')
                ...
                return format!("{}{}{}{}", scheme_and_colon, authority, normalized_path, query_and_fragment);
            }
        } else {
...
fn resolve_iri(iri: &str, base: &str) -> ToolResult<String> {
    ...
    if iri.starts_with('/') {
        ...
        return Ok(format!("{}://{}{}", base_parts.scheme, base_parts.authority, iri));
    }
    ...
    Ok(format!("{}://{}{}", base_parts.scheme, base_parts.authority, normalized_path))
}
...
fn parse_iri_components(iri: &str) -> ToolResult<IriComponents> {
    let scheme_end = iri.find(':')
        .ok_or("IRI must contain a scheme")?;
    ...
    if rest.starts_with("//") {
        ...
        let authority_end = rest[authority_start..].find('/')
            .map(|pos| pos + authority_start)
            .or_else(|| rest[authority_start..].find('?').map(|pos| pos + authority_start))
            .or_else(|| rest[authority_start..].find('#').map(|pos| pos + authority_start))
            .unwrap_or(rest.len());
        ...
        Ok(IriComponents { scheme, authority, path })
    } else {
...
fn analyze_iri(iri: &str) -> ToolResult<()> {
    ...
    if let Some(scheme_end) = iri.find(':') {
        ...
        if rest.starts_with("//") {
            ...
            let authority_end = authority_part.find('/')
