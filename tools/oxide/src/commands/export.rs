...
pub async fn run(dataset: String, file: PathBuf, format: String, graph: Option<String>) -> CommandResult {
    println!("Exporting data from dataset '{}' to {}", dataset, file.display());
...
"#
        ...
        _ => {
            return Err(format!("Export format '{}' serialization not yet implemented", format).into());
        }
    };
    ...
    let triple_count = sample_content.lines().filter(|line| {
        let line = line.trim();
