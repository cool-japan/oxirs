...
pub async fn run(
    ...
    match format.as_str() {
        "tdb2" | "memory" => {},
        _ => {
            return Err(format!("Unsupported format '{}'. Supported formats: tdb2, memory", format).into());
        }
    }
    ...
    if format == "tdb2" {
        if dataset_path.exists() {
            return Err(format!("Dataset directory '{}' already exists", dataset_path.display()).into());
        }
    ...
    println!("Dataset '{}' successfully initialized at {}", name, dataset_path.display());
...
fn create_default_config(name: &str, format: &str, path: &PathBuf) -> Result<String, Box<dyn std::error::Error>> {
    let config = format!(
