...
impl RdfTerm {
    ...
    pub fn to_string_repr(&self) -> String {
        match self {
            ...
            RdfTerm::Literal { value, lang: Some(lang), .. } => format!("\"{}\"@{}", value, lang),
            RdfTerm::Literal { value, datatype: Some(dt), .. } => format!("\"{}\"^^<{}>", value, dt),
                ...
                for (idx, var) in results.variables.iter().enumerate() {
                    if let Some(Some(term)) = binding.values.get(idx) {
                        let term_json = match term {
                            ...
                            RdfTerm::Literal { value, lang: Some(lang), .. } => serde_json::json!({
                                "type": "literal",
                                "value": value,
                                "xml:lang": lang
                            }),
                            RdfTerm::Literal { value, datatype: Some(dt), .. } => serde_json::json!({
                                "type": "literal",
                                "value": value,
                                "datatype": dt
                            }),
...
impl ResultFormatter for CsvFormatter {
    fn format(&self, results: &QueryResults, writer: &mut dyn Write) -> std::io::Result<()> {
        // Write header
        let header: Vec<String> = results.variables.iter().map(|v| format!("?{}", v)).collect();
